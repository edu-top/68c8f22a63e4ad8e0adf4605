...
- [Веб-компоненты](#веб-компоненты)
  ...
  - [Теневой DOM и события](#теневой-dom-и-события)
    ...
    - [Итого](#итого-5)
  - [HTML-импорты](#html-импорты)
    - [Синтаксис и использование](#синтаксис-и-использование)
    - [Преимущества](#преимущества)
    - [Современный статус](#современный-статус)
    - [Причины отказа от использования](#причины-отказа-от-использования)
...
Веб-компоненты предоставляют встроенные возможности браузера для этого, поэтому нам больше не нужно эмулировать их. Они строятся на следующих базовых технологиях:
...
- [Шаблоны](https://html.spec.whatwg.org/multipage/scripting.html#the-template-element) (элемент `<template>`) – для создания переиспользуемых шаблонов разметки, которые не отображаются и не выполняются до клонирования.
- [Слоты](https://html.spec.whatwg.org/multipage/custom-elements.html#slots) (элемент `<slot>`) – для композиции компонентов, позволяя проецировать контент из light DOM в shadow DOM.
...
Ранее предлагалось использовать HTML-импорты ([HTML imports](https://www.w3.org/TR/html-imports/)), позволявшbt импортировать HTML-документы, CSS и JavaScript с помощью тега `<link rel="import" href="file.html">`, однако данная технология в настоящее время считается устаревшей и удалена из браузеров в пользу нативных модулей ES и веб-компонентов.
...
> Здесь `true` в `tmpl.content.cloneNode(true)` означает глубокое клонирование: создается полная копия узла `<template>` (включая все его атрибуты и все дочерние элементы — текст, другие теги, вложенные структуры), а не только сам элемент `<template>`. Если бы было `false`, скопировался бы только пустой `<template>` без содержимого.
>
> - `cloneNode(true)` (**глубокое копирование**): Копирует сам узел И все его потомки (вложенные элементы, текст и т.д.).
> - `cloneNode(false)` (**поверхностное копирование**): Копирует только сам узел, его потомки не копируются (получится пустой узел).
...
### HTML-импорты
HTML Imports — это устаревшая технология веб-компонентов, позволявшая импортировать HTML-документы, CSS и JavaScript с помощью тега `<link rel="import" href="file.html">`, которую заменили стандартными модулями ES (ES Modules) и шаблонными литералами (*template literals*). HTML Imports предлагалось использовать для загрузки и импорта HTML-модулей (включая другие веб-компоненты) через специальный тег `<link rel="import">`, однако эта спецификация не была стандартизирована всеми основными браузерами и в конечном итоге была удалена из веб-стандартов.

#### Синтаксис и использование
Тег `<link rel="import">` размещался в `<head>` или `<body>`, загружая внешний HTML-файл как импорт, доступный через JavaScript-свойство import. Для вставки содержимого использовался код вроде `document.querySelector('link[rel="import"]').import.querySelector('#element').cloneNode(true)`, который клонировал и добавлял элементы в DOM.

#### Преимущества
Импорты упрощали модульность, позволяя выносить разметку, стили (с атрибутом `scoped`) и скрипты в отдельные файлы, подобно CSS или JS, для переиспользования в проектах вроде Web Components.

#### Современный статус
Технология была экспериментальной, поддерживалась в Chrome и Opera, но удалена из спецификаций W3C и не работает в современных браузерах с 2021–2022 годов. Рекомендуемые альтернативы — ES-модули (`<script type="module">`), Shadow DOM и фреймворки вроде React или Vue для модульной разработки.

#### Причины отказа от использования
HTML Imports отказались развивать и стандартизировать из‑за сочетания технических и организационных причин у экосистемы веб‑платформы.

1. **Основные технические проблемы**

    HTML Imports дублировали задачи, которые параллельно решали ES‑модули для JavaScript и Shadow DOM/Custom Elements для разметки и инкапсуляции, поэтому спецификация получалась избыточной и пересекающейся по ответственности. Кроме того, модель загрузки и кэширования импортов была сложной: требовалось учитывать цепочки зависимостей, события готовности, ошибки, а это усложняло реализацию в браузерах и разработку кроссбраузерных библиотек.

2. **Ограниченная поддержка браузеров**

    Технологию фактически реализовал только Chrome (и на его базе, например, ранний Electron), тогда как Firefox, Safari и Edge от неё дистанцировались и не планировали внедрение. Отсутствие широкого кроссбраузерного покрытия означало, что разработчики всё равно были вынуждены полагаться на полифилы или альтернативные подходы, снижая практический смысл HTML Imports.

3. **Эволюция стандартов веб‑компонентов**

    В процессе развития веб‑компонентов рабочие группы сделали ставку на связку: Custom Elements, Shadow DOM и ES‑модули, которые дают более явную и предсказуемую модель модульности. Импорты в HTML выглядели менее универсальным решением по сравнению с модульной системой JavaScript, где можно импортировать как код, так и шаблоны, а также использовать сборщики и бандлеры.

4. **Поддержка и сопровождение**

    Поддержка ещё одного отдельного механизма загрузки ресурсов увеличивала сложность движков браузеров и спецификаций, при этом выигрыш для разработчиков был ограниченным, потому что похожие задачи уже решались другими инструментами. В итоге рабочие группы и вендоры сочли, что затраты на поддержку не оправдываются, и решили вывести HTML Imports из спецификаций и удалить из браузеров в пользу стандартизированных модулей и современных подходов к сборке фронтенда.

#### Альтернативы
Вместо нее теперь рекомендуется использовать:
- **ES Modules** (`import` и `export`) для управления зависимостями и загрузки JavaScript-файлов, которые определяют пользовательские элементы (*custom elements*).
- **Шаблонные строки** (*template literals* — строки с обратными кавычками) для удобной генерации HTML-разметки внутри JavaScript-кода компонентов, включая многострочные шаблоны и интерполяцию данных.
- **Элементы `<template>` и `<slot>`**, которые являются стандартизированными частями текущей спецификации веб-компонентов и повсеместно поддерживаются для создания переиспользуемой разметки.
- **`fetch()` API** является рекомендованным подходом для динамической загрузки из внешнего файла необработанного (raw) HTML-содержимого, которое затем может быть вставлено в DOM.
- **Серверные инструменты динамической сборки/вставки** (такие как Webpack или Rollup) и **генераторы статических сайтов** помогают добиться аналогичного эффекта включения отдельных HTML-фрагментов в общее содержимое во время процесса сборки.

### Заключение
Таким образом, ключевые технологии веб-компонентов вкючают в себя:

- **Теневой DOM** (Shadow DOM) — это технология инкапсуляции, позволяющая создавать изолированные DOM-деревья для веб-компонентов, которые не влияют на основной документ и не подвержены внешнему воздействию.
- **Шаблоны** (template) используются для описания структуры компонента, которую можно повторно использовать без негативного влияния на стили и логику страницы.
- **Слоты** (slots) позволяют передавать контент внутрь теневого дерева, обеспечивая гибкость и переиспользование компонентов.​

Слоты и композиция в теневом DOM позволяют гибко проецировать контент из light DOM внутрь shadow DOM, обеспечивая модульность веб-компонентов. Слоты (<slot>) служат плейсхолдерами для внешнего контента, распределяя его по атрибутам slot="name". Неименованные слоты заполняют контент по умолчанию. Это техника декомпозиции UI: родительский компонент передает дочерний контент через слоты, сохраняя инкапсуляцию стилей и логики. Резервное содержимое (fallback content) отображается при отсутствии проекции. Слоты упрощают создание шаблонных компонентов с кастомизируемым содержимым, минимизируя дублирование кода

Стилизация теневого DOM обеспечивает инкапсуляцию стилей, предотвращая их утечку в основной документ и влияние внешних стилей на внутренние элементы компонента. Стили добавляются внутри shadow root через тег `<style>` или объект CSSStyleSheet, применяясь только к теневым элементам.​ Внешние стили не проникают внутрь, кроме CSS-переменных и `::part()`, а слоты позволяют стилизовать контент из light DOM снаружи. Это упрощает создание переиспользуемых веб-компонентов без конфликтов CSS. Режимы `open` и `closed` контролируют доступ к теневому DOM из JavaScript.
