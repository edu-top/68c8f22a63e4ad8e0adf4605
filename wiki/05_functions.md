...
Если начальное значение аккумулятора не указано, в качестве него используется первый элемент массива, а обход начинается со второго.
...
### Замыкания
<dfn title="замыкание">Замыкание</dfn> (**closure**) представляют собой конструкцию, когда функция, созданная в одной области видимости, запоминает свое лексическое окружение даже в том случае, когда она выполняет вне своей области видимости.
...
<details>
<summary><em>О компиляции и поднятии в JS</em></summary>

Hoisting в JavaScript — это процесс, при котором объявления переменных и функций "поднимаются" (hoisted) в начало своей области видимости до выполнения кода. Хотя JavaScript часто называют интерпретируемым языком, он на самом деле проходит две фазы при работе с кодом:

1. Этап парсинга (или компиляции), когда движок JavaScript (часто называемый интерпретатором) анализирует весь код, фиксирует объявления переменных и функций без их выполнения. При этом переменные получают значение `undefined`, а функции становятся доступны.

2. Этап выполнения, когда именно запускается код.

Таким образом, в JavaScript есть внутренняя компиляция (парсинг) в байткод или промежуточное представление, после чего происходит выполнение. Поэтому в контексте hoisting обычно говорят о работе "компилятора" и "интерпретатора". Это не ошибка называть JavaScript интерпретируемым языком с компиляционной фазой, так как движок сначала компилирует код в промежуточную форму, а потом исполняет его. Hoisting объясняется именно этим двухфазным процессом.​

Таким образом, описание hoisting как процесса двух проходов с получением объявлений переменных и последующим выполнением в целом корректно. JavaScript — интерпретируемый язык с механизмом компиляции в промежуточное представление, что обуславливает поведение hoisting.
...
То есть как будто происходит поднятие кода с определением переменных и функций вверх до их непосредственного использования. Поднятие на английский переводится как hoisting, сообственно поэтому данный процесс так и называется.[^3.6]
...
5. Напишите функцию, принимающую числовые переменные и функцию обратного вызова для выполнения любой заданной пользователем математической операции над любым количеством переданных чисел. Если не передано ни одного числа, то необходимо вернуть `null`.
