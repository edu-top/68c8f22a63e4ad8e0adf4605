...
- [Основы событий](#основы-событий)
  ...
  - [Практическая работа. Обработка событий](#практическая-работа-обработка-событий)
    - [Задание](#задание)
  - [Практическая работа. Создание карусели изображений](#практическая-работа-создание-карусели-изображений)
  - [Практическая работа. Реализация поведенческих шаблонов](#практическая-работа-реализация-поведенческих-шаблонов)
    - [Задание](#задание-1)
  - [Практическая работа. Предотвращение действий по умолчанию](#практическая-работа-предотвращение-действий-по-умолчанию)
    - [Задание](#задание-2)
  ...
  - [Практическая работа. Реализация поведенческих шаблонов при помощи делегирования](#практическая-работа-реализация-поведенческих-шаблонов-при-помощи-делегирования)
    - [Задание](#задание-3)
...
Для получения текущего объекта, для которого обрабатывается событие, внутри обработчика события мы можем использовать ключевое слово **`this`**:
```html
<button id="btn">Click Me</button>
<script>
const btn = document.getElementById("btn");
...
![Исходный код](../samples/11_events/blue-red-squares.html)
...
[Тестовый стенд](../samples/11_events/test_01/index.html)
...
#### Нисходящие события
События также могут быть нисходящими (*capturing*). Для их использования в метод **`addEventListener()`** в качестве третьего необязательного параметра передается логическое значение `true` или `false`. Значение **`true`** указывает, что событие является нисходящим.
...
Всплытие и погружение являются основой для «делегирования событий» – очень мощного приёма обработки событий. Его мы изучим далее.
...
[Исходный код](../samples/11_events/bagua/index.html)
...
[Исходный код](../samples/11_events/menu-buttons.html)
...
!!! info "Альтернатива привязке"

    Вместо привязки контекста можно использовать стрелочную функцию. Стрелочная функция захватывает `this` из внешнего лексического контекста (конструктора класса), поэтому значение `this` внутри `onClick` останется правильным, как и с `bind(this)`.

    *Прямая замена на стрелочную функцию*:
      ```js
      constructor(elem) {
        elem.onclick = (event) => this.onClick(event);  // или просто this.onClick
      }
      ```

    *Или изменить метод `onClick` на стрелочный*:

    ```js
    onClick = (event) => {  // классовое поле (ES2022+)
      let action = event.target.dataset.action;
      if (action) {
        this[action]();
      }
    ...
    constructor(elem) {
      elem.onclick = this.onClick;  // без bind!
    }
    ```

    Преимущества стрелочной функции
    - **Краткость**: `(event) => this.onClick(event)` вместо `this.onClick.bind(this)`
    - **Производительность**: не создает новый объект-обертку каждый раз (в отличие от `bind`)

    - **Лексический `this`**: автоматически правильный контекст

    Оба подхода работают идентично в данном случае с делегированием событий.
...
[Исходный код](../samples/11_events/behavior-counter.html)
...
[Исходный код](../samples/11_events/behavior-toggler.html)
...
![Menu buttons](../img/menu-buttons.png)

[Исходный код](../src/11_events/menu-buttons/index.html)
...
![Browser opens w3.org](../img/browser-opens-w3org.png)
...
Всё, что нам необходимо, это поймать событие `contents.onclick` и использовать функцию confirm, чтобы задать вопрос пользователю. Хорошей идеей было бы использовать `link.getAttribute('href')` вместо `link.href` для ссылок.

[Код решения](../src/11_events/capture-transition.html)
...
[Код решения](../src/11_events/image-gallery/index.html)
...
Возможность программной генерации событий открывает нам путь к созданию собственных и кастомных событий — мы можем определять и вызывать любые произвольные события.[^9.8]
...
В переменной `_money` хранится текущее количество денег на счете. С помощью функции pay условно тратим определенную сумму, если баланс позволяет. Но, допустим, нам надо как-то извещать систему, что произошло списание со счета. С одной стороны, мы могли бы это делать непосредственно в методе `pay` — вызывать в методе `console.log()` и выводить на консоль какой-то текст.

Но на момент написания этого кода мы можем быть не уверены, какой именно текст надо выводить на консоль. А может быть потребуется и не на консоль, а в окне браузера. Или посылать извещение на определенный сетевой ресурс. А может наша функция-конструктор будет использоваться в Node.js, где может потребоваться какая-то другая обработка. Да и использовать нашу функцию-конструктор могут совсем другие разработчики, у которых может быть собственно понимание того, что надо делать при списании средств.

В любом случае мы сталкиваемся с многовариантностью, но во всех этих ситуация главное, что нам надо сделать — уведомить систему, что произошло списание средств. И охватить все эти ситуации нам поможет определение собственных событий.

Для определения собственных событий мы можем применять конструктор **`Event`**, в который передается название события. Так, рассмотрим следующую программу:
```html
<!DOCTYPE html>
<html>
<head>
...
![Создание своих событий в JavaScript](../img/customevent1_cp.png)

Также, как и в общем случае, мы можем получить объект подобного события в обработчике:
```js
...
### Заключение
Распространение и делегирование событий в JavaScript — ключевые механизмы работы с событиями в DOM. Распространение включает три фазы: **захват** (*capture*), **цель** (*target*) и **всплытие** (*bubbling*), при этом большинство обработчиков по умолчанию срабатывают на фазе всплытия. Делегирование событий использует всплытие для обработки событий дочерних элементов через их общего родителя, что повышает производительность и упрощает управление динамически добавляемыми элементами. Оба подхода позволяют писать более гибкий, эффективный и поддерживаемый код.

### Практическая работа. Обработка событий
...
1. **Скрыть элемент по нажатию кнопки**
    ...
    ![Hide element](../img/hide-element.png)
...
2. **Спрятать себя**
    ...
    Создать кнопку, которая будет скрывать себя по нажатию. Например:

    <input type="button" onclick="this.hidden=true" value="Нажмите, чтобы спрятать">

    ![Hide button](../img/hide-button.png)
...
3. **Перемещение мяча по полю**

    Пусть мяч перемещается при клике на поле, туда, куда был клик, например:

    ![Move ball field](../img/move-ball-field.png)
    ...
    - При прокрутке страницы ничего не должно ломаться.
    - Код должен уметь работать с различными размерами мяча и поля, не привязываться к каким-либо фиксированным значениям.
    - Необходимо использовать свойства `event.clientX`/`event.clientY` для определения координат мыши при клике.
...
4. **Раскрывающееся меню**
    ...
    ![Collapsible menu](../img/collapsible-section_closed.png)

    ![Collapsible menu](../img/collapsible-section_open.png)

    - Для выполнения задачи использовать JavaScript, добившись функционала, аналогичного элементу `<details>`.
    - HTML/CSS исходного документа можно и нужно менять.

    Решение приложить в виде файлов исходного кода.
...
5. **Кнопка закрытия**
    ...
    Есть список сообщений. При помощи JavaScript для каждого сообщения добавить в верхний правый угол кнопку закрытия. Результат должен выглядеть примерно следующим образом:

    ![Closing button](../img/closing-btn.png)
...
### Практическая работа. Создание карусели изображений

Создать «Карусель» — ленту изображений, которую можно листать влево-вправо нажатием на стрелочки.

![Carousel](../img/carousel-1.png)

![Carousel](../img/carousel-2.png)

В дальнейшем к ней можно будет добавить анимацию, динамическую подгрузку и другие возможности, однако конкретной в данной работе разработка структуры HTML/CSS составляет 90% решения. Конкретный вид карусели остается на усмотрение разработчика.

<details>
<summary><em>Алгоритм решения</em></summary>
...
### Практическая работа. Реализация поведенческих шаблонов

#### Задание
Выполнить набор задач, представленных ниже.

1. **Делегированное удаление**

    Дан список сообщений с кнопками для удаления `[x]`. Внешний вид аналогичен списку из Практической работы "Обработка событий":

    ![Closing button](../img/closing-btn.png)

    - Необходимо заставить кнопки работать (при нажатии сообщение должно быть удалено).
    - Использовать делегирование событий. Должен быть лишь один обработчик на элементе-контейнере для всего.
...
2. **Разворачивающееся дерево**.

    Создать древовидный список, который по клику на заголовок скрывает-показывает потомков. Требования:

    - список должен быть динамически генерируемым на основе хранимых данных;
    - использовать только один обработчик событий (применить делегирование);
    - может быть развернуто/свернуто сколько угодно элементов списка;
    - клик вне текста заголовка (на пустом месте) ничего делать не должен.

    Простейший вариант реализации:

    ![User tree](../img/user-list_1.png)
    ![User tree](../img/user-list_2.png)
    ![User tree](../img/user-list_3.png)

    <details>
    <summary>Решение</summary>

    [Код решения](../src/11_events/user-tree.html)
...
3. **Сортируемая таблица**.
    ...
    Создать сортируемую таблицу: при клике на элемент `<th>` строки таблицы должны сортироваться по соответствующему столбцу. Каждый элемент `<th>` снабдить атрибутом `data-type`, указывающим на тип данных (строки или числа), который должен учитываться функцией сортировки. Учитывать, что таблица является динамически генерируемой и может быть большой, с любым числом строк и столбцов. Требования:

    - сделать подсветку на заголовке при наведении мышки;
    - реализовать маркер, указывающий направление сортировки;
    - сортируемыми должны быть все столбцы таблицы;
    - учитывать, что сортировка возможна только по одному столбцу.

    Простейший вариант реализации:

    ![User table](../img/user-table_1.png)
    ![User table](../img/user-table_2.png)
    ![User table](../img/user-table_3.png)
    ![User table](../img/user-table_4.png)

    <details>
    <summary>Решение</summary>

    [Код решения](../src/11_events/user-table.html)

    </details><br>

4. **Поведение подсказка**.

    Реализовать универсальные подсказки на конечных элементах древовидного списка из задачи 2 с выводом отформатированных пользовательских данных. При наведении мыши на элемент с атрибутом `data-tooltip`, над ним должна показываться подсказка и скрываться при переходе на другой элемент. Детали оформления:

      - Отступ от подсказки до элемента с `data-tooltip` должен быть `5px` по высоте.
      - Подсказка должна быть, по возможности, посередине элемента (с учетом ширины его видимого содержимого, т.е. для блочных элементов необязательно в середине экрана).
      - Подсказка не должна вылезать за границы экрана, в том числе если страница частично прокручена, если нельзя показать сверху – показывать снизу элемента.
      - Текст подсказки брать из значения атрибута `data-tooltip`. Это может быть произвольный HTML.
      - В один момент может быть показана только одна подсказка.

    Примерный внешний вид реализации:

    ![User tree tooltip](../img/user-tree-tooltip.png)

    Применить делегирование событий: установить оба обработчика на элемент `document`, чтобы отслеживать «заход» и «уход» курсора на элементы с атрибутом `data-tooltip` и управлять подсказками с их же помощью. Для решения понадобятся два события:
    - `mouseover` срабатывает, когда указатель мыши заходит на элемент.
    - `mouseout` срабатывает, когда указатель мыши уходит с элемента.

    После реализации поведения люди, даже не знакомые с JavaScript должны без проблем добавлять подсказки к элементам.

    <details>
    <summary>Решение</summary>

    - [JS](../src/11_event/data-tooltip.js)
    - [CSS](../src/11_event/data-tooltip.css)

    </details><br>

5. **Динамическая подсказка**.

    Реализовать универсальные подсказки на столбцах таблицы для показа направления сортировки. Текст подсказки должен быть динамическим и учитывать текущее состояние сортировки, показывая направление предполагаемой сортировки.

    Примерный внешний вид реализации:

    ![User table tooltip](../img/table-tooltip-sort-asc.png)
    ![User table tooltip](../img/table-tooltip-sort-desc.png)

Задачи представить в виде файлов исходного кода (*.html*/*.js*/*.css*). Прикладывать архивированные файлы в качестве ответа запрещено.

### Практическая работа. Предотвращение действий по умолчанию
...
1. **Исправить код**
    ...
    Браузер переходит по указанной ссылке, но нам этого не нужно. Предоставить исправленный вариант.
...
2. **Поймать переход по ссылке**

    Сделать так, чтобы при клике на ссылки внутри элемента `id="contents"` пользователю выводился вопрос о том, действительно ли он хочет покинуть страницу, и если он не хочет, то прерывать переход по ссылке.

    - Содержимое `#contents` может быть загружено динамически и присвоено при помощи `innerHTML`. Так что найти все ссылки и поставить на них обработчики нельзя. Здесь необходимо использовать делегирование.
    - Содержимое может иметь вложенные теги, *в том числе внутри ссылок*, например, `<a href=".."><i>...</i></a>`.
...
3. **Галерея изображений**
    ...
    Создать галерею изображений, в которой основное изображение изменяется при клике на уменьшенный вариант. При решении задачи необходимо использовать делегирование. Примерный вид:

    ![Image gallery](../img/image-gallery-example.png)
...
Решения предоставить в виде файлов исходного кода.
...
### Практическая работа. Реализация поведенческих шаблонов при помощи делегирования

#### Задание
Выполнить рефакторинг и дополнить страницу администратора новым функционалом, решив задачи, приведенные ниже.

1. **Делегированное удаление**. Переделать удаление элементов из динамически генерируемого списка пользователей в виде карточек на странице администратора с использованием делегирования событий. Должен быть лишь один обработчик на элементе-контейнере для всего.

2. **Разворачивающееся дерево**. Добавить вариант отображения пользователей в виде древовидного списка,  который по клику на заголовок скрывает-показывает потомков. Требования:

   - список должен быть динамически генерируемым на основе хранимых данных о пользователях;
   - использовать только один обработчик событий (применить делегирование);
   - может быть развернуто/свернуто сколько угодно элементов списка;
   - клик вне текста заголовка (на пустом месте) ничего делать не должен.
...
3. **Сортируемая таблица**. Добавить вариант отображения пользователей в виде таблицы. Сделать таблицу сортируемой: при клике на элемент `<th>` строки таблицы должны сортироваться по соответствующему столбцу. Каждый элемент `<th>` снабдить атрибутом `data-type`, указывающим на тип данных (строки или числа), который должен учитываться функцией сортировки. Учитывать, что таблица является динамически генерируемой иможет быть большой, с любым числом строк и столбцов. Требования:
...
4. **Поведение подсказка**. Реализовать универсальные подсказки на конечных элементах дерева пользователей с выводом отформатированных пользовательских данных. При наведении мыши на элемент с атрибутом `data-tooltip`, над ним должна показываться подсказка и скрываться при переходе на другой элемент. Детали оформления:
...
5. **Динамическая подсказка**. Реализовать универсальные подсказки на столбцах таблицы для показа направления сортировки. Текст подсказки должен быть динамическим и учитывать текущее состояние сортировки, показывая направление предполагаемой сортировки.
