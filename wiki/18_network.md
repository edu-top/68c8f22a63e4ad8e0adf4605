...
- [Объекты URL](#объекты-url)
  ...
  - [Кодирование](#кодирование)
    - [Кодирование в строках](#кодирование-в-строках)
...
!!! info "Разница в кодировании с URL"
...
Одним из способов для отправки ajax-запросов является применение объекта **`XMLHttpRequest`**. <dfn title="XMLHttpRequest">`XMLHttpRequest`</dfn> – это встроенный в браузер объект, который даёт возможность делать HTTP-запросы к серверу без перезагрузки страницы.
...
Что-то из этого списка звучит знакомо? Если да, тогда вперёд, приятного знакомства с `XMLHttpRequest`. Если же нет, возможно, имеет смысл изучать сразу Fetch.[^xmlhttprequest]

Данный объект создается с помощью одноименной функции-конструктора:
```js
...
- `setRequestHeader(name, value)`
    ...
    !!! warning "Ограничения на заголовки"
        Некоторые заголовки управляются исключительно браузером, например `Referer` или `Host`, а также ряд других. Полный список [тут](https://fetch.spec.whatwg.org/#forbidden-request-header).

        `XMLHttpRequest` не разрешено изменять их ради безопасности пользователей и для обеспечения корректности HTTP-запроса.

    !!! warning "Поставленный заголовок нельзя снять"
...
`response.headers` — это объект типа `Headers`, который реализует итератор `Map`‑подобного интерфейса, поэтому с ним можно работать почти так же, как с обычным `Map`.

Помимо `get()` и итерации через `for…of`, у `Headers` есть и другие методы:
- `has(name)` — проверяет, есть ли заголовок с таким именем.
...
- `forEach((value, name) => ...)` — удобный способ перебрать все заголовки, не используя деструктуризацию в цикле `for…of`.
...
- `keys()` и `values()` — возвращают итераторы по именам и значениям соответственно, если нужно работать только с одной частью.
...
!!! warning "Особенность заголовков в Fetch"

    Не все заголовки доступны клиентскому коду из‑за ограничений браузера (например, `Set-Cookie` и некоторые защищённые поля обычно не возвращаются в `response.headers`).

    Если серверу нужно передать клиенту дополнительную мета‑информацию, принято использовать кастомные заголовки вида `X-…` или `X‑My‑App‑Info`, которые браузер не фильтрует.
...
!!! info "Политика установки Referer (Referrer Policy) – не только для fetch"
...
- **`"include"`** – отправлять всегда, но при этом необходим заголовок `Access-Control-Allow-Credentials` в ответе от сервера, чтобы JavaScript получил доступ к ответу сервера, об этом говорилось в разделе "Fetch: запросы на другие сайты",
...
[^xmlhttprequest]: [XMLHttpRequest](https://learn.javascript.ru/xmlhttprequest)
