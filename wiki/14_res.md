...
```html
<!-- Обычный скрипт (нестрогий режим по умолчанию) -->
<script>
  a = 5;        // работает, создаёт глобальную переменную
  console.log(a); // 5
</script>

<!-- Модуль (строгий режим всегда) -->
<script type="module">
  a = 5;        // ReferenceError: a is not defined
</script>
```

Что блокируется в строгом режиме модулей
- присваивание необъявленным переменным

- дублирование параметров функции: `function fn(a, a) {}`

- `delete` для переменных: `delete a`

- октальные литералы: `018 → SyntaxError`

- глобальный `this = undefined` (вместо `window`)

Практический пример
```js
// Модуль автоматически строгий
export function strictExample() {
  "use strict"; // эта строка НЕ нужна и избыточна
...
Таким образом, режим `use strict` в модулях ES6 всегда активен **автоматически** — без явного указания директивы `"use strict";`. Поэтому в `type="module"` никогда не нужно писать `"use strict";` — это дублирование и плохой стиль.
...
В модулях ES (`type="module"`) верхнеуровневый `this` действительно всегда равен `undefined`, а не глобальному объекту (`window` в браузере или `global` в Node.js).

Это задумано языком и стандартом: код модуля исполняется в своего рода «обёрнутом» контексте, а не в глобальном, поэтому на верхнем уровне `this` не привязывается к глобальному объекту.

Такое поведение делает модули более предсказуемыми и изолированными и соответствует модели, в которой модульный код не загрязняет глобальную область видимости.

!!! warning "Важное уточнение про `this` внутри функций"

    Несмотря на то что `this` на верхнем уровне модуля — `undefined`, внутри функций `this` по‑прежнему определяется местом вызова (как в обычных скриптах):

    - у метода объекта `this` будет ссылаться на этот объект;

    - у обычной функции вне объекта в нестрогом режиме `this` может быть глобальным объектом, а в строгом — `undefined`.

    Так что особенность с `this` относится именно к **верхнему уровню модуля**, а не ко всему коду внутри модуля.
...
Затем после оператора **`from`** указывается модуль, из которого идет импорт. В данном случае указываем "./message.js", при этом предполагается, что оба модуля — *main.js* и *message.js* — будут находиться в одной папке.
...
![Загрузка и выполнение модулей в JavaScript и Node.js](../img/server-started.png)
...
Здесь мы просто импортируем модуль *message.js*. В данном случае предполагается что оба модуля — *main.js* и *message.js* — будут находиться в одной папке, поэтому при импорте указан путь "./message.js", где "./" указывает на ту же папку, где расположен файл *main.js*.
...
Обратите внимание: выполнение *main.js* ждет завершения асинхронной операции в *message.js* благодаря top-level await. Это особенно полезно для:

- инициализации данных из API;

- загрузки конфигурации;

- предварительной обработки ресурсов.

```js
// Практический пример message.js
const config = await fetch('/api/config').then(r => r.json());
const data = await processData(config);

console.log("Модуль message.js инициализирован с данными:", data);
```

Такой импорт идеален для **side-effect модулей** — тех, что выполняют инициализацию, логирование или настройку без необходимости возвращать значения.

### Экспорт и импорт компонентов модулей

#### Именной экспорт компонентов модуля
Для того, чтобы какие-то компоненты модуля (переменные/константы/функции/классы) модуля можно было подключить и использовать в другом модуле, их надо экспортировать. Мы можем экспортировать каждый компонент по отдельности. Для этого перед определением компонента указывается ключевое слово `export`. Например, пусть у нас будет следующий модуль ***message.js***
```js
// экспорт переменной
...
!!! question "По какой причине это возможно?"

    **Ответ**: `export` можно расположить выше объявления функций, потому что JavaScript-модули (ES6) используют механизм подъема (*hoisting*) для именованных экспортов. 

!!! info "Механизм подъема внутри модулей"

    В модулях все объявления `function` поднимаются (*hoist*) в начало области видимости модуля перед выполнением кода. Это позволяет ссылаться на функцию до её объявления в коде.

    ```js
    export {sayHi};     // ✅ Работает!

    function sayHi() {  // Объявление поднимается вверх
      alert('Hi!');
    }
    ```

    Движок сначала обработает все `function sayHi() {...}`, затем выполнит `export`.

    Почему именно в модулях?
    - Обычные функции в скриптах тоже hoist'ятся, но `export` — это директива модуля.

    - Модули создают единое лексическое окружение (*lexical scope*), где hoisting работает последовательно: сначала объявления функций (*function declarations*), потом код.

    - `var` тоже поднимаются, но с `undefined` до инициализации; объявления функций (*function declarations*) — полностью.
    ...
    *Практические примеры*
    ...
    // ✅ Работает (named export)
    export function sayHi() { alert('Hi!'); }

    // ❌ Ошибка (let/const НЕ hoist'ятся полностью)
    let message = 'Hi';
    export {message};  // ReferenceError при выполнении
    ```

    Такой подход удобен для реэкспорта и группировки экспортов в конце модуля.

Таким образом, в качестве альтернативы мы могли бы экспортировать все компоненты вместе в виде списка:
```js
let welcome = "Welcome";
const hello = "Hello";
...
1. Современные инструменты сборки ([webpack](https://webpack.js.org/), [vite](https://vitejs.dev/), [esbuild](https://esbuild.github.io/) и другие) собирают модули вместе и оптимизируют их, ускоряя загрузку и удаляя неиспользуемый код.
    ...
    …Тогда оптимизатор увидит, что другие функции не используются, и удалит остальные из собранного кода, тем самым делая код меньше. Это называется «встряхиванием дерева» (*tree-shaking*).

    А если бы мы написали:
    ```js
    import * as say from './say.js';
    say.sayHi(); // остальные функции тоже попадут в бандл!
    ```

    ...то tree-shaking бы не сработал — в этом случае весь модуль уйдёт в финальный файл целиком.

    !!! info О tree-shaking

        <dfn title="втряхивание дерева">Встряхивание дерева</dfn> (*tree-shaking*) — метод оптимизации в JavaScript, который автоматически удаляет неиспользуемый код («мертвый код») из итогового бандла во время сборки. Он позволяет значительно уменьшить размер файлов, ускоряя загрузку приложений, и работает за счет статического анализа ES6 модулей.

        **Как это работает**: сборщики (Webpack, Rollup, Vite) анализируют импорты и экспорты. Если функция или переменная импортирована, но не используется, она считается «сухой веткой» и удаляется.

        **Условия работы**: необходима система модулей ES6 (`import`/`export`), так как она позволяет проводить статический анализ, в отличие от динамического `require()`.

        **Преимущества**: уменьшение размера финального JS-файла, повышение производительности, снижение времени парсинга кода браузером.

        **Инструменты**: популяризирован [Rollup](https://github.com/rollup/rollup), активно используется в [Webpack](https://webpack.js.org/guides/tree-shaking/) и [Vite](https://vite.dev/).

2. Явно перечисляя то, что хотим импортировать, мы получаем более короткие имена функций: `sayHi()` вместо `say.sayHi()`.

3. Явное перечисление импортов делает код более понятным, позволяя увидеть, что именно и где используется. Это упрощает поиск зависимостей, поддержку и рефакторинг кода.

4. Избежание конфликтов имён. При импорте всего сразу (`import * as say`) все экспорты попадают в один объект. Если в проекте есть похожие имена из разных модулей, возникнут коллизии:
    ...
    // api.js
    export function validate() { ... } // Имя совпадает!

    import * as utils from './utils.js';
    import * as api from './api.js';
    utils.validate === api.validate // true — конфликт!
    ```

5. Лучшая поддержка автодополнения в IDE. При именованных импортах (*named imports*) редакторы (VS Code, WebStorm) точно знают типы и сигнатуры каждой функции, предлагая точные подсказки. При `import *` приходится лезть в объект.

6. Явность = меньшая магия. Код с именованными импортами самодокументируется — любой разработчик сразу видит зависимости. `import *` прячет детали реализации.

**Вывод**: именованные импорты — это не прихоть, а стандарт современной практики. Они дают оптимизацию, читаемость и предсказуемость.
...
Кроме того, здесь определена функция `printMessage()`. Однако из модуля также импортируется компонент с таким же именем. И чтобы избежать двойственности, импортируемому компоненту назначается псевдоним `printHello`: `printMessage as printHello`.

Далее для обращения к импортированным компонентам с псевдонимами используются их псевдонимы.[^19.5]
...
Стоит отметить, что в файле может быть не более одного `export default`.
...
Таким образом:
1. В одном модуле может быть только один `export default`, но сколько угодно именованных экспортов.
2. И при импорте, и при экспорте можно переименовывать сущности через `as`. Если нужно импортировать все именованные экспортов модуля (но не по умолчанию, или дополнительно к нему), можно использовать `* as`.

Часто в проектах придерживаются правила:

- один «главный» экспорт по умолчанию (например, класс компонента или фабричная функция),

- всё остальное — именованные экспорты.

Так код остаётся читаемым и удобным для повторного использования.
...
Чаще всего `default` в явном виде используют именно при импорте, чтобы переименовать дефолтный экспорт.

Как правило, в прикладном коде:

- пишут `export default` непосредственно перед «главной» сущностью,

- используют `as default` в `export { ... }` и `default as ...` в `import` только при необходимости переименования или демонстрации механизма.

Такой код остаётся максимально читаемым и привычным для большинства разработчиков.
...
// в любом файле:
import {User} from './user.js';  // всегда одно имя
...
- нет путаницы с именами;

- легко рефакторить (переименовать экспорт → все импорты обновятся);

- tree-shaking работает лучше.

Недостатки:

- больше кода при импорте: `{ User }` вместо `User`;

- менее читаемо при множественных импортах из одного файла.
...
- короче и чище синтаксис;

- удобно для основного экспорта модуля.

Недостатки (если нет строгого правила):
```js
😈 import anyName from './user.js';  // Хаос в команде
```
...
Таким образом, можно сформулировать определенные практические рекомендации для команд:

***Вариант 1**: Только именованные экспорты*
```js
// .eslintrc.js
{
  "rules": {
...
***Вариант 2**: Default + правило именования*
```json
// package.json или eslint
{
  "import/order": ["error"],
  "rule": "импорт = имя файла (camelCase)"
}
```

***Вариант 3**: Комбинированный (рекомендуемый)*
```js
// ✅ Хорошо: основной default + именованные утилиты
export default class User { ... }
...
import { useState, useEffect } from 'react'; // именованные хуки

// Lodash (только именованные)
import { debounce, throttle } from 'lodash';

// Типично для утилит
import debounce from 'lodash/debounce'; // тоже работает!
```

**Вывод**: Для небольших команд/проектов — `default` с правилом именования. Для больших команд — именованные экспорты или строгий ESLint.
...
Всё это следствие того, что цель директив `import`/`export` – задать костяк структуры кода. Благодаря им она может быть проанализирована, модули могут быть собраны в один файл специальными инструментами, а неиспользуемые экспорты удалены (*tree-shaking*). Это возможно только благодаря тому, что всё статично.

Но как мы можем импортировать модуль динамически, по запросу?[^modules-dynamic-imports]

#### Динамические импорты

Модули можно загружать динамически в любом месте другого модуля. В этом случае надо помнить, что модуль загружается асинхронно, а результат выражения **`import`** будет представлять объект `Promise`, результатом которого собственно и будет загружаемый модуль.
...
Динамическая загрузка модулей открывает нам возможность загружать модули в различных ситуациях, в том числе и в функциях, циклических и условных конструкциях и в других частях программы. Например, мы могли бы загружать модуль в зависимости от условия:
```js
...
В данном случае получаем текущий час. И если он меньше 17, то загружаем модуль и выводим значение константы `hello` из загруженного модуля. Иначе выводим некоторую строку.[^19.6]
...
Модульный подход позволяет использовать современные механизмы импортов/экспортов ES-модулей (`import`/`export`), улучшает читаемость и облегчает масштабирование приложения. При этом каждая часть имеет собственную четко ограниченную сферу ответственности и изолирована от других, что снижает связанность кода. Высокая сплочённость кода внутри модулей и низкая связанность между модулями облегчает сопровождаемость кода и снижает вероятность ошибок при изменениях.

**Использование модулей в JavaScript имеет следующие преимущества**:

- Модули позволяют разбивать сложный код на независимые, логически связанные части, что облегчает понимание, поддержку и развитие программного обеспечения.

- Каждый модуль изолирует свои переменные и функции, предотвращая конфликты имен в глобальной области видимости.

- Модули способствуют повышению повторного использования кода, так как их можно импортировать и применять в разных частях приложения или даже в разных проектах.

- Благодаря модульности становится проще управлять зависимостями между частями кода и контролировать их обновления.

- Современный стандарт ES6 поддерживает удобный синтаксис import/export, который упрощает организацию и структуру кода, а также поддерживает асинхронную загрузку модулей для повышения производительности веб-приложений.

Таким образом, модули делают разработку в JavaScript более структурированной, масштабируемой и удобной как для индивидуальной, так и командной работы.
...
: метод оптимизации в JavaScript, который автоматически удаляет неиспользуемый код («мертвый код») из итогового бандла во время сборки. Он позволяет значительно уменьшить размер файлов, ускоряя загрузку приложений, и работает за счет статического анализа ES6 модулей. Сборщики (Webpack, Rollup, Vite) анализируют импорты и экспорты. Если функция или переменная импортирована, но не используется, она считается «сухой веткой» и удаляется. Для этого необходима система модулей ES6 (`import`/`export`), так как она позволяет проводить статический анализ, в отличие от динамического `require()`. Преимущества: Уменьшение размера финального JS-файла, повышение производительности, снижение времени парсинга кода браузером.
...
[^19.6]: [Динамическая загрузка модулей](https://metanit.com/web/javascript/19.6.php)
