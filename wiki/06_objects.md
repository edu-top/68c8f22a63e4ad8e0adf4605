...
- [Объекты](#объекты)
  ...
  - [Ключевое слово this](#ключевое-слово-this)
    ...
    - [Привязка контекста при вызове](#привязка-контекста-при-вызове)
    - [Явная привязка контекста при создании функции (метод bind)](#явная-привязка-контекста-при-создании-функции-метод-bind)
    - [Контекст в стрелочных функциях](#контекст-в-стрелочных-функциях)
      - [Стрелочные и обычные функции](#стрелочные-и-обычные-функции)
    - [Динамическая природа контекста в JavaScript](#динамическая-природа-контекста-в-javascript)
      - [Исторические причины](#исторические-причины)
      - [Практические цели](#практические-цели)
    - [Полезные случаи использования](#полезные-случаи-использования)
    - [Эволюция и современные альтернативы](#эволюция-и-современные-альтернативы)
    - [Выводы](#выводы)
...
В стандарт ES2020 было добавлено определение объекта **`globalThis`**, который позволяет ссылаться на глобальный контекст вне зависимости, в какой среде и в какой ситуации выполняется код:
```js
...
Здесь определена глобальная переменная `bar` ("global_bar"). И также в функции `foo` определена локальная переменная `bar` (`"foo_bar"`). Значение какой переменной будет выводиться в функции `foo`?

Функция `foo` использует определение переменной, которое сделано во внешнем контексте. Для функции `foo` по умолчанию это *глобальный контекст*, поэтому она выводит значение *глобальной переменной* (так как данный скрипт запускается в нестрогом режиме, а значит ключевое слово `this` в функции `foo` ссылается на внешний контекст).
...
Подобное поведение может привести к некоторому непониманию в отдельных случаях. Так, рассмотрим другую ситуацию:
```js
var bar = "global_bar";
...
Однако функция `obj1.foo` также будет искать значение для `this.bar` опять же во внешнем контексте, а здесь это объект `obj2`. А в объекте `obj2` это значение равно `"obj2_bar"`.
...
#### Явная привязка контекста при создании функции (метод bind)
...
Метод `bind()` позволяет создать новую функцию с привязанным контекстом `this` и, опционально, начальными аргументами. Он не выполняет функцию сразу, а возвращает её копию для последующего вызова.
...
Первый параметр — значение `this` для новой функции, остальные — частичные аргументы.
...
Здесь `this` в `greetJohn` всегда ссылается на `person`.

*Применение в DOM*
```js
let elem = document.querySelector('#elem');
function logValue(param) {
  console.log(this.value + param);
}
let boundFunc = logValue.bind(elem);
boundFunc('123'); // text123 (если value="text")
```

Полезно для обработчиков событий, где `this` меняется.
...
Фиксирует первые аргументы.

**Особенности**
- Повторный `bind` не меняет уже привязанный `this`.
​
- Возвращаемая функция — "привязанная" (*bound function*), с фиксированным контекстом.
​
- Часто используется в колбэках для сохранения `this` в методах объектов.
...
Раньше для сохранения контекста чаще всего определяли переменную `self`, позволяющую зафиксировать необходимый контекст для дальнейшего использования.
...
Контекстом для стрелочной функции в данном случае будет выступать контекст объекта `school`. Соответственно, нам не надо определять дополнительные переменные для передачи данных в функцию.[^4.10]

##### Стрелочные и обычные функции

В обычных функциях `this` определяется динамически — из контекста вызова (как и где функция была вызвана), а в стрелочных функциях `this` захватывается лексически — из области видимости, где функция была объявлена.

- **Обычные функции**

    Значение `this` зависит от способа вызова: через объект (`obj.method()`), `call`/`apply`/`bind`, `new` или глобально (обычно `window`/`undefined` в `"use strict"`).
    ...
    *Пример*:
    ...
    setTimeout(obj.say, 0); // undefined (this меняется на window)
    ```

- **Стрелочные функции**

    Нет собственного `this` — берётся из внешнего лексического контекста на момент объявления. Не меняется при вызове или `bind`.

    *Пример*:
    ```js
    const obj = {
        name: 'Obj',
        say: () => console.log(this.name)  // this из внешнего контекста (window)
    };
    obj.say(); // undefined
    ```
...
-- | -- |  --
`this` | Динамический (вызов) | Лексический (объявление)
​`bind(obj)` | Привязывает `this` | Игнорирует, берёт внешний
​`new` | Работает (конструктор) | Ошибка
Обратные вызовы (`setTimeout`) | `this` теряется | Сохраняет внешний `this`

#### Динамическая природа контекста в JavaScript
В JavaScript ключевое слово `this` является динамическим, поскольку его значение определяется не во время создания функции (лексически), а во время её вызова — в зависимости от контекста выполнения.

Динамическое поведение ключевого слова `this` в JavaScript обусловлено его природой как ссылки на контекст выполнения, который определяется в момент вызова функции, а не в момент её объявления. Это решение было принято для обеспечения гибкости, полиморфизма и возможности повторного использования кода в динамической среде JavaScript. Соответственно, `this` в JavaScript динамический по историческим и практическим причинам.

##### Исторические причины
JavaScript создавался под влиянием языков с динамической диспетчеризацией методов (Self, Smalltalk). В них `this` (или аналог) всегда определяется контекстом вызова — это базовый принцип объектно‑ориентированного исполнения.

Это решение отражает динамическую природу языка, вдохновлённую функциональным программированием и самодостаточными объектами (*self-contained*), инкапсулирующими в себе все данные и методы. Главный принцип, лежащий в основе концепции, — независимость и способность функционировать без внешней помощи. Это позволяет функциям быть универсальными методами, которые автоматически адаптируются к разным объектам без дублирования кода, поддерживая полиморфизм и объектно-ориентированный стиль.

- **Динамическая природа языка**. JavaScript изначально разрабатывался как гибкий и динамический язык. Динамический `this` соответствует этой философии, позволяя коду адаптироваться к различным ситуациям выполнения.

- **Влияние Java** (синтаксически): JavaScript создавался как "легковесный" язык для браузера, и синтаксис с `this` был заимствован из Java для привычности.

- **Прототипное наследование**: в отличие от классического ООП, где методы жестко привязаны к классам, в JavaScript методы могут быть переданы и переиспользованы.

##### Практические цели

- **Гибкость контекста выполнения**: в JS методы не «прикреплены» жёстко к объекту, поэтому один и тот же метод можно использовать с разными объектами.

    Динамический `this` позволяет функциям адаптироваться к контексту вызова. Одна и та же функция может вести себя по-разному в зависимости от того, какой объект её вызывает. Это особенно полезно в объектно-ориентированном программировании, где важна **гибкость и адаптивность функций**, позволяющая методам работать с данными разных объектов

- **Позднее связывание** (*late binding*): контекст определяется в момент вызова, что позволяет создавать обобщенные функции.

    Динамический `this` способствует реализации <dfn title="полиморфизма">полиморфизма</dfn> — способности объектов одного типа использовать методы других типов. Это позволяет писать более абстрактные функции, которые работают с данными текущего контекста.

- **Прототипное наследование**: методы в цепочке прототипов работают с данными конкретного экземпляра. Это способствует **сокращению дублирования кода**.

    С помощью `this` можно создавать универсальные методы, которые не зависят от конкретного имени объекта. Это особенно актуально при работе с функциями-конструкторами, где `this` ссылается на новый экземпляр объекта, позволяя создавать множество однотипных объектов с помощью одной функции.

#### Полезные случаи использования

1. **Методы объектов**. Когда функция вызывается как метод объекта (через точечную нотацию), `this` автоматически ссылается на этот объект. Это позволяет методу получать доступ к свойствам и другим методам объекта без явного указания его имени. Так, в `user.sayHi()` ссылка `this` автоматически указывает на `user`, делая метод переиспользуемым. Например:
    ...
    console.log(product.getTotalPrice()); // 1200
    ```

2. **Функции-конструкторы**. При вызове функции с ключевым словом `new`, `this` ссылается на новый создаваемый объект. Это позволяет инициализировать свойства и методы экземпляра. Так, в `new Func()` ссылка `this` становится новым экземпляром. Например:
    ...
    const spokane = new Dog('Shiba Inu', 'Cheems');
    console.log(spokane.name); // Cheems
    ```

3. **Повторное использование методов**. Динамический `this` дает возможность использовать одну и ту же функцию в контексте разных объектов.
...
4. **Обработчики событий DOM**. В обычных функциях, используемых как обработчики событий, `this` ссылается на элемент, к которому привязан обработчик. Это позволяет внутри обработчика получить доступ к свойствам элемента. Данное свойство позволяет, например, легко организовать **делегирование событий**:
    ...
    Таким образом, в обработчиках DOM `this` ссылается на элемент, упрощая доступ к его свойствам.

5. **Полиморфное поведение**. Динамический `this` позволяет писать функции, которые могут работать с разными типами данных в зависимости от контекста. Например, одна и та же функция может обрабатывать разные объекты с одинаковыми методами. Функции вроде `Array.prototype.slice` работают с любым массивоподобным объектом через `this`, что делает ее **гибкой утилитой**. Таким образом можно реализовать **архитектурный шаблон "фабрика"**:
...
6. **Коллбэки и асинхронные операции**. Динамический `this` позволяет функциям-коллбэкам адаптироваться к контексту, в котором они вызваны. Однако в таких случаях часто используют стрелочные функции или методы `bind()`, `call()`, `apply()` для явного управления контекстом. Пример ниже демонстрирует явную передачу необходимого контекста вторым параметром в метод трансформации:
...
- **Стрелочные функции**: имеют лексический `this`, берут его из окружающего контекста

- **Классы**: более строгий подход к ООП
...
const obj = {
  value: 42,
  getValue: () => this.value, // берет this из внешнего контекста
  getValueTraditional: function() {
    return this.value; // берет this от obj
  }
};
```

#### Выводы
Динамический `this` может вызывать путаницу, особенно у новичков, так как его значение зависит от способа вызова функции, а не от места её объявления. Это может приводить к ошибкам, например, при передаче метода как коллбэка, когда `this` теряется. Для решения таких проблем существуют инструменты вроде `bind()`, стрелочных функций или явного вызова через `call()`/`apply()`.

В строгом режиме (`"use strict"`) поведение `this` меняется: при вызове функции без контекста `this` становится `undefined`, что чаще вызывает ошибки, но при этом помогает выявлять неожиданное или непредсказуемое поведение программы.

Таким образом, динамический `this` — это инструмент, который обеспечивает гибкость и мощность JavaScript, но требует понимания контекста выполнения для корректного использования. Динамический `this` — это компромисс между гибкостью и предсказуемостью. Хотя он может быть источником ошибок (особенно для новичков), он дает мощные возможности для:

- метапрограммирования;
- создания переиспользуемого кода;
- реализации различных паттернов проектирования.

В современном JS рекомендуется:

1. Использовать стрелочные функции там, где не нужен динамический контекст

2. Явно привязывать контекст через `.bind()` при необходимости

3. Использовать классы для классического ООП

4. Всегда понимать, в каком контексте выполняется функция
