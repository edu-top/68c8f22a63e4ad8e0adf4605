...
- [Объект RegExp](#объект-regexp)
  - [Регулярные выражения](#регулярные-выражения)
    - [test. Проверка соответствия строки шаблону](#test-проверка-соответствия-строки-шаблону)
    - [Метод exec](#метод-exec)
    - [Группы символов](#группы-символов)
  - [Синтаксис регулярных выражений](#синтаксис-регулярных-выражений)
    - [Определение классов символов](#определение-классов-символов)
    - [Метасимволы](#метасимволы)
    - [Ограничение применения регулярных выражений](#ограничение-применения-регулярных-выражений)
    - [Флаги выражений](#флаги-выражений)
      - [Флаг i. Регистр символов](#флаг-i-регистр-символов)
      - [Флаг s](#флаг-s)
      - [Комбинация флагов](#комбинация-флагов)
  - [Квантификаторы в регулярных выражениях](#квантификаторы-в-регулярных-выражениях)
    - [Необязательные символы](#необязательные-символы)
    - [Произвольное количество символов](#произвольное-количество-символов)
    - [Символ встречается как минимум один раз](#символ-встречается-как-минимум-один-раз)
    - [Точное количество вхождений](#точное-количество-вхождений)
    - [Определение минимального количества вхождений](#определение-минимального-количества-вхождений)
    - [Определение минимального и максимального количества вхождений](#определение-минимального-и-максимального-количества-вхождений)
  - [Поиск в строке](#поиск-в-строке-1)
  - [Группы в регулярных выражениях](#группы-в-регулярных-выражениях)
    - [Определение групп в регулярных выражениях](#определение-групп-в-регулярных-выражениях)
    - [Именнованные группы](#именнованные-группы)
    - [Утверждения](#утверждения)
  - [Регулярные выражения в методах String](#регулярные-выражения-в-методах-string)
    - [Метод match](#метод-match)
    - [Разделение строки. Метод split](#разделение-строки-метод-split)
    - [Поиск в строке. Метод search](#поиск-в-строке-метод-search)
    - [Замена. Методы replace](#замена-методы-replace)
- [Символы](#символы)
...
- [Локализация](#локализация)
  ...
  - [Int.Collator и сравнение строк](#intcollator-и-сравнение-строк)
    - [Настройка сравнения](#настройка-сравнения)
...
- `isNaN()`: определяет, является ли объект числом. Если объект не является числом, то возвращается значение `true`:
  ...
  Здесь `Number.isNaN("hello")` возвращает `false`, потому что этот метод проверяет только значения типа `Number` и возвращает `true`, если значение именно `NaN`. Строка "hello" не является числом и, следовательно, не равна `NaN`, поэтому метод возвращает `false`.

  Важно понимать отличие от глобальной функции `isNaN()`, которая сначала пытается привести аргумент к числу (коэрцирует), а потом проверяет результат. В случае с `Number.isNaN()` преобразования не происходит — метод просто проверяет, является ли переданное значение числом `NaN`. Поскольку "hello" — это строка, а не `NaN`, возвращается `false`.
  ...
  Ключ в том, что `Number.isNaN()` не приводит аргументы к числу перед проверкой. Он просто проверяет, является ли тип аргумента числом и равно ли он `NaN`. Поэтому строки или другие типы, не равные `NaN`, возвращают `false`. Это поведение отличает `Number.isNaN()` от глобальной функции `isNaN()`, которая сначала преобразует аргумент к числу, а затем проверяет, является ли результат `NaN`.
...
<dfn title="регулярные выражения">Регулярные выражения</dfn> представляют шаблон, который используется для поиска или модификации строки. Для работы со строками мы, конечно, можем использовать стандартный набор методов типа `String`. Однако регулярные выражения предоставляют более гибкий инструмент.[^6.2]
...
Выражение `[0-9][0-9][0-9][0-9]` соответствует любой последовательности из 4 цифр подряд. Например, такому шаблону соответствует строка "3452", но НЕ соответствует строка "jav5" (здесь только одна цифра). Строка "jav5" соответстветствовала бы шаблону "[a-z][a-z][a-z][0-9]" (первые три алфавитных символа в нижнем регистре, за которыми идет цифра).

Сразу стоит отметить, что выражение `[0-9][0-9][0-9][0-9]` не оптимально, и далее мы посмотрим, как его можно упростить.[^6.4]
...
<table>
<tbody><tr><td><p>\+</p></td><td><p>\d</p></td><td><p>-</p></td><td><p>\d</p></td><td><p>\d</p></td><td><p>\d</p></td><td><p>-</p></td><td><p>\d</p></td><td><p>\d</p></td><td><p>\d</p></td><td><p>-</p></td><td><p>\d</p></td><td><p>\d</p></td><td><p>\d</p></td><td><p>\d</p></td></tr>
<tr><td><p>+</p></td><td><p>1</p></td><td><p>-</p></td><td><p>2</p></td><td><p>3</p></td><td><p>4</p></td><td><p>-</p></td><td><p>5</p></td><td><p>6</p></td><td><p>7</p></td><td><p>-</p></td><td><p>8</p></td><td><p>9</p></td><td><p>0</p></td><td><p>2</p></td></tr>
</tbody></table>
...
Выражение `/^[a-zA-Z]{3,10}$/` говорит, что любой символ из диапазонов "a-z" и "A-Z" должен повторяться не меньше 3 раз и не больше 10 раз. Причем здесь также указано, что это должно быть отдельное слово. Для этого вначале указывается символ "^", а в конце символ "$".[^6.7]

### Поиск в строке
Для поиска в строке подстроки, которая соответствовует регулярному выражению, применяется метод **`exec()`** объекта `RegExp`. Этот метод принимает строку для поиска и возвращает результат в виде массива.[^6.8] Например:
```js
...
- Первая группа `(\d{4})` соответствует числу из четырех цифр

- Вторая группа `(\d{2})` соответствует числу из двух цифр
...
Группа `(?<=\$)` указывает, что перед строкой должен идти знак доллара `$`. Если его нет, то метод `exec()` не найдет соответствий и возвратит `null`.[^6.6]

### Регулярные выражения в методах String
Ряд методов объекта `String` могут использовать регулярные выражения в качестве параметра.[^6.3]
...
В JavaScript нельзя получить элемент из `Set` по индексу, поскольку `Set` — это неупорядоченная коллекция уникальных значений. Чтобы получить значение, нужно использовать итерацию, например, с помощью `for...of`, `forEach` или `values()`. Это позволяет пройти по всем элементам и найти нужный.
...
В данном случае в конструктор **`Intl.DateTimeFormat`** передается значение только для первого параметра `locales`. В первом случае это код `"ru"`, который представляет русскоязычную культуру, а во втором случае код `"en"` — англоязычная культура. И в зависимости от переданного кода культуры мы получим разные результаты при форматировании даты.[^18.2]
...
А константа `USAInRussian` также получает название страны по коду "US", только на русском языке.[^18.3]

#### Получение название региона
Для получения названия региона (это может быть название страны или название географического региона) применяется параметр `type: "region"`. Для получения названия региона в метод **`of()`** передается код региона. В качестве кода региона может выступать код [ISO-3166](https://www.iso.org/iso-3166-country-codes.html) из двух букв (например, US, RU, DE и т.д.) или трехчисловой код [UN M49](https://unstats.un.org/unsd/methodology/m49/).
...
Для получения названия валюты применяется значение `type: "currency"`, а в метод **`of()`** передается трехбуквенный код [ISO 4217](https://www.iso.org/iso-4217-currency-codes.html). Например:
```js
...
В разных культурах используются различные подходы к отображению чисел. Например, в одних культурах (в частности, в США, Великобритании) в качестве разделителя целой и дробной части применяется точка, а в других культурах — запятая. Аналогично разделителем между разрядами может служить как точка, так и запятая. И объект **`Intl.NumberFormat`** позволяет нам локализовать числительные под нужную культуру.[^18.4]
...
- **`unit`**: устанавливает единицу измерения. Применяемые единицы измерения можно найти в [следующей таблице](https://tc39.es/proposal-unified-intl-numberformat/section6/locales-currencies-tz_proposed_out.html#sec-issanctionedsimpleunitidentifier).
...
Для форматирования единиц измерения применяется значение `style: "unit"`.
[^18.6] При этом также необходимо указать название единицы измерения с помощью параметра `unit`:
```js
const value = 85;
...
- **`localeMatcher`**: представляет используемый алгоритм выбора локали. Может принимать два значения: `"lookup"` и `"best fit"` (значение по умолчанию). Значение `lookup` предполагает использование [алгоритма](https://datatracker.ietf.org/doc/html/rfc4647#section-3.4), определенного в стандарте. Значение `best fit` использует локаль, которую предоставляет среда выполнения
...
### Задание
Создать класс `Sample`, позволяющий осуществлять обработку выборки числовых данных.

1. Выборка числовых данных сохраняется в виде массива во внутреннем приватном поле `data`. Конструктор класса должен принимать выборочные данные как в виде массива, так и в виде отдельных значений. Сохранять дату и время создания экземпляра класса в публичном свойстве `created`.

2. Предусмотреть неизменяемые свойства, возвращающие основные характеристики выборочной совокупности, хранящейся в поле `data`:
...
3. Предусмотреть следующие публичные методы:

   - `getData` — возвращает исходный массив данных выборки;
   - `getSeries` — возвращает массив данных выборки, отсортированный по возрастанию;
...
4. Предусмотреть публичные свойства только для чтения, возвращающие логические значения и позволяющие оценить выборочную совокупность с точки зрения обобщенных характеристик:

   - `isNormal` — нормальность распределения (по правилу трёх сигм);
   - `isSymmetric` — симметричность распределения.

5. Предусмотреть неизменяемое публичное свойство `uniformity`, показывающее однородность признаков выборки и возвращающее одно из четырех предустановленных значений (`"low"` | `"medium"` | `"heigh"` | `"extreme"`).

Для расчета характеристик выборки по признаку $X$ использовать формулы, приведенные в приложенном файле *calc.pdf*. Результат предоставить в виде файла *Sample.js*.
...
1. Построить диаграмму рассеяния на `canvas`.
2. Найти уравнение линейной регрессии методом наименьших квадратов.
3. Построить на графике линию регрессии.

## Глоссарий
Локаль (*locale*)
: код языка в формате BCP 47 или набор языковых кодов.
...
[^6.7]: [Квантификаторы в регулярных выражениях](https://metanit.com/web/javascript/6.7.php)
[^6.8]: [Поиск в строке](https://metanit.com/web/javascript/6.8.php)
