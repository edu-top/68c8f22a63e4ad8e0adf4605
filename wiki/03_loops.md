...
4. Проверить, является ли введенное пользователем число простым. Вывод осуществлять с помощью модального окна. Проверять вводимые данные на валидность, в случае неверного ввода перезапрашивать данные у пользователя до тех пор, пока не будут введены корректные значения.
...
2. Вывести все целые числа из диапазона от $A$ до $B$, кратные трем ($A \leq B$).
3. Вывести все целые числа из диапазона от $A$ до $B$ ($A \leq B$), оканчивающиеся на цифру $X$.
4. Ввести целое число $N > 0$. Используя операции деления нацело и взятия остатка от деления, найти число, полученное при прочтении числа $N$ справа налево.
5. Ввести действительное число $x$ и натуральное число $n$. Вычислить $x (x - n) (x - 2n)(x - 3n)…(x - n^2)$.
