...
- [Формы](#формы)
  ...
  - [Textarea](#textarea)
  - [Практическая работа. Создание регистрационной формы](#практическая-работа-создание-регистрационной-формы)
...
### Практическая работа. Создание регистрационной формы

Создать страницу с формой регистрации в соответствии с прилагаемым образцом.

**_Базовые требования_**

- Исходный внешний вид формы должен в точности соответствовать прилагаемому образцу.
- Все поля должны иметь соответствующий тип и ограничения по размеру/формату.
- Все поля секции "Основная информация" обязательны для заполнения.
- Поле "Логин" должно автоматически фокусироваться при обновлении страницы.
- Элемент выбора аватара должно допускать выбор только файлов изображений.
- Поле "Дата рождения" должно запрещать регистрацию несовершеннолетних пользователей и ограничивать возраст до 120 лет.
- Надпись "выберите страну" в поле "Страна" не должна быть доступной для выбора в качестве устанавливаемого значения.
- Поле "Профессия / специальность" должно допускать как ввод информации, так и выбор из предлагаемого списка.
- Поле "Стаж " должно допускать ввод только целых неотрицательных значений в разумном диапазоне (не более 100 лет).
- Поле "Уровень квалификации" должен допускать ввод только целых значений от 1 до 10, а соответствующий ему слайдер иметь дискретное перемещение в указанном интервале значений с шагом 1.

**_Поведение формы_**

- Регистрация невозможна без согласия на обработку персональных данных.
- При попытке регистрации должна выполняться автоматическая проверка полей формы с показом встроенных валидационных сообщений в случае обнаружения несоответствий.
- При отмене форма должна восстанавливать первоначальное состояние.

**_Дополнительные требования_**

- Синхронизировать отображение и ввод уровня квалификации.
- Синхронизировать выбор цветовой темы в полях выбора цвета и указания цветового кода.
- После пройденной проверки полей показывать всплывающее окно, сообщающее об успешной регистрации.

Результат предоставить в виде html-файла (*index.html*).

![Layout](../img/layout_05.png)

<details>
<summary>Решение<summary>

[Код решения](../src/html/05_forms/index.html)
