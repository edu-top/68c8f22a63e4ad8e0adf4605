...
- `readAsDataURL` – когда мы хотим использовать данные в `src` для `img` или другого тега. Есть альтернатива – можно не читать файл, а вызвать `URL.createObjectURL(file)`, детали в разделе, посвященном `Blob`.
...
!!! info "FileReader для Blob"
    Как упоминалось в разделе, посвященном `Blob`, `FileReader` работает для любых объектов `Blob`, а не только для файлов.
...
!!! info "Для Web Worker также доступен `FileReaderSync`"
...
А если мы собираемся отправить `File` по сети, то это также легко, поскольку в сетевые методы, такие как `XMLHttpRequest` или `fetch`, встроена возможность отсылки `File`.[^file]
