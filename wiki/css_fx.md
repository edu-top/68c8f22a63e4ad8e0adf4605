...
- [Маскирование](#маскирование)
  ...
  - [Маски](#маски)
    ...
    - [Практические примеры](#практические-примеры)
      ...
      - [Вырезание нескольких аватарок](#вырезание-нескольких-аватарок)
  ...
  - [Кадрирование и маскирование в SVG](#кадрирование-и-маскирование-в-svg)
    ...
    - [Использование хорошо известных CSS техник](#использование-хорошо-известных-css-техник)
...
- [Как браузер рисует страницы](#как-браузер-рисует-страницы)
  - [Получение ресурсов, Fetching](#получение-ресурсов-fetching)
  - [Парсинг, Parsing](#парсинг-parsing)
    - [DOM](#dom)
    - [CSSOM](#cssom)
      - [Render Tree](#render-tree)
  - [Вычисление позиции и размеров, Layout](#вычисление-позиции-и-размеров-layout)
    - [Глобальный и инкрементальный Layout](#глобальный-и-инкрементальный-layout)
    - [«Грязные» элементы](#грязные-элементы)
  - [Непосредственно отрисовка, Paint](#непосредственно-отрисовка-paint)
    - [Порядок отрисовки](#порядок-отрисовки)
- [Оптимизация эффектов](#оптимизация-эффектов)
  - [Критический путь рендеринга](#критический-путь-рендеринга)
  ...
  - [Полная изоляция элемента](#полная-изоляция-элемента)
    - [Свойство `contain`](#свойство-contain)
- [Глоссарий](#глоссарий)
...
- `none`
  ...
  Значение `none` у CSS‑свойства `filter` означает, что к элементу не применяется никаких графических фильтров (размытие, яркость, серый цвет и т.п.). Что делает `filter: none`:

  - отменяет все ранее заданные фильтры (например, `blur()`, `grayscale()`, `brightness()` и др.) и возвращает элемент в исходное состояние.

  - является значением по умолчанию для свойства `filter`, то есть изначально у любого элемента уже стоит `filter: none`, если его явно не задавали.
  ...
  img:hover {
    filter: none; /* при наведении убираем фильтр, изображение становится цветным */
  }
  ```

  ![Исходное изображение](../img/image_filter.jpg)

  Таким образом `none` часто используют, чтобы «включать» фильтр только в определённых состояниях (через `:hover`, `:focus` и т.д.), а в остальное время показывать элемент без эффектов.

- `blur()`

    Примеряет размытие Гаусса к изображению, при этом край и детали становятся мягче, более «размытыми». Значение в скобках указывает сколько пикселей сливаются друг с другом. Чем больше значение, тем больше размытие. Можно указать положительное значение в любых единицах измерения, кроме процентов.
    ...
    [Демо](../samples/css/filter_blur.html)
...
- `brightness()`
    ...
    [Демо](../samples/css/filter_brightness.html)
...
- `contrast()`
    ...
    [Демо](../samples/css/filter_contrast.html)
...
- `drop-shadow()`
    ...
    [Демо](../samples/css/filter_drop-shadow.html)
...
- `grayscale()`
    ...
    Делает изображение чёрно-белым, превращая цвета элемента в оттенки серого, от «чуть серовато» до «полностью чёрно‑белое». В скобках можно указать значение от 0% до 100%. Значение 100% сделает изображение полностью чёрно-белым. Значение 0% обозначает фактически отсутствие эффекта (вернёт изображению исходные цвета). Значением может быть целое или дробное число без единиц измерения.
    ...
    [Демо](../samples/css/filter_grayscale.html)

- `hue-rotate()`

    Меняет цвета изображения за счёт поворота цветового круга, при этом все оттенки смещаются на заданный угол. Угол поворота указывается в скобках функции. Можно указывать угол в градусах `deg` — например `hue‑rotate(90deg)` сместит цвета на четверть круга, а `hue‑rotate(180deg)` — на половину. Также угол можно задавать в поворотах `turn`.
    ...
    [Демо](../samples/css/filter_hue-rotate.html)

- `invert()`

    Инвертирует цвета изображения, как бы выворачивает их, превращая в противоположные, при этом белое становится чёрным, тёмное — светлым и т.д. В результате получается что-то вроде негатива. Можно указать процент инверсии от 0% до 100%. При 100% цвета на картинке полностью инвертированы. Отрицательные значения или значения больше 100% не допускаются.
    ...
    [Демо](../samples/css/filter_invert.html)

- `opacity()`

    Меняет прозрачность изображения. Можно указать процент прозрачности от 0% до 100%. 0% делает картинку полностью прозрачной. 100% не меняет прозрачность изображения. Значения между ними делают элемент полупрозрачным. Отрицательные значения или значения больше 100% не допускаются.

    Это очень похоже на работу свойства `opacity` с той разницей, что для фильтра браузер, как правило, применяет аппаратное ускорение для улучшения производительности.
    ...
    [Демо](../samples/css/filter_opacity.html)

- `saturate()`

    Меняет насыщенность цветов изображения — от «приглушённые» до «очень яркие». Значение 0% полностью убирает насыщенность цветов (цвета полностью выцветают). Значение 100% не изменяет исходное изображение (базовая насыщенность). Допускаются значения больше 100% что приводит к *перенасыщенности*. Нельзя указать отрицательное значение.
    ...
    [Демо](../samples/css/filter_saturate.html)

- `sepia()`

    Меняет цвета изображения на сепию — коричневые оттенки, тем самым добавляя эффект старой сепиевой фотографии (изображение становится тёплым, коричневато‑серым). Значение 100% полностью преобразует изображение в сепию. Значение 0% не изменяет исходное изображение. Отрицательные значения или значения больше 100% не допускаются. Можно использовать целое или дробное число без единиц измерения в качестве значения.
    ...
    [Демо](../samples/css/filter_sepia.html)

    !!! question "Что такое сепия?"

      Название происходит от латинского слова *sepia*, что переводится как «каракатица». Изначально красящее вещество добывали из чернильного мешка этого морского моллюска.

      В эпоху пленочной фотографии (XIX — начало XX века) монохромные снимки были подвержены выцветанию и разрушению со временем из-за воздействия химических веществ. Чтобы сделать изображение более долговечным, его подвергали процессу вирирования (тонирования). Серебряное изображение заменялось на более стабильные соединения, которые окрашивали картинку в теплый коричневый цвет. Такая фотография и называлась сепией.

!!! tip "Наложение фильтров"

    Можно указать сразу несколько фильтров для одного элемента, перечислив их через пробел.[^filter-functions] Например, скомбинируем несколько значений в одном `filter`:

    ```css
    filter: blur(2px) brightness(120%) contrast(110%) saturate(140%);
    ```

    Это позволяет добиваться сложных эффектов, похожих на обработку в графических редакторах, прямо в CSS.
...
| --------------------- | ----------------------------------------------------------------------------- | ---------------------------------------- |
| `mix-blend-mode`        | Смешивает весь элемент с нижними слоями или фоном родителя               | Текст над изображением, glitch-эффекты   |
...
!!! info "Поддержка браузеров"
    ...
    Свойства имеют полную поддержку с 2015–2017 годов (Chrome 41+, Firefox 32+, Safari 8+). В настоящее время работают везде без префиксов.
...
В [статье](https://ishadeed.com/article/thinking-about-the-cut-out-effect/) (англ.), посвящённой эффекту вырезания, рассказано о нескольких способах его реализации в CSS.

Один из приведённых там примеров хорошо подходит для маскирования.

![Mask](../img/rckzqx3sius3ozpks1nigxgdyco.png)
...
![Mask](../img/mq7fssweycexjyf8enbwldkjkk0.png)

В упомянутой статье есть много подробных примеров вроде приведённого выше.

##### Дополнительные ресурсы

Если вы хотите отточить навыки на дополнительных примерах, то рекомендую следующие материалы:

- [A Fancy Hover Effect For Your Avatar](https://css-tricks.com/a-fancy-hover-effect-for-your-avatar/) от Temani Afif
- [Mask Compositing: The Crash Course](https://css-tricks.com/mask-compositing-the-crash-course/) Ana Tudor
- [CSS only Speech Bubble](https://codepen.io/t_afif/pen/eYMbrJN?editors=0100) Temani Afif
- [Fancy gradient border on hover](https://codepen.io/t_afif/pen/qBpRbWr?editors=0100) от Temani Afif
...
На первый взгляд, странно стирать то, что было только что нарисовано. Но когда вы попытаетесь создать полукруг в SVG, то сразу столкнётесь со следующими свойствами.[^Clipping_and_masking]
...
- **маска** (*masking*), с другой стороны, позволяет создавать полупрозрачные эффекты (например, размытые края).
...
По центру (`100,100`) нарисован круг с радиусом `100`. Атрибут `clip-path` ссылается на элемент `<clipPath>`, который содержит элемент `rect`. Этот прямоугольник позволит отрисовать верхнюю половину чёрного холста. Обратите внимание, что элемент `clipPath` обычно помещается в раздел `defs`.
...
![SVG mask](../img/clip-path-linear-gradient.png)
...
![SVG opacity](../img/clip-path-radial-gradient.png)
...
Начнём по порядку.[^how-the-browser-creates-pages]
...
<dfn title="DOM">DOM</dfn> (Document Object Model) — абстрактное представление HTML-документа, с помощью которого браузер может получать доступ к его элементам, изменять его структуру и оформление.
...
И в этом тоже порядок, атрибут [`defer`](https://doka.guide/html/defer-async/) скажет браузеру продолжать парсить страницу и выполнить скрипт потом:
```html
<body>
...
<dfn title="CSSOM">CSSOM</dfn> (CSS Object Model) — по аналогии с DOM, представление стилевых правил в виде дерева.
...
![CSSOM дерево](../img/CSSOM-1200w.webp)
...
![Render tree](../img/render-tree-1200w.webp)
...
![Общая схема парсинга HTML и CSS](../img/1-1200w_1.webp)

На первых шагах мы разбираемся с HTML и CSS, а затем объединяем их в Render Tree.

### Вычисление позиции и размеров, Layout
После того как у браузера появилось дерево рендеринга (*Render Tree*), он начинает «расставлять» элементы на странице. Этот процесс называется компоновкой (*Layout* — макет).
...
<dfn title="глобальная компоновка">Глобальная компоновка</dfn> (*global layout*) — это процесс просчёта всего дерева полностью, то есть каждого элемента. <dfn title="инкрементальная компоновка">Инкрементальная комоновка</dfn> (*incremental layout*) — просчитывает только часть.
...
![Дерево «грязных» и перерисованных элементов](../img/2-1200w_1.webp)
...
Пример использования `contain` для изоляции:
```css
.card {
...
Также свойство помогает повысить производительность страницы. Например, браузер понимает, когда при изменении свойств элемента нужно перерисовать страницу, а когда нет.[^contain]
...
- **`none`** — изоляция не применяется (значение по умолчанию). Оно указывает, что контейнер не влияет на размеры или положение потомков.
- `layout` — элемент изолируется от макета. Элемент не влияет на размеры других элементов и не мешает их позиционированию. Таким образом, изменения внутри не влияют на внешние элементы, и наоборот. 
- `paint` — включает режим отсечения. Содержимое элемента не отрисовывается за его пределами, при этом элемент не влияет на окрашивание других элементов. Браузер не будет перерисовывать другие элементы, когда данный элемент изменяет свой вид, например, фоновый цвет или прозрачность.
- `size` — размер элемента рассчитывается без учета его потомков. Rонтейнер пытается автоматически подстроить свои размеры так, чтобы соответствовать потомкам и учесть их размеры и положение.
- `inline-size` — элемент не будет изменять свою ширину при изменении содержимого. Аналогично `size`, но только для `inline`-размеров.
...
## Глоссарий
CSSOM (CSS Object Model)
: представление стилевых правил в виде дерева.
...
Глобальная компоновка (*global layout*)
: процесс просчёта всего дерева полностью, то есть каждого элемента.

Инкрементальная компоновка (*incremental layout*)
: процесс расчёта и отрисовки элементов пользовательского интерфейса или графических структур, при котором вычисляются и обновляются только измененные части макета, а не вся страница целиком.
