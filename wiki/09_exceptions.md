...
- [Обработка исключений и отладка](#обработка-исключений-и-отладка)
  ...
  - [Практическая работа. Обеспечение безопасной обработки данных](#практическая-работа-обеспечение-безопасной-обработки-данных)
...
### Практическая работа. Обеспечение безопасной обработки данных
Модифицировать класс для обработки данных (`Sample`), созданный в ходе практической работы "Обработка признаков объекта".

1. Обрабатывать входящие данные при инстанцировании класса для предотвращения создания объекта с некорректными данными. В случае нарушения допустимого формата (предоставляемые аргументы не являются числами или массивом чисел) выбрасывать исключение `TypeError` с соответствующим пояснением.
2. Добавить ограничения на размер входящих данных при создании объекта: объем выборки должен быть от 1 до 100000 элементов. В случае нарушения данного ограничения генерировать исключение `RangeError` с соответствующим пояснением.
3. Предусмотреть обработку ошибок для неожиданных ситуации в математических вычислениях там, где потенциально возможно выполнение некорректных операций. В случае возникновения подобных ситуаций выбрасывать пользовательское исключение `CalculationError`, помимо стандартных полей содержащее сведения о значении, приводящем к возникновению ошибки.
4. В методах-геттерах, осуществляющих вызовы других внутренних методов класса (геттеров), потенциально генерирующих исключения, выполнять перехват ошибок с генерацией пользовательского исключения `IntermediateCalculationError`, помимо стандартных полей содержащего сведения о значении, вызывающем возникновение ошибки, а также перехваченном исключении.
5. Предусмотреть защиту исходных данных, хранящихся в приватном поле класса, от любых возможных изменений, а также невозможность какой-либо модификации созданного экземпляра класса (итоговый объект должен быть защищен от любых изменений).

Результат предоставить в виде файла *Sample.js*.
