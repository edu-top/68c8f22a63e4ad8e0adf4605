...
В процессе работы прогаммы могут возникать различные ошибки, которые нарушают привычный ход программы и даже заставляют ее прервать выполнение. Язык JavaScript имеет инструменты для обаботки таких ситуаций.[^16.1]
...
В конструкторе мы определяем дополнительное свойство — `argument`. Оно будет хранить значение, которое вызвало ошибку. С помощью параметра `value` конструктора получаем это значение. Кроме того, переопреляем имя типа с помощью свойства `this.name`.[^16.3]
