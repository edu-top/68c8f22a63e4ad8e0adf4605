...
Координаты задаются в пикселях от левого верхнего угла (0,0); для точного расчета используются инструменты вроде Image Map Generator. Клики по зонам ведут на разные страницы; для открытия ресурса в новой вкладке необходимо добавить атрибут `target="_blank"`.
