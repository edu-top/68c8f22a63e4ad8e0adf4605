...
- [Служебные элементы](#служебные-элементы)
  - [Элемент head и метаданные веб-страницы](#элемент-head-и-метаданные-веб-страницы)
    ...
    - [Элемент link](#элемент-link)
      - [Атрибуты](#атрибуты)
      - [Подключение ресурсов](#подключение-ресурсов)
        - [Подключение таблиц стилей: `stylesheet`](#подключение-таблиц-стилей-stylesheet)
        - [Подключение иконки: `icon`](#подключение-иконки-icon)
        - [Обозначение справочного ресурса: `help`](#обозначение-справочного-ресурса-help)
        - [Создание ссылки на лицензию: `license`](#создание-ссылки-на-лицензию-license)
        - [Создание ссылки на манифест: `manifest`](#создание-ссылки-на-манифест-manifest)
        - [Указание наличия поиска: `search`](#указание-наличия-поиска-search)
        - [Указание условий использования: `terms-of-service`](#указание-условий-использования-terms-of-service)
        - [Обозначение последовательности документов: `next` и `prev`](#обозначение-последовательности-документов-next-и-prev)
      - [Оптимизация](#оптимизация)
        - [Условная загрузка ресурсов с медиавыражениями](#условная-загрузка-ресурсов-с-медиавыражениями)
        - [Указание альтернативной версии: `alternate`](#указание-альтернативной-версии-alternate)
        - [Указание авторства: `author`](#указание-авторства-author)
        - [Разрешение конфлитов: `canonical`](#разрешение-конфлитов-canonical)
        - [Предзагрузка ресурсов: `preload`](#предзагрузка-ресурсов-preload)
        - [Предзагрузка и кэширование: `prefetch`](#предзагрузка-и-кэширование-prefetch)
        - [Предварительное подключение к домену: `preconnect`](#предварительное-подключение-к-домену-preconnect)
        - [Предварительное разрешение записи DNS: `dns-prefetch`](#предварительное-разрешение-записи-dns-dns-prefetch)
        - [Предзагрузка и пререндер страницы: `prerender`](#предзагрузка-и-пререндер-страницы-prerender)
        - [Предзагрузка модульных скриптов: `modulepreload`](#предзагрузка-модульных-скриптов-modulepreload)
        - [Поддержание связи между сайтами: `pingback`](#поддержание-связи-между-сайтами-pingback)
        - [Задержка отображения: `expect`](#задержка-отображения-expect)
      - [Другие значения атрибута `rel`](#другие-значения-атрибута-rel)
        - [Создание постоянной ссылки: `bookmark`](#создание-постоянной-ссылки-bookmark)
        - [Тегирование ссылок](#тегирование-ссылок)
        - [Модификация поведения ссылок](#модификация-поведения-ссылок)
    - [Элемент meta](#элемент-meta)
      - [Атрибуты](#атрибуты-1)
      - [Нестандартные метаданные](#нестандартные-метаданные)
...
#### Элемент link
Элемент `<link>` подключает внешние ресурсы — стили, фавиконки, шрифты и прочее, позволяя загружать на страницу содержимое из внешнего файла.

Матёрые программисты советуют описывать CSS-стили в отдельных файлах: так их легче редактировать, сортировать и подключать к разным страницам сайта. Чтобы применить стили из внешнего файла на странице, используйте тег `<link>`.[^link]
...
Во всех описанных случаях `<link>` пишется внутри `<head>`.[^link]

Чаще всего `<link>` используется для CSS-стилей, подгружённых из файла. А также для фавиконки и других важных ссылок.
...
!!! question "Почему принято размещать `<link>` со стилями внутри `<head>`, а `<script>` перед закрывающим тегом `</body>`? Когда можно нарушить это правило?

    Когда мы размещаем стили внутри тега `<head>`, браузер понимает, как и с какими стилями отрисовывать контент при первом рендере. То есть, чтобы отобразить страницу, браузеру нужно знать какой контент нарисовать (HTML) и как должны выглядеть элементы (CSS). Если браузер не получит CSS заранее, то HTML-элементы сначала отобразятся со стилями по умолчанию, а после загрузки CSS применятся новые стили. Это вызовет мигание контента. Добавление тега `<link>` co стилями в начало документа предотвращает повторную перерисовку.
    ...
    Мы можем размещать скрипт в `<head>`, если он должен выполнится первым и не полагается на то, существует ли элемент на странице. Или если добавляем атрибуты `async` или `defer` к тегу `<script>`. Про их отличия можно почитать подробнее в разделе про `<script>`.
...
- `href` — URL-ссылка на внешний файл. Ссылка может быть как полной или абсолютной http://localhost/sitename/css/style.css, так и неполной или относительной /sitename/css/style.css.
    ...
    !!! warning "Отличия `src` от `href`"
        ...
        🛠 При подключении стилей не забываем про каскад!
...
!!! info "Что такое RSS"
    ...
    <dfn>RSS</dfn> (Rich Site Summary — обогащённая сводка сайта, или Really Simple Syndication — очень простое распространение) — это семейство XML-форматов для автоматической трансляции обновлений сайтов (новостей, статей, подкастов). Технология позволяет пользователям получать новый контент в «ридер» (агрегатор) без необходимости посещать каждый сайт отдельно. RSS-лента содержит структурированный текст (заголовки, описания).

    Основные факты об RSS:
    - **Как это работает**: сайт публикует RSS-файл (ленту), который обновляется автоматически. Пользователь подписывается на ленту через программу-агрегатор (например, Feedly, документацию Microsoft или Outlook).
    - **Преимущества**: cбор всех новостей в одном месте, экономия времени, отсутствие спама, получение информации в реальном времени.
    - **Применение**: bспользуется для блогов, новостных порталов, подкастов и рассылок.
    - **Как читать RSS**: lля чтения лент нужно установить приложение-агрегатор, добавить в него URL-адрес RSS-ленты сайта, и все новые публикации будут автоматически отображаться в приложении.

Например, эта строка подключает внешний CSS‑файл Font Awesome через CDN и использует несколько атрибутов, поясняющих браузеру, что именно делать с этим ресурсом.

```html
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/7.0.1/css/all.min.css" integrity="sha512-2SwdPD6INVrV/lHTZbO2nodKhrnDdJK9/kg2XD1r9uGqPo1cUbujc+IYdlYdEErWNu69gVcYgdxlmVmzTWnetw==" crossorigin="anonymous" referrerpolicy="no-referrer" />
```

Атрибуты `rel` и `href` отвечают за тип ресурса и его адрес, `integrity` — за целостность, `crossorigin` и `referrerpolicy` — за безопасность и приватность запроса.

Основные атрибуты в <link> здесь:
- `rel="stylesheet"` — указывает, что связанный ресурс является внешним CSS‑файлом стилей; браузер должен применить его к странице.

- `href="https://cdnjs.cloudflare.com/.../all.min.css"` — задаёт путь (URL) к самому файлу стилей, который браузер должен загрузить с CDN.

Атрибуты безопасности и кэширования:
- `integrity="sha512-..."` — хэш‑сумма (SRI, Subresource Integrity), которую браузер проверяет при загрузке файла, чтобы убедиться, что содержимое не было изменено на стороне CDN.

- `crossorigin="anonymous"` — указывает политику кросс‑доменного запроса; anonymous означает, что запрос идёт без отправки учётных данных (куки, HTTP‑авторизации).

- `referrerpolicy="no-referrer"` — ограничивает, какие данные о реферере (откуда пришёл пользователь) отправляются при загрузке CSS‑файла; `no‑referrer` означает, что реферер вообще не передаётся.

##### Подключение ресурсов
Одна из основных функций тега `<link>` — подключение внешних ресурсов.

###### Подключение таблиц стилей: `stylesheet`
...
!!! info "Что такое MIME"
    ...
    <dfn title="MIME">MIME</dfn> (Multipurpose Internet Mail Extensions — многоцелевые расширения интернет-почты) — это стандарт, который описывает способы передачи различных типов данных (изображений, аудио, видео, документов) по электронной почте и через интернет. MIME позволяет форматировать сообщения, содержащие не только текст, но и вложения, обеспечивая их корректное отображение в почтовых программах и браузерах.

    Основные понятия MIME
    - **MIME-тип** (Media type): строка, отправляемая вместе с файлом, определяющая его формат. Состоит из типа и подтипа (например, `image/png` или `application/pdf`).
    ...
    - ёapplication/octet-streamё — бинарный файл по умолчанию.

    Стандарт MIME позволяет обойти ограничения, существовавшие ранее, когда электронная почта могла передавать только текстовые данные.
...
###### Обозначение справочного ресурса: `help`
...
###### Создание ссылки на лицензию: `license`
Атрибут `rel="license"` используется для создания ссылки на документ с условиями лицензии. Его можно применять с элементами `<link>`, `<a>`, `<area>` и `<form>`. Такая ссылка указывая на документ с условиями авторского права.
...
###### Создание ссылки на манифест: `manifest`
...
<dfn title="манифест">Манифест</dfn> — это JSON-файл с информацией о приложении. Файл манифеста определяет, какие ресурсы должны быть кэшированы, а также задаёт информацию для установки веб-приложения на устройство (например, значки и название).
...
###### Указание наличия поиска: `search`
...
!!! info "Что такое OpenSearch"

    <dfn title="OpenSearch">OpenSearch</dfn> — набор технологий, позволяющих веб-сайтам и поисковым системам публиковать результаты поиска в форматах, удобных для распространения и сбора.
...
###### Обозначение последовательности документов: `next` и `prev`
...
Использование `prev` и `next` помогает поисковым системам и вспомогательным технологиям лучше понимать структуру контента. Это улучшает навигацию и делает последовательные документы более доступными.[^rel]

##### Оптимизация
`<link>` — очень мощный и гибко настраиваемый тег. С его помощью можно многое оптимизировать, давайте посмотрим, как.

Делается это в основном через атрибут `rel`. Атрибут `rel` описывает как связаны текущая страница и подключаемый ресурс. Работает с `<a>`, `<link>` и `<area>`.[^rel] Основные функции:
...
Подробно эта тема разобрана в разделе, посвященном адаптивному дизайну.

###### Указание альтернативной версии: `alternate`
Значение `alternate` указывает на альтернативную версию текущего документа.
...
###### Указание авторства: `author`
Значение `author` показывает, где найти информацию об авторе страницы или статьи. Работает с тегами `<link>`, `<a>` и `<area>`.
...
###### Разрешение конфлитов: `canonical`
...
Таким образом, `<link rel="preload">` говорит браузеру как можно скорее загрузить и кэшировать ресурс (например, скрипт или таблицу стилей). Это полезно, когда ресурс понадобится через несколько секунд после загрузки страницы, и вы хотите ускорить процесс.
...
Этот атрибут полезен для подготовки к следующему взаимодействию пользователя, например, при переходе на новую страницу или загрузке тяжёлых медиафайлов.[^rel]
...
Таким образом, `<link rel="prefetch">` просит браузер загрузить и кэшировать ресурс в фоновом режиме. Загрузка происходит с низким приоритетом, поэтому не мешает более важным ресурсам. Это полезно, если ресурс понадобится на следующей странице, а вы хотите заранее его кэшировать.
...
Использование `preconnect` позволяет ускорить загрузку ресурсов, так как соединение уже установлено, когда браузеру потребуется обратиться к серверу.[^rel]
...
Таким образом, `<link rel="preconnect">` просит браузер заранее подключиться к домену, когда вы хотите ускорить установку соединения в будущем.
...
Например, веб-шрифты от [Google Fonts](https://fonts.google.com/) подключаются следующим образом:
```html
...
> Указывайте только те домены, которые реально понадобятся. Слишком много DNS-запросов могут замедлить начальную загрузку страницы.[^rel]

Таким образом, `<link rel="dns-prefetch">` просит браузер заранее выполнить резолвинг DNS для домена, если вы скоро будете подключаться к нему и хотите ускорить начальное соединение.
...
Но не злоупотребляйте такой возможностью! Предварительный рендеринг чрезвычайно дорого обходится с точки зрения трафика и памяти. Поэтому не используйте `<link rel="prerender">` более чем для одной страницы.[^link]

###### Предзагрузка модульных скриптов: `modulepreload`
...
###### Поддержание связи между сайтами: `pingback`
...
##### Другие значения атрибута `rel`
Остальные значения атрибута `rel` используются не в теге `<link>`, в других тегах (`<a>`, `<area>`, `<form>`).

###### Создание постоянной ссылки: `bookmark`
...
###### Модификация поведения ссылок
Следующие свойства не создают новых ссылок, но задают существующим дополнительное поведение.
...
#### Элемент meta
Элемент **`meta`** определяет метаданные документа. Он описывает содержание страницы для браузера, поисковиков и соцсетей.
...
Таких метатегов может быть любое количество. Все они размещаются внутри тега `<head>`, желательно в самом начале.[^meta] Пример:
...
Рекомендуется вставить как минимум метатеги `description`, `viewport`, `charset`. Они помогут оптимизировать сайт для браузера пользователя и поисковых систем.

В `<meta>` прописывается информация, которую нельзя указать в `<base>`, `<link>`, `<script>`, `<style>` или `<title>`.

Чтобы документ корректно отображал текст, необходимо задать кодировку с помощью атрибута **`charset`**. Метатег, который задаёт кодировку страницы, не отличается по структуре от обычных:
```html
<meta charset="...">
```
...
!!! tip "Положение метатега с кодировкой"
...
!!! warning "Использование `http-equiv` и `name`"
...
Таким образом, атрибут `name` содержит имя метаданных, а `content` — их значение.
...
- `charset`
    ...
    Задаёт кодировку страницы. Рекомендуется писать здесь `UTF-8` — это самый распространённый вариант.
...
- `http-equiv`
    ...
    !!! tip "Использование `http-equiv` для перенаправления"
...
- `name`
    ...
    - `"viewport"` — задаёт параметры окна просмотра в браузере. Страницу можно сделать адаптивной, подогнав ширину окна под размеры устройства. В примере ниже `width` указывает ширину окна просмотра, а `initial-scale` — коэффициент масштабирования страницы при первом открытии:
        ...
        !!! info "Мобильная версия"

            Таким образом с помощью метатегов можно задавать поведение мобильной версии сайта. Например, можно прописать, на сколько на мобайле можно увеличить масштаб и насколько уменьшить относительно стандарта.

            Значение `width=device-width` адаптирует ширину окна просмотра к экрану устройства. Значение `initial-scale=1` сохраняет соотношение 1:1 между CSS пикселямии и независимыми пикселями устройства.

            Если метатег отсутствует, мобильные браузеры по умолчанию показывают страницу для экрана компьютера. Так как ширина экрана мобильного устройства значительно меньше экрана компьютера, браузеры пытаются оптимизировать содержание — увеличивают шрифт, масштабируют содержание по размеру экрана или показывают только часть контента, которая помещается на экране.[^metatags]
    ...
    - `"description"` — краткое описание страницы, которое видит пользователь, когда находит сайт в поисковике. Например: «Рассказываем, как нарезать картошку тонкими ломтиками» для сайта о кулинарии. Это описание помогает поисковикам найти страницу, а пользователю — узнать, о чём она. Так что не забывайте указывать `"description"`.
        ...
        !!! info "Пропуск `description`"
            ...
            ![Description](../img/1-1200w.webp)

            Если хочется контролировать, что именно увидит пользователь в поиске, то всегда следует заполнять `description`.
...
Через метатеги задаётся и Open Graph-разметка: заголовок, картинка и описание для репоста в соцсетях.[^meta]
...
Метатегами можно указать роботам правила, как загружать и индексировать страницы сайта.[^metatags]

Метатеги, которые учитываются поисковыми роботами

Метатег | Описание
-- | --
`Description` | Может использоваться в описаниях страниц сайта в результатах поиска.<br>**Пример**: `<meta name="description" content="..."/>`
`keywords` | Может влиять на соответствие страницы поисковым запросам.<br>**Пример**: `<meta name="keywords" content="..."/>`
`robots` | Определяет правила загрузки и индексирования страниц сайта.<br>**Пример**: `<meta name="robots" content="..."/>`
`viewport` | Адаптирует сайт для мобильных устройств. Контролирует масштабирование окна просмотра в браузере.<br>**Пример**: `<meta name="viewport" content="width=device-width, initial-scale=1">`
`Content-Type` | Определяет тип документа и его кодировку.<br>**Пример**: `<meta http-equiv="Content-Type" content="type; charset=..."/>`
`refresh` | Перенаправляет посетителя на страницу с адресом URL через N секунд. N считается временным перенаправлением при небольших значениях. Это аналогично серверным редиректам с HTTP-кодом 302/303/307.<br>Используйте метатег при переезде сайта.<br>**Пример**: `<meta http-equiv="refresh" content="N;url=_адрес цели перенаправления_">`
...
`nofollow` | Не переходить по ссылкам на странице.<br>Робот не перейдет по ссылкам при обходе сайта, но может узнать о них из других источников. Например, на других страницах или сайтах. | ✓ | ✓
...
Роботы разных поисковых систем и сервисов могут иначе интерпретировать директивы.

Примеры использования директив показаны ниже

<details>
<summary><b>Указать несколько директив</b></summary>

Для метатега `robots` можно указать директивы через запятую. Пример:
```html
<meta name="yandex" content="noindex, nofollow" />
```

Для заголовка X-Robots-Tag можно передать несколько заголовков в одном ответе, а также перечислить директивы через запятую. Пример:
```
HTTP/1.1 200 OK
...
<details>
<summary><b>Указать директивы для определенных роботов</b></summary>
...
<details>
<summary><b>Запретить индексирование страниц</b></summary>

Для метатега `robots` запись, которая запрещает индексирование страницы:
```html
<html>
    <head>
        <meta name="robots" content="noindex" />
...
HTTP-ответ, где заголовок `X-Robots-Tag` запрещает индексирование страницы. Пример:
```
HTTP/1.1 200 OK
Date: Tue, 25 May 2010 21:42:43 GMT
...
: стандарт, который описывает способы передачи различных типов данных (изображений, аудио, видео, документов) по электронной почте и через интернет. MIME позволяет форматировать сообщения, содержащие не только текст, но и вложения, обеспечивая их корректное отображение в почтовых программах и браузерах.
...
RSS (Rich Site Summary — обогащённая сводка сайта, или Really Simple Syndication — очень простое распространение)
: семейство XML-форматов для автоматической трансляции обновлений сайтов (новостей, статей, подкастов). Технология позволяет пользователям получать новый контент в «ридер» (агрегатор) без необходимости посещать каждый сайт отдельно. RSS-лента содержит структурированный текст (заголовки, описания).
...
Манифест
: JSON-файл с информацией о приложении. Файл манифеста определяет, какие ресурсы должны быть кэшированы, а также задаёт информацию для установки веб-приложения на устройство (например, значки и название).
...
[^meta]: [meta](https://doka.guide/html/meta/)
[^link]: [link](https://doka.guide/html/link/)
[^rel]: [Атрибут rel](https://doka.guide/html/rel/)
[^OpenSearch]: [OpenSearch](https://ru.wikipedia.org/wiki/OpenSearch)
[^metatags]: [Метатеги](https://yandex.ru/support/webmaster/ru/controlling-robot/metatags)
