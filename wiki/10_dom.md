...
Впрочем, HTML-атрибуты и свойства DOM не всегда одинаковы, мы увидим это в следующей главе.[^basic-dom-node-properties]
...
##### Удаление элемента
Для удаления элемента также имеется метод **`removeChild()`** объекта Node. Этот метод удаляет один из дочерних узлов:
```html
<!DOCTYPE html>
<html>
<head>
...
В данном случае удаляется первый параграф из блока `div`. В настоящее время данный метод считается устаревшим.
