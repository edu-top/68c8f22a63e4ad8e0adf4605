...
- [Объектная модель документа](#объектная-модель-документа)
  ...
  - [Практическая работа. Создание уведомления](#практическая-работа-создание-уведомления)
    - [Задание](#задание-4)
  - [Практическая работа. Работа с координатами и прокруткой](#практическая-работа-работа-с-координатами-и-прокруткой)
    - [Задание](#задание-5)
  - [Практическая работа. Создание заметки](#практическая-работа-создание-заметки)
    - [Задание](#задание-6)
...
- `behavior` – анимация прокрутки (`smooth`, `instant`, `auto`), определяет, будет ли прокрутка плавной или мгновенной:

    - `"smooth"` – плавная анимация (как в современных браузерах по умолчанию для UX).

    - `"instant"` – резкий скачок без анимации.

    - `"auto"` – браузер сам решает (обычно `instant` в десктопе, `smooth` на мобильных).

    Пример: `{ behavior: "smooth" }` – элемент плавно "въезжает" в вид.

- `block` – вертикальное выравнивание (`start`, `center`, `end`, `nearest`), определяет, где именно разместить элемент по вертикали в видимой области. Значение по умолчанию: `start`

    - `"start"` – верх элемента совпадает с верхом видимой области (по умолчанию).

    - `"center"` – элемент по центру экрана.

    - `"end"` – низ элемента совпадает с низом видимой области.

    - `"nearest"` – минимальный сдвиг: если элемент частично виден, прокрутка минимальна.

    Пример: `{ block: "center" }` идеально для модальных окон или важных уведомлений.

- `inline` – горизонтальное выравнивание (`start`, `center`, `end`, `nearest`), полезно при горизонтальной прокрутке. Значение по умолчанию: `nearest`

    - `"start"` – левый край элемента к левому краю контейнера.

    - `"center"` – по центру горизонтально.

    - `"end"` – правый край к правому краю.

    - `"nearest"` – минимальный сдвиг (по умолчанию).

*Практические комбинации*
```js
// Элемент в центр экрана с плавной анимацией
element.scrollIntoView({ behavior: "smooth", block: "center" });

// Кнопка "Наверх" – в самый верх мгновенно
backToTop.scrollIntoView({ behavior: "instant", block: "start" });

// Таблица с горизонтальной прокруткой – ближайшая позиция
table.scrollIntoView({ block: "nearest", inline: "nearest" });
```

Совет: `nearest` экономит движения пользователя – если элемент уже наполовину виден, страница не дёргается. Идеально для списков и таблиц.
...
### Практическая работа. Работа с координатами и прокруткой
...
1. **Прокрутка снизу**
    ...
    - Написать соответствующую функцию, возвращающую размер прокрутки  (пусть называется `scrollBottom`) снизу для любого произвольного элемента.

    - Проверить: если прокрутки нет вообще или элемент полностью прокручен, то функция должна возвращать `0`.
...
2. **Ширина полосы прокрутки**

    Написать код, который возвращает ширину стандартной полосы прокрутки.

    Для Windows она обычно колеблется от `12px` до `20px`. Если браузер не выделяет место под полосу прокрутки (так тоже бывает, она может быть прозрачной над текстом), тогда значение может быть `0px`.

    Код должен работать в любом HTML-документе, независимо от его содержимого.
...
3. **Мяч в центре поля**
    ...
    - Вычислить координаты центра поля.
    - Использовать вычисленные значения, чтобы поместить мяч в центр поля.
    - Элемент должен позиционироваться за счёт JavaScript, а не CSS.
    - Внешний вид поля и мяча могут быть любыми.
    - Код должен работать с любым размером мяча (10, 20, 30 пикселей) и любым размером поля без привязки к исходным значениям.
...
### Практическая работа. Создание заметки

#### Задание

Выполнить набор задач, представленных ниже.

1. **Показать заметку рядом с элементом**

    Создать функцию `positionAt(anchor, position, elem)`, которая позиционирует элемент `elem` в зависимости от значения свойства `position` рядом с элементом `anchor`.
...
2. **Показать заметку около элемента (абсолютное позиционирование)**

    Изменить код решения предыдущего задания так, чтобы элемент заметки использовал свойство `position:absolute` вместо `position:fixed`.

    Это предотвратит расхождение элементов при прокрутке страницы.

    За основу следует использовать решение предыдущего задания. Чтобы проверить решение в условиях с прокруткой, добавить стиль элементу `<body style="height: 2000px">`.
...
3. **Расположить заметку внутри элемента (абсолютное позиционирование)**

    Усовершенствовать решение предыдущего задания: научить функцию `positionAt(anchor, position, elem)` вставлять `elem` внутрь `anchor`.
    ...
    За основу следует использовать решение предыдущего задания.

    ![Note inside](../img/note-inside.png)
