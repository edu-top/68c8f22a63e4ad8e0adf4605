...
Однако теперь можно заметить, что при переходе по ссылка адресная строка перестала меняться. Проблема в том, что мы сами только что заблокировали переходы по соответствующим адресам и при клике по ссылкам код только обновляет контент, но не меняет адресную строку браузера.
