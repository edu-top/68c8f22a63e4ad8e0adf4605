...
4. Реализовать закрытие модального окна по нажатию на специальную кнопку закрытия (крестик в верхем правом углу) или клавишу <kbd>Escape</kbd>. Добавить настройку, отвечающую за возможность закрытия диалога по клику вне основной области модального окна (по заднику). Если открытие нескольких экземпляров невозможно, то реализовать автоматическое закрытие открытых до этого диалоговых окон.

5. Отслеживать действия пользователя, сформировав собственную систему событий (`modalOpen`, `modalClose`). Добавить возможность установки обработчиков через специальные методы (`onModalOpen`, `onModalClose`). Обрабатывать выбор пользователя, сохраняя его в данные кастомных событий.
