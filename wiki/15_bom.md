...
- [Открытие, закрытие и позиционирование окон](#открытие-закрытие-и-позиционирование-окон)
  ...
  - [Итого](#итого)
  - [Выводы](#выводы-1)
- [Общение между окнами](#общение-между-окнами)
  - [Политика "Одинакового источника"](#политика-одинакового-источника)
    - [Доступ к содержимому айфрейма](#доступ-к-содержимому-айфрейма)
  - [Окна на поддоменах: document.domain](#окна-на-поддоменах-documentdomain)
    - [Критические ограничения](#критические-ограничения)
    - [Айфрейм: подождите документ](#айфрейм-подождите-документ)
  - [Коллекция window.frames](#коллекция-windowframes)
  - [Атрибут айфрейма sandbox](#атрибут-айфрейма-sandbox)
  - [Обмен сообщениями между окнами](#обмен-сообщениями-между-окнами)
...
- [Атака типа clickjacking](#атака-типа-clickjacking)
  ...
  - [Примеры слабой защиты](#примеры-слабой-защиты)
    - [Блокировка top-навигации](#блокировка-top-навигации)
    - [Атрибут «sandbox»](#атрибут-sandbox)
  - [Заголовок X-Frame-Options](#заголовок-x-frame-options)
  - [Отображение с ограниченными возможностями](#отображение-с-ограниченными-возможностями)
  - [Атрибут cookie: samesite](#атрибут-cookie-samesite)
  - [Итого](#итого-2)
...
- [Использование браузерного API](#использование-браузерного-api)
    ...
    - [Использование History API](#использование-history-api)
...
- [Практика](#практика)
  - [Практическая работа. Создание модального окна](#практическая-работа-создание-модального-окна)
  - [Практическая работа. Создание одностраничного веб-приложения](#практическая-работа-создание-одностраничного-веб-приложения)
    - [Задание](#задание)
...
Все рассмотренные методы являются модальными: останавливают выполнение скриптов и не позволяют пользователю взаимодействовать с остальной частью страницы до тех пор, пока окно не будет закрыто.
...
Несмотря на ограничения, эти функции удобны для быстрого прототипирования или простых задач:

- `alert()` идеален для уведомлений об ошибках или важных событиях.

- `confirm()` помогает подтвердить опасные действия, например, удаление данных.

- `prompt()` подходит для ввода коротких значений, как имя пользователя.

- `find()` (в контексте браузера) упрощает поиск по странице.

- `print()` запускает печать без лишних настроек.

Они не требуют HTML-разметки и работают везде, где поддерживается JavaScript.

Альтернативой для современных интерфейсов может быть создание кастомных диалогов, используя HTML/CSS/JS:

- кастомные модальные окна на `<dialog>` элементе (нативная поддержка в современных браузерах).
...
- **Нативная модальность**: метод `showModal()` автоматически блокирует взаимодействие с фоном (как `alert`), создавая задник (*backdrop*).
...
Объект `window` служит фундаментом веб-разработки, предоставляя доступ к глобальному контексту браузера, управлению окнами, экранами и взаимодействию с пользователем. Свойства объекта `Screen` (такие как `width`, `height`, `availWidth`) дают точную информацию о дисплее, помогая адаптировать интерфейсы под разные устройства — от мобильных экранов до 4K-мониторов.

Диалоговые окна (`alert`, `confirm`, `prompt`) остаются незаменимыми для быстрых уведомлений и подтверждений, несмотря на их простоту и ограничения в кастомизации. Для современных приложений `<dialog>` и библиотеки вроде SweetAlert2 предлагают гибкие альтернативы с анимациями, стилями и полной доступностью.

Методы `print()` и поиск по странице упрощают повседневные задачи: от генерации печатных версий до мгновенного нахождения контента без лишнего кода.

Вместе эти инструменты образуют мощный арсенал для создания отзывчивых, удобных веб-приложений. Они работают везде, где есть JavaScript, не требуют фреймворков и идеальны для прототипов, утилит и критических взаимодействий. В эпоху сложных SPA знание нативных возможностей window остается ключевым навыком разработчика.
...
Попапы идеально подходят для OAuth, потому что после успешной авторизации провайдер (Google, Facebook и т.д.) может легко закрыть попап и отправить данные обратно в родительское окно через `postMessage`. Это позволяет реализовать плавный пользовательский процесс (*user flow*) без перезагрузки основной страницы.
...
- `url`
    ...
    URL для загрузки в новом окне — адрес страницы (например, `"https://example.com"` или `about:blank`).
...
Второй параметр (**`name`**) обозначает имя окна. Распространенные значения:
...
Важные особенности `window.opener`
1. Доступность только для попапов:

    ```js
    // В обычном окне (запущенном напрямую)
    console.log(window.opener); // null

    // В попапе (открытом через window.open)
    console.log(window.opener); // ссылка на родительское окно
    ```

2. Безопасное взаимодействие:

    ```js
    // ✅ В попапе: безопасно обращаться к opener
    if (window.opener) {
    window.opener.postMessage('Popup ready!', '*');
    }

    // ✅ В главном окне: проверять origin
    newWin.addEventListener('message', (e) => {
...
3. !! Двойная опасность — требует защиты:

    ```js
    // 🚨 ПЛОХО: любой сайт может переписать ваше окно
    let popup = window.open('https://evil.com');
    popup.document.write(`
    <script>
        window.opener.document.body.innerHTML = 'Хакерская атака!';
    </script>
    `); // Работает! Если evil.com получит доступ к opener
    ```

    Защита от злоупотреблений `opener`:

    **Способ 1**: отключить `opener` при открытии
    ```js
    let popup = window.open('/my-popup.html', 'popup', 'noopener');
    console.log(popup.opener); // null — безопасно!
    ```

    **Способ 2**: проверка в попапе

    ```js
    // В попапе:
    if (window.opener && window.opener.location.origin === window.location.origin) {
    // Безопасно взаимодействовать
    window.opener.postMessage('Данные', window.location.origin);
    }
    ```

4. Современная рекомендация — всегда используйте `noopener`:

    ```js
    // ✅ Лучшая практика
    window.open('/popup.html', 'popup', 'width=300,height=300,noopener,noreferrer');
    ```

*Таблица связей между окнами*

| Направление     | Ссылка            | Условие доступа                  |
| --------------- | ----------------- | -------------------------------- |
| Главное → Попап | переменная `open()` | Всегда (до закрытия)             |
| Попап → Главное | `window.opener`     | Только если не noopener          |
| Попап → Главное | `null`              | При `window.open(..., 'noopener')` |

**Вывод**: `window.opener` — мощный, но опасный инструмент. Используйте `noopener` для внешних ссылок и `postMessage` для безопасного общения.
...
По нажатию на вторую кнопку у объекта `exampleWindow` вызываем метод `close` и устанавливаем переменную в `null`.
...
!!! warning "Только попапы"
    ...
    **✅ Работает:**
    ```js
    let popup = window.open('about:blank', 'test', 'width=300,height=300');
    popup.moveTo(100, 100);  // ✅ Работает
    popup.resizeTo(400, 400); // ✅ Работает
    ```

    **❌ Блокируется:**
    ```js
    // Вкладки пользователя
    window.moveTo(0, 0);      // Игнорируется
    window.resizeTo(800, 600); // Игнорируется

    // Свернутые/развёрнутые окна
    window.resizeTo(1000, 1000); // Бесполезно

    // Новые вкладки (не попапы)
    window.open('url', '_blank'); // Нет контроля позиции/размера
    ```
...
Современные браузеры (по состоянию на 2026) добавили:

- **Popup blocker** — блокирует `window.open()` без пользовательского действия

- **Размерные лимиты** — попап не может быть больше 80% экрана

- **`noopener` по умолчанию** для `_blank`

- **User gesture requirement** — `open()` только по клику
...
**Вывод**: контроль окон — только для собственных попапов + только размер/позиция. Всё остальное заблокировано.
...
Примеры управления прокруткой окна:
```js
window.scrollBy(100, 200);
// Прокручиваем содержимое браузера до позиции (100, 200)
window.scrollTo(100, 200);
...
Различия: окно vs элементы
```js
// Окно/документ
window.scrollTo(0, 100);                // Прокручивает ВСЕ окно

// Конкретный элемент
let div = document.querySelector('.scrollable');
div.scrollTo(0, 100);                   // Только содержимое div

// Автофокус на элемент
div.scrollIntoView({block: 'center'});  // Окно прокрутится к div
```
...
### Выводы
Метод `window.open()` позволяет создавать новые браузерные окна или вкладки для загрузки внешнего контента, межстраничного взаимодействия или отладки, с возможностью настройки размеров, позиции, наличия панелей инструментов и статуса. Этот метод принимает URL, имя окна (для повторного использования), строку с параметрами (например, `width=500,height=400,menubar=no`) и возвращает ссылку на новое окно для дальнейшего управления — записи скриптов, фокуса или закрытия через `close()`. В современных браузерах попапы часто блокируются по умолчанию при автоматическом вызове без явного действия пользователя (клик, сабмит), что требует асинхронного вызова `window.open()` до асинхронных операций для обхода ограничений; при неудаче рекомендуется fallback на редирект или уведомление. Несмотря на устаревание в пользу `iframe` или модальных оверлеев из-за проблем с UX, безопасностью (всплывающие окна ассоциируются со спамом и фишингом) и мобильной адаптацией, `window.open()` остается полезным для авторизации через попапы (OAuth), предпросмотра документов или расширения пользовательского пространства без потери контекста основной страницы.

## Общение между окнами
*[SOP]: Same-Origin Policy
*[CORS]: Cross-Origin Resource Sharing
*[COEP]: Cross-Origin Embedder Policies
*[COOP]: Cross-Origin Owner Policies

<dfn title="политика одинакового источника">Политика «Одинакового источника»</dfn> (Same-Origin Policy, SOP) — это фундаментальный механизм безопасности в браузерах, введённый Netscape Navigator в 1995 году. Она защищает от несанкционированного доступа к ресурсам между разными источниками, предотвращая атаки вроде XSS или кражи данных.

Таким образом, политика «Одинакового источника» (*Same Origin*) ограничивает доступ окон и фреймов друг к другу. Идея заключается в том, что если у пользователя открыто две страницы: *john-smith.com* и *gmail.com*, то у скрипта со страницы *john-smith.com* не будет возможности прочитать письма из *gmail.com*. Таким образом, задача политики «Одинакового источника» – защитить данные пользователя от возможной кражи.[^cross-window-communication]

Основные правила доступа
- **Полный доступ** (чтение/запись): к `window`, `document`, `localStorage`, `cookies` и DOM, если источники совпадают полностью (протокол + домен + порт).

- **Блокированный доступ**: к контенту из другого источника. Браузер выдаст ошибку вроде `SecurityError: Blocked a frame with origin...`.

Пример в коде:
```js
// Окно A: http://site.com
const popup = window.open('http://site.com/page.html'); // Доступ разрешён
console.log(popup.document.title); // Работает

const iframe = document.querySelector('iframe[src="http://www.site.com"]');
console.log(iframe.contentDocument); // Ошибка: другой домен
```

Исключения и нюансы
- Location: можно менять (`otherWindow.location = 'http://example.com'`), но нельзя читать (`otherWindow.location.href` — блок).

- POSTMessage: безопасный способ обмена данными между окнами разного происхождения.

    ```js
    otherWindow.postMessage('данные', 'http://site.com');
    window.addEventListener('message', (e) => {
      if (e.origin === 'http://site.com') console.log(e.data);
    });
    ```
...
- CORS (Cross-Origin Resource Sharing)
    ...
    Разрешает кросс-доменные запросы (AJAX, fetch) через заголовки:
    ```
    Access-Control-Allow-Origin: http://site.com
    ```

    Без CORS `fetch('http://api.other.com')` заблокирован.

- SameSite cookies: `Strict`/`Lax` — защита от CSRF.

- COEP/COOP (Cross-Origin Embedder/Owner Policies): строгий контроль `iframe`.

Эти механизмы эволюционировали, но SOP остаётся основой — без неё веб был бы уязвим к массовым утечкам данных.
...
Когда мы обращаемся к встроенному в айфрейм-окну, браузер проверяет, имеет ли айфрейм тот же источник. Если это не так, тогда доступ будет запрещён (разрешена лишь запись в `location`, это исключение).

Для примера давайте попробуем чтение и запись в айфрейм с другим источником:
```html
...
С другой стороны, если у айфрейма тот же источник, то с ним можно делать всё, что угодно:
```html
...
!!! info "`iframe.onload` и `iframe.contentWindow.onload`"
    ...
    …Но `iframe.onload` всегда доступно извне айфрейма, в то время как доступ к `iframe.contentWindow.onload` разрешён только из окна с тем же источником.

### Окна на поддоменах: document.domain
По определению, если у двух URL разный домен, то у них разный источник. То есть по умолчанию браузер считает **разными источниками** страницы с **разными доменами** или **поддоменами**. Например:
- `john.site.com` ≠ `peter.site.com` ≠ `site.com`

Это означает **CORS-запросы** между ними блокируются, **postMessage** требует явного указания `origin`, а **iframe-контент** из одного поддомена не доступен другому.
...
Для страниц с **общим доменом второго уровня** (например, `*.site.com`) можно **приравнять источники**, установив в каждом таком окне:
```js
document.domain = 'site.com'; // Общий домен 2-го уровня
```

После этого браузер начинает считать john.site.com, peter.site.com и site.com одним источником, и они смогут взаимодействовать без ограничений. В частности, будут доступны:
...
Ещё раз стоит отметить, что это доступно только для страниц с одинаковым доменом второго уровня.

Полный рабочий пример (legacy):
```html
<!-- https://john.site.com -->
<script>
document.domain = 'site.com';  // ДО взаимодействия!

const iframe = document.createElement('iframe');
iframe.src = 'https://peter.site.com/iframe.html';
document.body.append(iframe);

iframe.onload = () => {
    // Теперь доступно!
    console.log(iframe.contentWindow.name);  // Работает
...
1. Только вниз по иерархии:

    ```js
    ✅ site.com → john.site.com
    ✅ john.site.com → site.com
    ✅ john.site.com → peter.site.com

    ❌ john.site.com → example.com (разный домен 2 уровня)
    ```

2. Одинаковый протокол и порт:

    ```js
    ❌ http://john.site.com ≠ https://john.site.com
    ❌ john.site.com:8080 ≠ john.site.com:3000
    ```

3. Устанавливается ДО взаимодействия:

    ```js
    // Неправильно - поздно
    iframe.contentWindow.postMessage(data, '*');  // Ошибка!
    document.domain = 'site.com';

    // Правильно
    document.domain = 'site.com';
    iframe.contentWindow.postMessage(data, '*');  // Работает!
    ```

4. Современные ограничения браузеров

    `document.domain` устарел и заблокирован в:
...
Так что данная опция доступна только для legacy-сайтов на старых браузерах.

✅ *Современные альтернативы*

| Задача        | document.domain      | Современное решение            |
| ------------- | -------------------- | ------------------------------ |
...
#### Айфрейм: подождите документ
Когда айфрейм с того же источника, мы имеем доступ к документу в нём. Но есть подвох. Не связанный с кросс-доменными особенностями, но достаточно важный, чтобы о нём знать.

Когда айфрейм создан, в нём сразу есть документ. Но этот документ – другой, не тот, который в него будет загружен!
...
Нам не следует работать с документом ещё не загруженного айфрейма, так как это не тот документ. Если мы поставим на него обработчики событий, то они будут проигнорированы.

Как поймать момент, когда появится правильный документ?

Правильный документ точно будет доступен, когда сработает `iframe.onload`. Но он срабатывает только тогда, когда загрузится весь айфрейм со всеми его ресурсами.
...
### Атрибут айфрейма sandbox
Атрибут **`sandbox`** позволяет наложить ограничения на действия внутри `<iframe>`, чтобы предотвратить выполнение ненадёжного кода. Атрибут помещает айфрейм в «песочницу», отмечая его как имеющий другой источник и/или накладывая на него дополнительные ограничения.
...
- `allow-top-navigation`
    ...
    Позволяет айфрейму менять `parent.location`.

- `allow-forms`

    Позволяет отправлять формы из айфрейма.

- `allow-scripts`

    Позволяет запускать скрипты из айфрейма.

- `allow-popups`

    Позволяет открывать всплывающие окна из айфрейма с помощью `window.open`.

Больше опций можно найти в [справочнике](https://developer.mozilla.org/ru/docs/Web/HTML/Element/iframe).
...
![Iframe sandboxed](../img/iframe-sandboxed.png)
...
!!! info "На заметку:"
    Атрибут `"sandbox"` создан только для того, чтобы добавлять ограничения. Он не может удалять их. В частности, он не может ослабить ограничения, накладываемые браузером на айфрейм, приходящий с другого источника.
...
Интерфейс `postMessage` состоит из двух частей: отправка сообщения и приём сообщения.

**Первая часть**, ответственная за отправку сообщения, — это метод `win.postMessage(data, targetOrigin)`, который вызывается из окна‑отправителя:
- `data` — данные, которые уходят в другое окно (обычно сериализуются как JSON, если нужна поддержка старых браузеров);

- `targetOrigin` — строка вида 'https://example.com', указывающая, куда именно разрешено отправлять сообщение; если поставить '*', ограничений по источнику не будет (но это менее безопасно).


Таким образом, отправитель может напрямую обращаться к другому окну или `<iframe>` и просить его «принять» определённое сообщение.

**Вторая часть**, ответственная за прием сообщения, — это обработчик события 'message', который нужно повесить на целевое окно:
```js
window.addEventListener('message', function(event) {
  // Проверяем источник
  if (event.origin !== 'https://доверенный-домен.com') return;
...
- событие `MessageEvent`, генерируемое `postMessage`, содержит `event.origin` (источник отправителя) и `event.data` (полезная нагрузка);

- проверка `event.origin` позволяет получателю убедиться, что сообщение пришло именно от ожидаемого домена, что делает обмен кросс‑доменным, но контролируемым и безопасным.

Таким образом, `postMessage` безопасен именно потому, что обе стороны явно согласуют протокол: отправитель выбирает нужный `targetOrigin`, а получатель фильтрует входящие `message` по `origin` и по формату данных.

#### postMessage
Окно, которое хочет отправить сообщение, должно вызвать метод [postMessage](https://developer.mozilla.org/ru/docs/Web/API/Window/postMessage) окна получателя. Другими словами, если мы хотим отправить сообщение в окно `win`, тогда нам следует вызвать `win.postMessage(data, targetOrigin)`.
...
![Post message](../img/post-message.png)
...
[facebook.html](../samples/15_bom/clickjacking_01/facebook.html)

[index.html](../samples/15_bom/clickjacking_01/index.html)

![Clickjacking](../img/clickjacking_01.png)
...
[facebook.html](../samples/15_bom/clickjacking_02/facebook.html)

[index.html](../samples/15_bom/clickjacking_02/index.html)

![Clickjacking](../img/clickjacking_02.png)
...
!!! info "Clickjacking-атака для кликов мыши, а не для клавиатуры"
...
[iframe.html](../samples/15_bom/clickjacking_03/iframe.html)

[index.html](../samples/15_bom/clickjacking_03/index.html)

![Clickjacking](../img/clickjacking_03.png)
...
![iframe sameorigin](../img/iframe-sameorigin.png)
...
[iframe.html](../samples/15_bom/clickjacking_04/iframe.html)

[index.html](../samples/15_bom/clickjacking_04/index.html)

![Clickjacking](../img/clickjacking_04.png)

### Атрибут cookie: samesite
Атрибут `samesite` также может помочь избежать clickjacking-атаки.

Файл куки с таким атрибутом отправляется на сайт только в том случае, если он открыт напрямую, не через фрейм или каким-либо другим способом. Подробно об этом – в разделе, посвященном куки (`document.cookie`).
...
- Или, если мы хотим разрешить отображение страницы во фрейме и при этом оставаться в безопасности, то можно использовать перекрывающий блок `<div>`.[^clickjacking]
...
В качестве примера применения инструментов браузерного API рассмотрим навигацию по одностраничному приложению (*Single Page Application* — SPA).

#### Наивная реализация
В качестве отправной точки можно использовать hash-based навигацию, позволяющую реализовать простейшую навигацию для SPA. Определим простейший одностраничный сайт в виде следующей веб-страницы ***index.html***:
```html
<!DOCTYPE html>
<html>
<head>
...
Большинство упомянутых выше недостатков можно устранить, дополнив обработку кликов по ссылкам реагированием на событие `hashchange`, регистрирующее изменение идентификатора фрагмента (*hash*).
...
[Исходный код](../samples/15_bom/spa/index2.html)

В функции `handleClick` используется `preventDefault()` для дополнительной перестраховки. Браузер по умолчанию для ссылок с #хэш не делает перезагрузку страницы — он просто меняет `location.hash` и вызывает событие `hashchange`. Наш обработчик `handleClick` вручную устанавливает тот же хэш, что браузер и так установил бы сам. Поэтому конкретном в данном случае `preventDefault()` не обязателен — код работает корректно и без него.

Когда `preventDefault()` действительно нужен:
```html
<!-- ❌ Без preventDefault() - ПРОБЛЕМА при относительных ссылках -->
<a href="about.html">About</a>  // Браузер загрузит новый файл!

<!-- ✅ С preventDefault() - SPA работает -->
<a href="about.html">
  event.preventDefault();       // Останавливаем загрузку
  location.hash = 'about';      // Меняем только хэш
</a>
```

Использование `preventDefault()` предотвратит проблемы, если кто-то случайно заменит `#about` на `about.html` или абсолютный URL.

Кроме того, можно заметить, что клики по ссылкам здесь обрабатываются в фазе погружения (*capturing phase*) вместо стандартной фазы всплытия, о чем свидетельствует `true` в `addEventListener("click", handleClick, true)`.

Основная причина — перехват кликов ДО обработки браузером:
```html
<!--  ❌ Без true (по умолчанию) - ПРОБЛЕМА при вложенных ссылках -->
<nav>
  <a href="#home">Home</a>  <!-- SPA обработчик -->
  <a href="https://external.com">External</a>  <!-- Внешняя ссылка -->
</nav>

<!--  Если кто-то добавит вложенность: -->
<a href="#home">
  <span>Click me</span>  <!-- Клик по span "пробьет" вашу ссылку -->
</a>
```

В простом SPA `true` избыточен, но защищает от будущих проблем:

- Вложенные элементы внутри ссылок

- Динамически добавленные обработчики на ссылки

- Сторонние скрипты, меняющие поведение ссылок

По сути это тоже задел на будущее: `true` — это 0 затрат, +100% надежности.

#### Переосмысление
Однако у исправленного решения всё ещё остается недостаток в виде некрасивых ссылок. Было бы гораздо приятнее иметь ссылки с разделителями не в виде хэшей, а слэшей. Кроме того, это гораздо лучше с точки зрения индексации страниц поисковыми системами.

Для реализации подобной маршрутизации (*slashed-based routing*) необходимо, чтобы страницы находились на сервере. Добавим простейший вариант серверного кода для node.js.
...
Запуск сервера осуществляется командой `node app.js`.

[Серверный код](../samples/15_bom/spa/app.js)

Здесь используется библиотека express, существенно упрощающая процесс серверной маршрутизации.

Главное назначение серверного кода — для разных адресов отдавать один и тот же файл, который по-прежнему содержит основной обрабатывающий маршруты код. При таком подходе нет необходимости обрабатывать изменения хэша или следить за загрузкой DOM для выполнения соответствующей маршрутизацией. Вместо этого нужно всего лишь загрузить соответствующую начальную страницу в клиентском скрипте:
```js
// по умолчанию загружаем Home Page
contentElement.textContent = pages.home.content;
```

Универсальную функцию маршрутизации необходимо немного подправить, что заменить хэши на слэши:
```js
function route(url) {
    // получаем имя страницы, которая совпадает с адресом перехода
...
Аналогичную замену следует произвести и в адресах ссылкок:
```html
<nav><a href="/home">Home</a> | <a href="/about">About</a> | <a href="/contacts">Contacts</a></nav>
```

И в объектах, хранящих содержимое страниц:
```js
const pages = {
    home: { content: "Home Page", url: "/home"},
    about: { content: "About Page", url: "/about"  },
...
Теперь, если в функции-обработчике кликов просто вызывать функцию маршрутизации, то увидим, что страница всё время перезагружается и мы не видим предполагаемого содержимого. Это происходит потому, что при нажатии выполняется переход на другой адрес с обращением к серверу и загрузкой того же файла (*index.html*). Это нежелательное развитие событий можно заблокировать, используя метод `preventDefault()`:
```js
function handleClick(event){
    // получаем адрес перехода
    const url = event.target.getAttribute('href');
    route(url);  // обновляем контент
    return event.preventDefault(); // предотвращение перехода
}
```

После этого любая попытка обратиться к серверу при нажатии на ссылку будет остановлена и взамен выполнится клиентский код маршрутизации с загрузкой содержимого соответствующей страницы.

[Клиентский код](../samples/15_bom/spa/index3.html)

Однако теперь можно заметить, что при переходе по ссылкам адресная строка перестала меняться. Проблема в том, что мы сами только что заблокировали переходы по соответствующим адресам и при клике по ссылкам код только обновляет контент, но не меняет адресную строку браузера.

Чтобы адрес менялся без перезагрузки страницы, необходимо использовать History API с методом `pushState()`.

#### Использование History API
History API используется в нашем приложении для создания плавной одностраничной навигации (SPA-подобного опыта) без перезагрузки страницы. Это позволяет решить целый ряд проблем, связанных с ручным обновлением содержимого, например, восстановить полноценную работу с историей браузера.

Сейчас маршруты не пишутся автоматически при `preventDefault()`, так как код перехватывает клики по ссылкам и блокирует стандартное поведение браузера. Для исправления ситуации добавим следующие строки в конце функции `route()`:
```js
// Меняем адресную строку
window.history.pushState(page, page.title, url);
// если браузер не устанавливает заголовок
document.title = page.title;
```

Параметр `title` может игнорироваться браузером, поэтому лучше для перестраховки добавить его отдельно, для чего можно расширить объекты страниц соответствующим свойством:
```js
const pages = {
    home: { content: "Home Page", title: "Home", url: "/home"},
    about: { content: "About Page", title: "About", url: "/about"  },
    contacts: { content: "Contact Page", title: "Contact", url: "/contacts"}
};
```

Теперь при клике по ссылкам адреса меняются, а соответствующее содержимое загружается на страницу, но сломалась история браузера: кнопки Вперед/Назад меняют адресную строку, но это никак не влияет на содержимое страницы.

Каждый раз, когда текущая запись в истории посещений меняется (например, при нажатии на кнопку «Назад» в браузере), срабатывает событие `popstate`. Соответственно, чтобы починить перемещение по истории просмотров с помощью кнопок браузера Назад/Вперед, необходимо обрабатывать данное событие:
```js
window.addEventListener("popstate", (event) => {
    if(!event.state) return;  // игнорируем, если состояния нет

    contentElement.textContent = event.state.content;   // получаем старое состояние
});
```

Теперь переходы по ссылкам и взаимодействие с кнопками истории браузера работают правильно. Осталось починить переход/загрузку по прямой ссылке (или обновление страницы/нажатие F5), поскольку по-прежнему по умолчанию загружается состояние, соответствующее странице "home". Для этого нужно просто добавить следующую строку в клиентский код:
```js
route(window.location.pathname || '/home');
```

[Клиентский код](../samples/15_bom/spa/index4.html)

#### Финальная версия
Осталось исправить некоторые недочеты. Например, мы не хотим, чтобы находясь на определенной странице, пользователь заново загружал данные этой страницы, повторно нажимая на одну и ту же ссылку. И для этой цели берем в истории просмотров текущее состояние и проверяем его свойство `url`. Если текущее состояние (по сути текущая страница) имеет тот же адрес `url`, который запрошен, то нет смысла заново перезагружать содержимое страницы, так что добавим следующую строку в функцию `route()` сразу перед установкой содержимого:
```js
if (history.state?.url == url) return;
```

Это предотвратит повторные переходы, но, к сожалению, заблокирует обновление по <kbd>F5</kbd>, поскольку при этом маршрут остается прежним, что совпадает с содержимым объекта `state` в истории и препятствует рендерингу, а также добавлению объекта в историю. Чтобы исправить ситуацию, можно завести дополнительный параметр в функции `route`, отвечающий за форсированную загрузку содержимого:
```js
function route(url, forced) {
  // получаем имя страницы, которая совпадает с адресом перехода
  const pageName = url.split("/").pop() || 'home';
  // получаем страницу из объекта pages
  const page = pages[pageName] || pages.home;
  // если текущий адрес совпадает с запрошенным, то игнорируем переход
  if (history.state?.url == url && !forced) return;
  // устанавливаем содержимое
  contentElement.textContent = page.content;
  // меняем адресную строку (добавляем в историю)
...
Теперь можно вызывать инициализацию с параметром:
```js
route(window.location.pathname, true);
```

[Клиентский код](../samples/15_bom/spa/index5.html)

#### Заключение
Web API BOM (Browser Object Model) включает ключевые объекты для работы с состоянием браузера: history управляет стеком истории навигации через методы `pushState()`, `replaceState()` и событие `popstate`, позволяя создавать SPA без перезагрузки страниц; `location` предоставляет доступ к компонентам URL (`pathname`, `search`, `hash`) и методам навигации вроде `assign()` или `reload()` для динамического изменения адреса; `navigator` содержит информацию об окружении (`userAgent`, `language`, `geolocation`) и возможности браузера (online статус, permissions). Вместе они образуют основу клиентской маршрутизации, аналитики и адаптивности веб-приложений, обеспечивая бесшовный (*seamless*) UX без дополнительных серверных запросов.
...
В качестве возвращаемого результата метод `window.requestAnimationFrame()` выдает уникальный `id`, который может потом использоваться для остановки анимации:
```js
// получаем id
...
### Выводы
Планирование вызовов в JavaScript — `setTimeout()`, `setInterval()` и `requestAnimationFrame()` — предоставляет мощные инструменты для асинхронного выполнения кода, каждый из которых решает специфические задачи. `setTimeout()` идеален для однократных отложенных действий с гибкой настройкой задержки, `setInterval()` подходит для периодического повторения операций, но требует осторожности с накоплением ошибок и утечками памяти, а `requestAnimationFrame()` обеспечивает оптимальную синхронизацию с частотой обновления экрана для плавных анимаций, минимизируя нагрузку на браузер. Выбор метода зависит от контекста: для таймеров и фоновых задач подойдут первые два, а для визуальных эффектов — `requestAnimationFrame()`. Важно всегда очищать интервалы и таймеры с помощью `clearTimeout()` и `clearInterval()`, чтобы избежать нежелательных повторений и оптимизировать производительность приложения.
...
### Практическая работа. Создание модального окна

Создать универсальное настраиваемое модальное окно на базе компонента `<dialog>`.

1. Реализовать API в виде отдельного класса `CustomModal`, позволяющего создать отдельный экземпляр диалогового окна с функциями управления (открыть, закрыть). 

2. Предусмотреть возможность открытия и закрытия диалога как программным способом, так и по инициативе пользователя (через UI). Добавить CSS-анимации для плавного открытия и закрытия модального окна. Реализовать разные варианты управления несколькими экземплярами:

   - автоматическое закрытие предыдущего при открытии нового;
   - z-index менеджмент для наложения диалогов.

3. Параметризировать создаваемый диалог с тем, чтобы обеспечить возможность гибкой настройки внешнего вида и функционала:
     - расположение: вверху, по центру, внизу;
     - ограничение максимального размера (в пикселях);
     - цвет фона и подложки (*backdrop*);
     - содержимое с возможностью передачи строки HTML;
     - тип диалога: уведомление с кнопками  ОК, подтверждение с кнопками OK/Отмена;
     - возможность открытия нескольких экземпляров.

    <details>
    <summary>Пример базового использования</summary>
    ...
    // Создание экземпляра
    const modal = new CustomModal('confirm', '<p>Сохранить изменения?</p>', options);
...
4. Реализовать закрытие модального окна по нажатию на специальную кнопку закрытия (крестик в верхем правом углу) или клавишу <kbd>Escape</kbd>. Добавить настройку, отвечающую за возможность закрытия диалога по клику вне основной области модального окна (по заднику). Если открытие нескольких экземпляров невозможно, то реализовать автоматическое закрытие открытых до этого диалоговых окон (например, через статический метод `CustomModal`).

5. Отслеживать действия пользователя, сформировав собственную систему событий (`modalOpen`, `modalClose`). Добавить возможность установки обработчиков через специальные методы (`onModalOpen`, `onModalClose`). Обрабатывать выбор пользователя, сохраняя его в данные кастомных событий: например, как тернарную переменную `true` (нажата "ОК") | `false` (нажата "Отмена") | `null` (окно закрыто без нажатия кнопок).

    <details>
    <summary>Пример обработки событий</summary>
    ...
    // Подписка на события
    modal.onModalOpen((detail) => {
      console.log('Диалог начат');
    });

    modal.onModalClose((result) => {
      console.log('Диалог окончен');
...
Оформить работу в виде отдельного файла, содержащего разработанный класс.
...
modal.onModalClose((result) => {
  console.log('Модал закрыт, результат:', result);
  // result: { confirmed: true/false, choice: 'ok'|'cancel'|'escape'|'backdrop' }
...
Реализация класса с системой событий
```js
class CustomModal {
  ...
  // Методы подписки
  onModalOpen(handler) {
    this.handlers.modalOpen.push(handler);
  }

  onModalClose(handler) {
    this.handlers.modalClose.push(handler);
  }

  onModalConfirm(handler) {
    this.handlers.modalConfirm.push(handler);
  }

  onModalCancel(handler) {
    this.handlers.modalCancel.push(handler);
  }
  ...
  open() {
    ...
    this._dispatch('modalOpen', {});
  }

  close(result = null) {
    this.dialog.close();
    this._dispatch('modalClose', result);
  }

  _createDialog() {
    this.dialog = document.createElement('dialog');
    this.dialog.className = `custom-modal custom-modal--${this.options.position}`;

    const buttons = this._createButtons();
    this.dialog.innerHTML = `
      <div class="modal-content" style="background: ${this.options.backgroundColor}; max-width: ${this.options.maxWidth}">
  ...
  _createButtons() {
    if (this.type === 'alert') {
      return '<button class="modal-btn modal-btn-ok">OK</button>';
    }
    return `
  ...
  _bindEvents() {
    ...
    this.dialog.addEventListener('click', (e) => {
      if (e.target.classList.contains('modal-btn-ok')) {
        this._handleConfirm();
      } else if (e.target.classList.contains('modal-btn-cancel')) {
        this._handleCancel();
  ...
  _handleConfirm() {
    const result = { confirmed: true, choice: 'ok' };
    this._dispatch('modalConfirm', result);
    this.close(result);
  }

  _handleCancel() {
    const result = { confirmed: false, choice: 'cancel' };
    this._dispatch('modalCancel', result);
    this.close(result);
  }

  _dispatch(eventName, detail) {
    this.handlers[eventName].forEach(handler => handler(detail));
  }
  ...
  _applyStyles() {
    ...
    style.textContent = `
      ...
      .custom-modal {
        border: none;
        border-radius: 8px;
        animation: modal-${this.options.animation} 0.3s ease;
      }
      ...
      .modal-btn { padding: 8px 16px; border-radius: 4px; border: 1px solid #ccc; }
      .modal-btn-ok { background: #007bff; color: white; border-color: #007bff; }
    `;
...
```js
// 1. Базовый сценарий
const saveModal = new CustomModal('confirm', 'Сохранить изменения?');
saveModal.onModalConfirm(() => saveData());
saveModal.onModalCancel(() => console.log('Отменено'));
saveModal.open();

// 2. Сложная логика
const deleteModal = new CustomModal('confirm', 'Удалить файл?', options);
deleteModal.onModalClose((result) => {
  if (result.confirmed) {
    deleteFile(result.detail?.fileId);
  }
});
deleteModal.open();

// 3. Цепочка модалов
firstModal.onModalConfirm(() => {
  secondModal.open();
});
```
...
### Практическая работа. Создание одностраничного веб-приложения

#### Задание
Создать простейшее одностраничное приложение на базе hash-based маршрутизации.
...
### Глоссарий
PKCE (*Proof Key for Code Exchange*, произносится «пикси»)
: расширение протокола OAuth 2.0, разработанное для безопасного обмена кодами авторизации. Оно защищает мобильные и одностраничные приложения (SPA) от атак перехвата кода (*authorization code interception*), требуя от клиента доказать владение уникальным секретом (`code_verifier`) при каждом запросе.
