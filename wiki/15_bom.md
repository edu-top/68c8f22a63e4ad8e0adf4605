...
Это предотвратит повторные переходы, но, к сожалению, заблокирует обновление по <kbd>F5</kbd>, поскольку при этом маршрут остается прежним, что совпадает с содержимым объекта `state` в истории и препятствует рендерингу и добавлению объекта в историю. Чтобы исправить ситуацию, можно добавить дополнительный параметр в функцию `route`, отвечающий за форсированную загрузку содержимого:
```js
