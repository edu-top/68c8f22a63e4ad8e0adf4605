...
# Основы CSS. Замещаемые элементы

- [Замещаемые элементы](#замещаемые-элементы)
  - [Ключевые особенности](#ключевые-особенности)
  - [Встраивание содержимого](#встраивание-содержимого)
  - [MIME](#mime)
    - [MIME-типы](#mime-типы)
- [Изображения](#изображения)
  - [Элемент `img`](#элемент-img)
    - [Атрибуты](#атрибуты)
    - [Заполнение `alt`](#заполнение-alt)
      - [Основные правила заполнения](#основные-правила-заполнения)
      - [`alt` и поисковики](#alt-и-поисковики)
  ...
  - [Векторная графика](#векторная-графика)
    ...
    - [Как подключать](#как-подключать)
      - [Использование как изображения: `<img>`](#использование-как-изображения-img)
      - [Подключение в виде фона: `background-image`](#подключение-в-виде-фона-background-image)
      - [Прямая вставка кода в HTML](#прямая-вставка-кода-в-html)
      - [Встраивание через объект: `<object>`](#встраивание-через-объект-object)
      - [Подключение через спрайт](#подключение-через-спрайт)
  ...
  - [Фоновые изображения](#фоновые-изображения)
    - [Настройка области изображения](#настройка-области-изображения)
      - [Свойство `background-clip`](#свойство-background-clip)
      - [Свойство `background-origin`](#свойство-background-origin)
      - [Свойство `background-position`](#свойство-background-position)
      - [Свойство `background-size`](#свойство-background-size)
    - [Картинка плюс цвет](#картинка-плюс-цвет)
    - [Несколько фоновых картинок](#несколько-фоновых-картинок)
    - [Несколько разрешений и типов фонового изображения](#несколько-разрешений-и-типов-фонового-изображения)
    - [Градиент на фоне](#градиент-на-фоне)
    - [Отличия фоновых изображений от обычных](#отличия-фоновых-изображений-от-обычных)
- [Настройка заменяемых элементов](#настройка-заменяемых-элементов)
  - [Свойство `object-fit`](#свойство-object-fit)
  - [Свойство `object-position`](#свойство-object-position)
  - [Свойство `object-view-box`](#свойство-object-view-box)
- [Видео](#видео)
  - [Элемент `video`](#элемент-video)
  - [Атрибуты](#атрибуты-2)
...
- [Встраиваемое содержимое](#встраиваемое-содержимое)
  - [Элемент `iframe`](#элемент-iframe)
    - [Атрибуты](#атрибуты-4)
  - [Элемент `object`](#элемент-object)
  - [Элемент `embed`](#элемент-embed)
    - [Атрибуты](#атрибуты-5)
  - [Выбор механизма встраивания](#выбор-механизма-встраивания)
- [Адаптивные изображения](#адаптивные-изображения)
  - [Базовые механизмы](#базовые-механизмы)
    ...
    - [Интерполяция изображений](#интерполяция-изображений)
...
## Замещаемые элементы
<dfn title="заменяемые элементы">Заменяемые (замещаемые) элементы</dfn> (*replaced elements*) в CSS — это элементы, чье представление выходит за рамки модели форматирования CSS. Их содержимое формируется внешними ресурсами или пользовательским агентом (браузером, а не обычным HTML-контентом или текстом, управляемым CSS) и отображается внешним объектом, поэтому CSS обычно не контролирует внутреннее отображение этого элемента.

Другими словами, <dfn title="замещаемый элемент">замещаемый элемент</dfn> — это HTML-элемент, отображающий внешний ресурс, например, картинку или видео. Браузер берёт содержимое не из HTML-разметки, а подставляет готовое «встроенное» представление. В CSS мы можем менять размер и расположение такого элемента, но не управлять его внутренним содержимым.[^object-view-box]

Основные примеры:
- `<img>` — изображение, заменяется контентом из атрибута src.

- `<video>` — видеоплеер, воспроизводит медиафайлы.

- `<audio>` — аудиоплеер, воспроизводит медиафайлы.

- `<iframe>` — встроенный фрейм, отображает другой HTML-документ.

- `<embed>`, `<object>` — встроенный внешний объект (например, плагин).

- `<canvas>` — область для рисования с помощью JavaScript.

- `<input>` — поля ввода, так как они отображаются как виджеты операционной системы. Например, `<input type="image">` ведет себя как изображение.

Псевдоэлементы (`::before`, `::after`) также могут создавать заменяемые элементы (анонимные) с помощью свойства `content`. Свойство `content` используется для замены содержимого элемента сгенерированным значением.

### Ключевые особенности
Такие элементы имеют внутреннюю (интринсическую — *intrinsic*) размерность — собственные встроенные размеры (*natural dimensions* — ширину и высоту), определяемые самим ресурсом (например, пиксели изображения или соотношение сторон видео), независимо от CSS.

Именно поэтому для них нужны специальные свойства вроде `object-fit` и `object-position` — обычные `width`/`height` лишь задают контейнер, а содержимое подстраивается отдельно. Например, свойство `object-fit` применяется для управления тем, как заменяемый элемент (`img`, `video`) вписывается в свой контейнер (`cover`, `contain`, `fill`).

Обычно вы не можете стилизовать внутренности заменяемого элемента (например, элементы внутри `<iframe>` или кнопки внутри `<video>`) напрямую из CSS. В отличие от обычных элементов (`<div>`, `<p>`), их отображение выходит за рамки CSS модели форматирования, поэтому браузер рассчитывает их размеры по своим алгоритмам.

### Встраивание содержимого
В HTML существует несколько ключевых способов встраивания внешнего или мультимедийного содержимого в документ, таких как изображения, видео, страницы или плагины. Эти механизмы позволяют интегрировать ресурсы без нарушения структуры основной страницы.

- **Изображения**

  Для изображений используют элемент `<img>` с обязательным атрибутом `src` для URL ресурса. Более гибкий вариант — `<picture>` с вложенными `<source>`, который адаптирует изображение под устройство (например, по `srcset` для разных разрешений).

- **Мультимедиа**

  Видео и аудио встраивают через `<video>` и `<audio>` с атрибутами `src` или вложенными `<source>` для поддержки форматов (MP4, WebM). Для внешних сервисов (YouTube) применяют `<iframe>` с embed-ссылкой.

- **Страницы**

  Элемент `<iframe>` создаёт изолированный фрейм для другой HTML-страницы, с атрибутами `src`, `width`, `height` и `sandbox` для безопасности (ограничение скриптов, форм).
...
<dfn title="MIME">MIME</dfn> (/maɪm/, *Multipurpose Internet Mail Extensions* — многоцелевые расширения интернет-почты) — это стандарт, который описывает способы передачи различных типов данных (не-ASCII текста, вложений типа изображений, аудио, видео, документов и многосоставных сообщений) по электронной почте, а также, в общем случае, спецификация для кодирования информации и форматирования сообщений таким образом, чтобы их можно было пересылать по Интернету.[^MIME] Изначально был создан для почты (SMTP), но стал стандартом для HTTP-заголовков (`Content-Type`), определяя формат данных между сервером и клиентом. Он определяет тип содержимого (MIME-тип) — строку (например, `image/png`, `text/html`), указывающую браузеру или почтовому клиенту, как обрабатывать файл. MIME позволяет форматировать сообщения, содержащие не только текст, но и вложения, обеспечивая их корректное отображение в почтовых программах и браузерах.

Основные понятия MIME:
- <dfn title="MIME-типы">MIME-тип</dfn> (называемый "internet media type", а иногда "content type"): строка, отправляемая вместе с файлом, определяющая его формат (например, передаваемый аудиофайл может быть помечен как `audio/ogg` тип, а изображение — `image/png`). Состоит из основного типа (*type*) и подтипа (*subtype*), разделенных косой чертой, например, `image/png`, `audio/ogg` или `application/pdf`. Типы по умолчанию: `text/plain` (для текста) и `application/octet-stream` (для бинарных данных, если тип неизвестен).
...
Основной формат электронных сообщений определен в [RFC 5322](https://datatracker.ietf.org/doc/html/rfc5322), который является обновлённой версией [RFC 2822](https://datatracker.ietf.org/doc/html/rfc2822) (который, в свою очередь, является обновлённой версией [RFC 822](https://datatracker.ietf.org/doc/html/rfc822)). Эти стандарты определяют похожие форматы для текстовых e-mail-заголовков и содержимого и правил, относящихся к общеиспользуемым полям, таким как `To:`, `Subject:`, `From:` и `Date:`. MIME определяет набор e-mail-заголовков для определения дополнительных атрибутов сообщения, включая тип контента, и определяет множество кодировок, которые могут быть использованы для представления 8-битных бинарных данных с помощью символов из 7-битного ASCII. MIME также определяет правила для кодирования символов из Extended ASCII (с кодами 128—255) в заголовках e-mail-сообщения, таких как `Subject:`.

MIME расширяем для новых типов — его определение включает метод для регистрации новых типов контента и других атрибутов.[^MIME]

!!! info "Про почтовые протоколы"

    <dfn title="SMTP">SMTP</dfn> (Simple Mail Transfer Protocol) — это стандартный сетевой протокол, предназначенный для отправки электронной почты. Он работает в архитектуре клиент-сервер, обеспечивая передачу сообщений от почтового клиента отправителя на почтовый сервер и далее между серверами до получателя. SMTP используется только для отправки, в то время как за получение отвечают протоколы POP3 или IMAP.
...
<dfn title="Internet Media Types">Internet Media Types</dfn>, также известные как MIME-типы (Multipurpose Internet Mail Extensions), представляют собой стандарты для обозначения типов данных, которые могут быть переданы посредством сети Интернет с применением стандарта MIME. MIME-типы используются для указания формата данных, отправляемых и получаемых в электронной почте, веб-браузерах и других интернет-приложениях.

MIME-тип играет точно такую же роль, как и расширение файла в системе Windows. Когда HTTP-сообщение содержит `Content-type` заголовок, тело запроса будет парситься в соответствии с MIME-типом, указанным в заголовке.[^MIME_type]
...
Ниже приведён список MIME-заголовков и расширений файлов, которые соответствуют различным типам данных, используемым в интернете.[^Список_MIME-типов]

Согласно RFC 2045, RFC 2046, RFC 4288, RFC 4289 и RFC 4855, выделяются следующие базовые (общие) типы передаваемых данных:

- `application`: для обозначения данных приложений или бинарных данных, таких как файлы программного обеспечения, документы и сжатые файлы.
- `audio`: для обозначения аудиофайлов и потоков, таких как MP3 и Ogg.
- `example`: для обозначения примеров данных, часто используется в документации и учебных материалах.
- `image`: для обозначения изображений, таких как JPEG, PNG и GIF.
- `message`: для обозначения целых сообщений, таких как электронные письма.
- `model`: для обозначения 3D-моделей и данных, связанных с ними, таких как VRML и 3D-объекты.
- `multipart`: для обозначения сообщений, состоящих из нескольких частей, таких как вложения в электронной почте.
- `text`: для обозначения текстовых данных, таких как HTML, CSS и простые текстовые файлы.
- `video`: для обозначения видеофайлов и потоков, таких как MP4 и WebM.
...
Эти стандарты, разработанные и поддерживаемые Инженерным советом Интернета (IETF) и другими организациями, обеспечивают единообразие и совместимость в передаче данных в интернете. Полный список зарегистрированных MIME-типов доступен в базе данных IANA, которая управляет регистрацией и поддержкой этих типов.

## Изображения
В HTML для работы с изображениями используют несколько связанных элементов: `<img>`, `<picture>`, `<source>` и `<figure>`. Они позволяют не только вставить картинку на страницу, но и сделать её адаптивной, добавить альтернативные форматы и оформить подпись или смысловую группу.

- `<img>` — это основной тег для отображения изображения. Он вставляет файл по адресу, указанному в атрибуте `src`, а атрибут `alt` помогает описать изображение для доступности и в случае, если оно не загрузится.

- `<picture>` применяют тогда, когда нужно подать браузеру несколько вариантов одного изображения. Внутри него размещают элементы `<source>`, которые задают разные источники в зависимости от условий, например ширины экрана, плотности пикселей или поддерживаемого формата.

- `<source>` сам по себе изображение не отображает, а лишь предлагает браузеру выбрать подходящий файл. Такой подход особенно полезен, если нужно использовать современные форматы вроде WebP или AVIF, но при этом сохранить запасной вариант для старых браузеров.

- `<figure>` используют для смыслового объединения изображения с подписью, обычно через `<figcaption>`. Этот элемент помогает оформить иллюстрации, схемы, графики и другие самостоятельные визуальные блоки как отдельные части контента.
...
В [руководстве Google по поисковой оптимизации](https://developers.google.com/search/docs/appearance/google-images/) нет прямого упоминания о влиянии `alt` на положение сайта в выдаче, но есть предупреждение о неправильном использовании атрибута. В содержимом `alt` нет места продающим словам вроде «стоимость», «оформить», «купить» и другим. Также не стоит «спамить» ключевыми словами. Это потенциально понизит сайт в выдаче, а Google даже может пометить его как спам. Такая же проблема возникнет, если у всех картинок с одного сайта одинаковое описание.[^alt]

#### Создание наложения

🛠 Частый дизайнерский приём — наложение поверх картинки цветного оверлея ("вуали"). И если для фоновой картинки это можно сделать при помощи псевдоэлемента, то с тегом `<img>` этот приём не сработает. Для этого элемента невозможно задать псевдоэлемент, так уж устроены *замещаемые* элементы, вроде `<iframe>`, `<video>`, `<img>` и других.
...
SVG (*Scalable Vector Graphics* — масштабируемая векторная графика) — формат векторной графики на основе XML, доступный в вебе и предназначенный для отображения двухмерных изображений в веб-браузерах. В отличие от растровых (PNG, JPG), SVG-изображения масштабируются без потери качества, имеют меньший размер файла для простых фигур и поддерживают анимацию, стилизацию через CSS и скрипты. SVG — это открытый стандарт, разработанный World Wide Web Consortium (W3C)
...
Как вы видите, второй вариант выглядит великолепно в отличие от первого. Это нам на руку при работе с адаптивными сайтами и с сайтами, которые должны отлично смотреться на экранах с повышенной плотностью пикселей (так называемые ретина-экраны).
...
!!! warning "Ограничения свойства"
    ...
    🛠 CSS-свойство `stroke` влияет на цвет обводки только при вставке всего SVG-кода непосредственно в HTML — SVG-изображение, вставленное через SVG-шрифт, `<img>`, `<object>` или `background`, `background-image` не увидит его.[^stroke] Если SVG подключён как отдельный файл через `<img>`, `object`, `background` или `background-image`, он ведёт себя как изображение, и внешние CSS-правила страницы обычно на него не влияют.
...
- **Подключение в качестве изображения**: тег `<img>` — простой и понятный способ, хорошо подходит для обычных изображений, но SVG внутри него сложнее стилизовать.

- **Фоновое изображение через CSS** — удобно для декоративных иконок и элементов интерфейса, когда SVG не нужно изменять из кода страницы.

- **Встраивание прямо в HTML** — удобно, если нужно управлять цветом, размером и отдельными элементами SVG через CSS и JavaScript.

- **Встраивание через объект**: тег `<object>` — позволяет подключать отдельный SVG-документ и при необходимости взаимодействовать с ним отдельно от основной страницы.

- **Использование SVG-спрайта** — полезно, если на странице много одинаковых иконок и важно сократить количество повторяющегося кода.

##### Использование как изображения: `<img>`
...
##### Подключение в виде фона: `background-image`
...
Неприятная особенность: если одна и та же SVG-картинка используется в нескольких местах на сайте, и в какой-то момент вы захотите в ней что-то изменить, то нужно будет найти все вставки и вносить исправления в код. При вставке SVG при помощи `<img>` или `background-image` достаточно будет внести изменения в svg-файл и правки применятся ко всем картинкам.

##### Встраивание через объект: `<object>`
Ещё один способ — подключить SVG через тег `<object>.` В этом случае изображение загружается как отдельный документ, поэтому его удобно использовать, когда SVG должен оставаться самодостаточным.

```html
<object
  type="image/svg+xml"
  data="img/svg/dots.svg">
</object>
```

У такого подхода есть важное преимущество: SVG сохраняет свою структуру и может открываться как отдельный файл. Но есть и недостаток — стилизовать его через CSS страницы обычно нельзя так же гибко, как встроенный SVG.

##### Подключение через спрайт
<dfn title="SVG-sprite">SVG-sprite</dfn> — это набор иконок, объединённых в один SVG-файл, где каждая иконка хранится в отдельном символе. Такой способ особенно удобен, когда на странице используется много одинаковых графических элементов.
...
Преимущество спрайта в том, что браузеру не нужно загружать много отдельных файлов, а значит, уменьшается количество запросов к серверу. Кроме того, одну и ту же иконку можно использовать несколько раз без дублирования SVG-кода.

Но у этого способа есть и ограничения: если спрайт подключён как внешний файл, не все стили можно менять через CSS так же свободно, как у встроенного SVG. Поэтому SVG-sprite чаще всего используют для набора простых иконок интерфейса.

> SVG-sprite позволяет переиспользовать иконки и частично стилизовать их через CSS, но по гибкости обычно уступает SVG, встроенному прямо в HTML.

Если спрайт встроен прямо в HTML, то и цвет, и анимацию, и другие стили можно менять почти так же, как у обычного inline-SVG. Если же спрайт подключён внешним файлом через `<use>`, то доступ к внутренним элементам обычно уже ограничен, и часть CSS-анимаций или точечной стилизации может не работать так свободно.

Что можно сделать со спрайтом:
- Менять `fill` и `stroke` у иконок.

- Управлять размером через CSS.

- Переиспользовать одну и ту же иконку много раз.

- Иногда анимировать простые свойства, если структура и способ подключения это позволяют.

У встроенного SVG больше свободы: можно анимировать отдельные части, обращаться к `path`, `circle`, `rect` и так далее. У спрайта эта гибкость часто ниже, зато он удобнее для больших наборов одинаковых иконок.
...
!!! tip "Несколько советов"
    ...
    - 💡 Всегда перед использованием SVG в проекте прогоняйте его через оптимизатор [SVGO](https://svgo.dev/). Он удалит всё ненужное, код станет приятнее и будет меньше весить.
...
### Фоновые изображения
Свойство `background-image` задаёт элементу фоновую картинку. Это полезное свойство, если нужно поставить на фон картинку или задать декоративное изображение. Можно даже градиент!
...
!!! attention "Различия в отображении"
...
Границы картинки и её расположение относительно краёв элемента мы задаём с помощью `background-clip` и `background-origin`. Масштабирование осуществляется с помощью свойства `background-size`.

#### Настройка области изображения
Для настройки области изображения используют ряд CSS-свойств. Свойство `background-clip` решает, должен ли фон заполнять площадь всего элемента или не выходить за рамки контентной области, а `background-origin` — управляет тем, какую область фоновая картинка будет занимать внутри элемента. Фоновая картинка также подчиняется свойству `background-position` и занимает нужную позицию. Если нужно чтобы фоновая картинка заняла всю площадь элемента, то следует использовать свойство `background-size`.

##### Свойство `background-clip`
Свойство **`background-clip`** определяет, в каких областях будет находиться фоновая картинка и фоновый цвет блока.[^background-clip]
...
Когда добавляется фоновое изображение, браузер должен знать, откуда его начинать рисовать. Именно это определяет `background-origin`.
...
Если у элемента есть рамка или отступы, это визуально меняет положение фонового изображения, даже если его размер не меняется.[^background-origin]
...
[Демо](../samples/css/background-origin.html)
...
[Демо](../samples/css/bg-origin.html)

!!! tip "Замечания по поводу использования"
    ...
    💡 Если для фона указано свойство `background-attachment` со значением `fixed`, то `background-origin` игнорируется — фон будет привязан к видимому пространству, а не к элементу.
...
[Демо](../samples/css/background-position_1.html)
...
[Демо](../samples/css/background-position_2.html)
...
🛠 Помимо перечисленных вариантов можно использовать функцию `calc()` для указания более гибкого значения.[^background-position]

##### Свойство `background-size`
Свойство `background-size` позволяет изменять размер фонового изображения. Если фоновая картинка не совпадает по размеру с размерами блока, то при помощи этого свойства можно сделать так, чтобы она, картинка занимала всю площадь блока или, наоборот, была определённого размера.[^background-size]
...
!!! info "Замечания по поводу значений свойства"
...
![Фон с красивой панорамой](../img/landscape-1200w.webp)
...
[Демо](../samples/css/background-size_1.html)
...
[Демо](../samples/css/background-size_2.html)
...
[Демо](../samples/css/background-size_3.html)
...
![Примеры background-size](../img/contain-and-cover-1200w.webp)

!!! note "Как часть сокращенного свойства"

    🛠 Если для задания фона используется шорткат `background`, то значение для `background-size` указывается последним, после слэша `/`. Пример:
...
[Демо](../samples/css/background-size_4.html)

Демо позволяет изменять размеры фоновой картинки и посмотреть, как будет меняться фоновый паттерн.
...
![Background-image w/ color](../img/background-image-color.png)

[Демо](../samples/css/bg-img_1.html)
...
![Broken image link](../img/broken-img-link_1.png)
...
![Broken image link](../img/broken-img-link_2.png)
...
![Multiple background images](../img/bg-img_2.png)

[Демо](../samples/css/bg-img_2.html)
...
CSS-функция `image-set()` позволяет перечислить несколько фоновых изображений с условиями, по которым браузер решит, какое из них наиболее подходящее для загрузки в данный момент.[^image-set]

По принципу работы она похожа на тег `<picture>`. Внутри круглых скобок указываем через запятую две и более ссылки на изображения при помощи функции `url()` и пишем условие для выбора этой картинки.
...
Проверить поддержку можно в [демке](../samples/css/image-set.html).
...
![Broken image link](../img/broken-img-link_3.png)

#### Отличия фоновых изображений от обычных
...
Чтобы это исправить, нужно добавить элементу необходимые ширину и высоту. А ещё не забыть про `background-repeat` и `background-size`, чтобы фон выглядел отлично.[^background-image]

## Настройка заменяемых элементов
Фоны изначально предназначались для декоративных целей, поэтому их развивали первыми для типографики и макетов в CSS2/CSS3. По мере эволюции CSS от статичных к динамичным и контент-ориентированным макетам возникла необходимость управления семантическими элементами (вроде `<img>`, `<video>`) с собственными размерами и поведением, требующими учета доступности, SVG, видео и соотношения сторон. Это потребовало отдельных модулей CSS Images. До 2010-х проблемы адаптивности для изображений решали через JavaScript или хаки; новые свойства `object-*` создали для нативной responsive-поддержки даже без `<picture>`.

Свойства `object-fit`, `object-position` и `object-view-box` предназначены для управления отображением заменяемых элементов, таких как `<img>` или `<video>`, и по сути являются аналогами свойств, уже давно имеющихся для фоновых изображений. Они позволяют гибко масштабировать, позиционировать и обрезать контент внутри контейнера, подобно `background-size`, `background-position` и `background-clip`.

Свойство `object-fit` позволяет подстроить картинку или видео под контейнер без деформации, `object-position` определяет, как картинка или видео должны располагаться внутри контейнера, `object-view-box` кадрирует изображение с помощью CSS.

*Аналоги фоновых свойств*

| Свойство для `img`/`video` | Аналог для `background-*`                        | Назначение                                                                                                               |
| ---------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `object-fit`             | `background-size`                                | Масштабирование: `cover`, `contain`, `fill` подстраивают изображение под контейнер без искажений.                       |
| `object-position`        | `background-position`                            | Позиционирование: `center top`, `50% 30%` сдвигают видимую область.                                                   |
| `object-view-box`        | Частично `background-clip` | Определяет "окно просмотра" как в SVG `viewBox`, используя `inset()` или `xywh()` для фокусировки на части изображения. |

Эти свойства применяются напрямую к элементу (`<img>`, `<video>`), а не к контейнеру, что делает их семантически правильными для контента (в отличие от декоративных фонов). Они работают только при заданных размерах контейнера и идеальны для адаптивных изображений без JavaScript.

Данные свойства идеальны для адаптивных изображений в отзывчивом (*responsive*) дизайне, где `<img>` предпочтительнее фоновых (*background*) изображений для доступности и SEO.

### Свойство `object-fit`
Свойство, которое позволяет управлять тем, как картинка `<img>` или видео `<video>` будет подстраиваться под заданные размеры.

По своему поведению очень похоже на свойство `background-size` для фоновых изображений. Дословно свойство переводится как «объект подходит», что вполне точно отражает принцип его работы. Свойство управляет тем, как подгружаемая картинка будет вписываться в рамки заданных размеров.
...
[Демо](../samples/css/object-fit_1.html)
...
Картинка не деформировалась, подстроилась под нужные размеры. Другой вопрос, что на ней теперь почти ничего не видно 😅, но это мелочи.[^object-fit]
...
!!! tip "На практике"
    ...
    🛠 Пожалуй, самое полезное из всех доступных значений — это `cover`. Оно позволяет имитировать поведение фонового изображения для элементов `<img>` или `<video>`. В последнем случае это особенно актуально потому что нет других способов красиво адаптировать видео под любой размер экрана без нарушения пропорций по дороге.

[Демо](../samples/css/object-fit_3.html)

!!! info "Замечания по поводу свойства"
    ...
    💡 Свойство и его значение не наследуется (хотя внутрь картинки сложно что-то вложить)

    💡 Не анимируется 😒

Свойство `object-fit` очень здорово работает в сочетании со свойством `object-position`.

### Свойство `object-position`
Свойство помогает спозиционировать отрисовку картинки `<img>` или видео `<video>` внутри контейнера, если к нему применили свойство `object-fit`.[^object-position]

Например, расположим изображение по верхнему краю элемента `<img>` и на расстоянии 50 пикселей от левого края. Для наглядности обозначим границы элемента.
...
[Демо](../samples/css/object-position_1.html)
...
[Демо](../samples/css/object-position_2.html)
...
### Свойство `object-view-box`
Свойство `object-view-box` задаёт, какая часть изображения должна быть видна.[^object-view-box]
...
[Демо](../samples/css/object-view-box_3.html)
...
У `inset()` и `xywh()` разная логика задания области, хотя результат может быть похожим. `inset()` описывает отступы от краёв исходной области, а `xywh()` задаёт координаты и размеры прямоугольника напрямую.

- `inset(top right bottom left)` удобно читать как «сдвинь границы внутрь».
Например, `inset(15% 15% 15% 15%)` означает, что область обрезки отступает на 15% от каждой стороны. Это похоже на то, как работает `inset()` в `clip-path`.

- `xywh(x y width height)` задаёт прямоугольник через его положение и размер.
Например, `xywh(10% 10% 75% 75%)` означает: начни с точки (10%, 10%) и задай область шириной 75% и высотой 75%. Такой вариант удобнее, когда нужно мыслить не от краёв, а от конкретной позиции и габаритов.

Главное отличие:
- `inset()` удобнее, если вы хотите обрезать одинаково со всех сторон или просто задать поля.

- `xywh()` удобнее, если важны точные координаты верхнего левого угла и размер окна.
...
[Демо](../samples/css/object-view-box_1.html)
...
[Демо](../samples/css/object-view-box_2.html)

Таким образом, функция `inset()` задаёт область отображения через отступы от каждой стороны, а `xywh()` — через координаты левого верхнего угла и ширину с высотой. Поэтому `inset()` удобнее для симметричного кадрирования, а `xywh()` — для более точного позиционирования окна отображения.

## Видео
Для воспроизведения видео в HTML5 используется элемент `<video>`. Данный элемент дает нативный способ вставить видео на страницу сайта.  Вместе с ним часто применяются вспомогательные элементы: `<source>` для указания альтернативных источников видео в разных форматах (MP4, WebM, OGG), `<track>` для добавления субтитров, текстовых описаний или глав, а также атрибуты вроде `controls`, `autoplay`, `loop` и `muted` для управления воспроизведением.

### Элемент `video`
Тег `<video>` добавляет на страницу видеоплеер, который может воспроизвести одно или несколько видео.

В самом простом случае ссылку на видео задают через атрибут `src` прямо у самого `<video>`:
```html
...
- `src="my-video.mp4"` — указывает путь к видеофайлу напрямую.

- `controls` — показывает нативные элементы управления (воспроизведение, пауза, громкость и т.д.).

- Текст внутри `<video></video>` выводится только в браузере, который не поддерживает `video`.

Такой способ проще, но он не позволяет указать несколько форматов (MP4/WebM/Ogg) — это делается через вложенные `<source>`.
...
!!! tip "Поддержка новых форматов"
    ...
    🛠 Не забывайте добавлять хотя бы два дублирующих видео в формате WebM и MP4. Суть в том, что WebM очень мощно сжимает ролики, и если браузер поддерживает формат, то видео загрузится сильно быстрее и не расходует трафик. Ну а если вы используете старый браузер без поддержки WebM, то откроется обычный MP4 и сработает как обычно.
...
### Фоновое видео
На некоторых сайтах можно встретить фоновое видео вместо статичной картинки. Выглядит эффектно, а делается очень просто:
```html
...
## Аудио
Для воспроизведения звука без видео в HTML5 применяется элемент **`<audio>`**. Тег `<audio>` встраивает аудиофайл на страницу, создавая нативный аудиоплеер, управляемый средствами браузера. Этот элемент во многом аналогичен `<video>`, но предназначен исключительно для звукового контента и не показывает никакого визуального кадра.

Для обеспечения совместимости с разными браузерами используется вложенный элемент `<source>`. Он задаёт несколько форматов одного и того же аудиофайла — браузер выберет тот, который поддерживается.

Также внутри `<audio>` можно применять элемент `<track>` для добавления текстовых дорожек, например субтитров, транскрипций или описаний для людей с нарушениями слуха. Текст, расположенный внутри контейнера `<audio>`, отображается только в тех браузерах, которые не поддерживают встроенный аудиоплеер, и может содержать ссылку для скачивания файла.
...
- `preload`: устанавливает режим загрузки файла. Подсказывает браузеру, нужно ли загружать аудио или информацию о нём сразу со страницей. Без этого атрибута предварительная загрузка файла будет зависеть от настроек конкретного браузера. У этого атрибута несколько значений:
    ...
    - `metadata` — загружается только информация о файле, например, размер и продолжительность. Рекомендуется использовать именно этот параметр, чтобы не загружать аудио, пока пользователь не захочет его прослушать;
    - `auto` — аудиофайл загружается со страницей, чтобы пользователь мог сразу начать его слушать. Если не указывать никакое значение `preload`, то атрибут будет работать как `auto`. Следует учитывать, что если стоит атрибут `autoplay`, то `preload` не работает.
...
!!! warning "О поддержке `kind='descriptions'`"
    ...
    На данный момент `kind='descriptions'` не поддерживается ни одним браузером, проверяйте поддержку на [Can I Use](https://caniuse.com/?search=descriptions).
...
<dfn title="автономное содержимое">Автономное содержимое</dfn> (*self-contained content*) — это блоки элементов, представляющие самостоятельные фрагменты информации: иллюстрации, графики, примеры кода, цитаты и другие объекты, которые логически отделимы от основного потока текста и при необходимости могут быть перемещены в документе без потери смысла. Такие фрагменты часто сопровождаются отдельными подписями или примечаниями и служат для наглядного дополнения или иллюстрации основной мысли, не нарушая структуру основного контента. В HTML для разметки автономного содержимого используется элемент `<figure>`, а его подпись — `<figcaption>`.
...
![Figure и figcaption в HTML5](../img/figure-1.34.png)
...
<dfn title="встраиваемое содержимое">Встраиваемое содержимое</dfn> (*embedded content*) — это ресурсы третьих сторон или отдельные файлы (видео, аудио, PDF, презентации, интерактивные элементы и т. п.), которые можно разместить прямо на веб‑странице, чтобы предоставить пользователю мультимедийный или интерактивный опыт без перенаправления на другие сайты. В HTML для этой цели используются элементы вроде `<object>`, `<embed>` и `<iframe>`, каждый из которых имеет свои особенности по поддержке форматов, обработке фолбэков и взаимодействию с плагинами или встроенными проигрывателями. Правильный выбор и настройка таких тегов помогают обеспечить совместимость, доступность и удобство использования содержимого.
...
![iframe в html5](../img/iframe-1.33.png)
...
Этот контейнер обычно называют фреймом. Внешний вид фрейма и его положение на странице можно задать через CSS.[^iframe]
...
!!! warning "Скорость загрузки содержимого"
...
- `src` — URL-адрес файла, который вы встраиваете. Устанавливает полный путь к загружаемому ресурсу.
...
- `sandbox` — ограничивает определённые действия и функции встраиваемого документа. Если просто указать атрибут `sandbox`, то он применит сразу все возможные ограничения. Либо можно указать нужные ограничения в любом порядке через пробел:
  ...
  !!! warning "Одновременное использование нескольких значений"
...
!!! tip "Использование атрибута `title`"
...
!!! warning "Предупреждение о безопасности"
    ...
    🛠 На практике `<iframe>` используется нечастно, как правило, только когда надо, например, встроить видео с YouTube.
...
### Элемент `object`
Тег `<object>` используется для встраивания на страницу других страниц, документоподобного содержимого (карты типа Google Maps) или мультимедийных элементов (PDF, Flash, видео- или аудиоплееров). Этот тег может использоваться вместо устаревших `<embed>` и `<applet>`.[^object]
...
[Демо](../samples/css/object.html)

Если браузер не поддерживает тип встроенного файла, то он покажет вложенное содержимое в качестве фолбэка (резервного содержимого). В нашем примере, если встраивание PDF-инструкции не поддерживается, то мы увидим ссылку на её скачивание.[^object]

Доступные атрибуты:

- `data` — ссылка на встраиваемый ресурс. Содержит URL-адрес или относительный путь к встраиваемому файлу.
- `type` — MIME-тип подгружаемого объекта (например, application/pdf для документов или image/png для картинок). Помогает браузеру понять, сможет ли он отобразить объект.
- `name` — уникальное имя объекта, которое позволяет ссылаться на него через скрипты (JavaScript) или связывать с элементами форм.
- `usemap` — атрибут для связи объекта с тегом `<map>`.
- `width` — ширина объекта (отображаемой области в пикселях);
- `height` — высота объекта (отображаемой области в пикселях);
- `form` — идентификатор формы. Позволяет установить связь с одной или несколькими формами, с которой данный объект должен быть связан для отправки данных.

Ранее использовались такие параметры, как `archive`, `classid`, `codebase`, `codetype`, `declare` и `standby` для подключения апплетов и устаревших плагинов. В современном коде их применять не следует

### Элемент `embed`
Тег `<embed>` позволяет встраивать на веб-страницу содержимое в сторонних форматах, которые браузер может отобразить с помощью плагинов, — внешние приложения, такие как аудио, видео, анимацию, PDF-файлы, и многое другое. Чаще всего использовался для видео- и аудиоконтента.
...
!!! info "Отсутствие плагина"
    ...
    ![No plugin](../img/no-plugin-1200w.webp)
...
[Демо](../samples/css/embed.html)

Тег `<embed>` создаёт своеобразное окошко, в котором отображается подгружаемый контент.

Чем-то его поведение похоже на теги `<object>` и `<iframe>`, но в отличие от тегов `<object>` и `<iframe>`, тег `<embed>` не поддерживает альтернативный контент.[^embed]

> Тег `<embed>` не считается семантическим, так как не придает никакого конкретного значения содержимому.

!!! warning "Альтернативные способы"
...
Помимо этих атрибутов можно использовать любые глобальные атрибуты.[^embed]

### Выбор механизма встраивания
Обычно, когда нужно встроить целый документ или стороннюю страницу (например, плеер YouTube, виджет или другую веб‑страницу) используют `<iframe>`: он обеспечивает хорошую изоляцию, широкую поддержку браузеров и более гибкие средства безопасности (`sandbox`, разделение origin). Элемент `<object>` подойдёт, когда необходимо вставить конкретный внешний ресурс (например, PDF или другой медиафайл) и обеспечить формальное резервное содержимое (*fallback*); он даёт более семантичную обработку фолбэка по сравнению с `<embed>`. <embed> — более простой и старый тег для контента через плагины или «сырых» медиаресурсов: он создаёт окно для контента, но предоставляет меньше контроля и семантики, чем `<object>` или `<iframe>`.

Таким образом, для встраивания внешних HTML-документов и медиаплееров следует стоит отдавать предпочтение `<iframe>`, а `<object>` оставить для документоподобных ресурсов, когда нужен доступный фолбэк. `<embed>` во многом считается устаревшим; некоторые современные браузеры постепенно отказываются от его поддержки, поэтому лучше использовать его только при отсутствии надёжной альтернативы. Также по возможности следует применять нативные HTML5‑элементы (`<video>`, `<audio>`) для медиа и всегда обеспечивайте корректные MIME‑типы, доступность, размеры и механизмы безопасности (например, sandbox) для встраиваемого содержимого.
...
Тег `<picture>` адаптирует изображения под разные устройства и поддерживаемые форматы и используется, когда для разных устройств или вариантов отображения нам нужны разные картинки.
...
Тег позволяет подать разные файлы изображений в зависимости от условий: ширины экрана, разрешения устройства, поддерживаемых форматов (WebP, AVIF и т. п.) или других медиа‑атрибутов. Внутри `<picture>` обязательно должен быть тег `<img>` — он служит финальным запасным вариантом и обязательным элементом для доступности (атрибут `alt`).

Пример с медиа‑запросами и форматом:
```html
<picture>
  <source srcset="image-800.avif" type="image/avif" media="(min-width: 1200px)">
  <source srcset="image-800.webp" type="image/webp" media="(min-width: 1200px)">
  <source srcset="image-480.webp" type="image/webp" media="(max-width: 1199px)">
  <img src="image-480.jpg" alt="Пейзаж на закате">
</picture>
```

Браузер последовательно проверяет источники сверху вниз и использует первый поддерживаемый `<source>`. Если ни один `<source>` не подходит, загружается файл из `<img>`.

Ретина/высокое DPI:
```html
<picture>
  <source srcset="photo-1x.jpg 1x, photo-2x.jpg 2x" media="(min-resolution: 2dppx)">
  <img src="photo-1x.jpg" alt="Портрет">
</picture>
```

Когда использовать `<picture>`:

- нужно раздавать разные файлы в зависимости от размера экрана (например, портретная/ландшафтная композиция);

- хотите отдать предпочтение современным форматам (AVIF/WebP) при их поддержке и подставить JPEG/PNG как fallback;

- нужен контроль над тем, какая версия изображения загружается для разных условий, чтобы оптимизировать трафик и производительность;

Когда не нужен:

- для простого адаптивного масштабирования одного файла достаточно атрибута `srcset` и `sizes` в теге `<img>`.

- если различия между версиями минимальны, использование `<picture>` излишне усложняет код.

Рекомендации по производительности и доступности:

- всегда указывайте `alt` в `<img>`;

- минимизируйте количество и размер вариантов изображений;

- используйте современные форматы (AVIF/WebP) для экономии трафика, но оставляйте надёжный JPEG/PNG fallback.

- для корректной работы с отложенной загрузкой используйте атрибуты `loading="lazy"` на `<img>` (поддерживается большинством современных браузеров).

- тестируйте последовательность `<source>` и медиа‑условия в разных браузерах и устройствах, чтобы убедиться, что загружается ожидаемая версия.

Иллюстрация типичного сценария:

- мобильное устройство получит лёгкий WebP/JPEG с меньшим разрешением.

- десктоп с хорошим подключением — AVIF/WEBP с высоким разрешением.

- браузер без поддержки современных форматов — JPEG/PNG из `<img>`.
...
!!! warning "Поддержка экспериментальных значений"

    Проверяйте поддержку экспериментальных значений `smooth` и `high-quality` на [Can I use](https://caniuse.com/?search=image-rendering).[^image-rendering]
...
[Демо](../samples/css/image-rendering.html)
...
- **Размеры файлов**. Изображения 2x/3x значительно больше по весу; используйте их только там, где визуальная разница заметна (фото, крупные иллюстрации, UI‑иконки при необходимости). Для декоративных или мелких картинок достаточно векторных форматов (SVG).

- **Форматы**. Комбинируйте дескрипторы плотности с современными форматами (AVIF/WebP) через `<picture>` или `srcset` по URL, чтобы снизить трафик: современные форматы дают заметную экономию при сохранении качества.

- **Атрибуты `width`/`height`**. Указывайте реальные CSS‑размеры через атрибуты `width` и `height` или через CSS; это помогает браузеру зарезервировать место и избежать сдвигов контента (CLS).

- **`srcset` и `sizes`**. Дескрипторы `1x`/`2x`/`3x` работают для указания мультирезолюций по плотности. Если вам нужно управлять выбором размера в зависимости от реальной зоны отображения (ширина в CSS), используйте комбинацию `srcset` с пробельными дисплейными дескрипторами и атрибутом `sizes`, либо применяйте `<picture>` с медиавыражениями.

- **Кэширование**. Серверные заголовки кеширования и CDN помогают уменьшить задержки при доставке больших HiDPI‑файлов.

- **Автоматизация**. Генерируйте 1x/2x/3x версии автоматически при сборке проекта (imagemin, sharp, build pipelines). Это снижает ручную работу и позволяет задавать точные качества сжатия для каждого варианта.

- **Тестирование**. Проверяйте поведение на реальных устройствах с разной DPR и в разных браузерах; некоторые старые браузеры могут по‑разному интерпретировать дескрипторы плотности.

- **Компромисс качества/трафика**. Для большинства случаев достаточно 2x; 3x стоит применять выборочно (пользовательские фотографии, важные крупные иллюстрации). Можно также использовать **прогрессивную загрузку**: сначала загружается лёгкая версия, затем — более точная (поддерживается JavaScript‑подходами).

Пример сочетания типов и плотности через `<picture>`:
```html
<picture>
  <source type="image/avif" srcset="photo-1x.avif 1x, photo-2x.avif 2x">
  <source type="image/webp" srcset="photo-1x.webp 1x, photo-2x.webp 2x">
  <img src="photo-1x.jpg" srcset="photo-1x.jpg 1x, photo-2x.jpg 2x" alt="Пейзаж">
</picture>
```

Базовые рекомендации:

- Используйте дескрипторы `2x`/`3x` для HiDPI там, где это заметно.

- Комбинируйте с современными форматами и <picture> для оптимизации трафика.

- Указывайте размеры, применяйте кэширование и автоматизируйте генерацию вариантов.
...
## Заключение
Заменяемые элементы — это HTML‑элементы, содержимое которых не формируется из вложенного DOM, а отображается браузером на основе внешнего ресурса: классические примеры — `<img>`, `<video>`, `<iframe>`, `<object>`, `<embed>`, а также элементы формы типа `<input>` и `<textarea>`. Для таких элементов браузер отвечает за получение ресурса, расчёт внутреннего соотношения сторон и отображение пикселей, поэтому у них есть особые правила по размеру, выравниванию и поведению в потоке документа. Понимание того, что элемент «замещаемый», важно при верстке: это влияет на обработку CSS‑свойств (например, `object‑fit`, intrinsic sizing), на необходимость явно задавать атрибуты `width`/`height`, и на способы обеспечения доступности (`alt`, `title`, подписи).

Адаптивное использование заменяемых элементов включает выбор правильного тега и техники доставки ресурса под конкретный контекст: для изображений — `<img>` с `srcset`/`sizes` или `<picture>` с `<source>` для art‑direction и подстановки форматов; для медиа — нативные `<video>`/`<audio>` с несколькими источниками; для встраивания страниц и виджетов — `<iframe>` с sandbox и правильными заголовками безопасности; для документоподобных ресурсов — `<object>` с фолбэком. Ключевые практики — отдавать предпочтение нативным элементам (вместо устаревших плагинов), комбинировать дескрипторы плотности и медиа‑условия для HiDPI и разных экранов, указывать размеры чтобы избегать сдвигов контента, обеспечивать доступные подписи и фолбэки, и оптимизировать форматы (AVIF/WebP, прогрессивный JPEG). Такой подход минимизирует трафик, повышает производительность и совместимость, одновременно гарантируя корректное отображение на широком наборе устройств и браузеров.
...
MIME-тип (*internet media type*, иногда *content type*)
: строка, отправляемая вместе с файлом, определяющая его формат (например, передаваемый аудиофайл может быть помечен как `audio/ogg` тип, а изображение — `image/png`). Состоит из основного типа (*type*) и подтипа (*subtype*), разделенных косой чертой, например, `image/png`, `audio/ogg` или `application/pdf`. Типы по умолчанию: `text/plain` (для текста) и `application/octet-stream` (для бинарных данных, если тип неизвестен).

SMTP (Simple Mail Transfer Protocol)
: стандартный сетевой протокол, предназначенный для отправки электронной почты. Он работает в архитектуре клиент-сервер, обеспечивая передачу сообщений от почтового клиента отправителя на почтовый сервер и далее между серверами до получателя. SMTP используется только для отправки, в то время как за получение отвечают протоколы POP3 или IMAP.

SVG-sprite
: набор иконок, объединённых в один SVG-файл, где каждая иконка хранится в отдельном символе. Такой способ особенно удобен, когда на странице используется много одинаковых графических элементов.

Автономное содержимое (*self-contained content*)
: блоки элементов, представляющие самостоятельные фрагменты информации: иллюстрации, графики, примеры кода, цитаты и другие объекты, которые логически отделимы от основного потока текста и при необходимости могут быть перемещены в документе без потери смысла. Такие фрагменты часто сопровождаются отдельными подписями или примечаниями и служат для наглядного дополнения или иллюстрации основной мысли, не нарушая структуру основного контента.
...
: ресурсы третьих сторон или отдельные файлы (видео, аудио, PDF, презентации, интерактивные элементы и т. п.), которые можно разместить прямо на веб‑странице, чтобы предоставить пользователю мультимедийный или интерактивный опыт без перенаправления на другие сайты. В HTML для этой цели используются элементы вроде `<object>`, `<embed>` и `<iframe>`, каждый из которых имеет свои особенности по поддержке форматов, обработке фолбэков и взаимодействию с плагинами или встроенными проигрывателями. Правильный выбор и настройка таких тегов помогают обеспечить совместимость, доступность и удобство использования содержимого.

Заменяемые (замещаемые) элементы (*replaced elements*)
: элементы, чье представление выходит за рамки модели форматирования CSS. Их содержимое формируется внешними ресурсами или пользовательским агентом (браузером, а не обычным HTML-контентом или текстом, управляемым CSS) и отображается внешним объектом, поэтому CSS обычно не контролирует внутреннее отображение этого элемента.

Замещаемый элемент (*replaced element*) — это HTML-элемент, отображающий внешний ресурс, например, картинку или видео. Браузер берёт содержимое не из HTML-разметки, а подставляет готовое «встроенное» представление. В CSS мы можем менять размер и расположение такого элемента, но не управлять его внутренним содержимым.
...
[^object]: [object](https://doka.guide/html/object/)
[^iframe]: [iframe](https://doka.guide/html/iframe/)
[^embed]: [embed](https://doka.guide/html/embed/)
[^alt]: [alt](https://doka.guide/html/alt/)
[^image-rendering]: [image-rendering](https://doka.guide/css/image-rendering/)
[^background-image]: [background-image](https://doka.guide/css/background-image/)
[^background-clip]: [background-clip](https://doka.guide/css/background-clip/)
[^image-set]: [image-set](https://doka.guide/css/image-set/)
[^background-origin]: [background-origin](https://doka.guide/css/background-origin/)
[^object-fit]: [object-fit](https://doka.guide/css/object-fit/)
[^background-size]: [background-size](https://doka.guide/css/background-size/)
[^background-position]: [background-position](https://doka.guide/css/background-position/)
[^object-position]: [object-position](https://doka.guide/css/object-position/)
[^object-view-box]: [object-view-box](https://doka.guide/css/object-view-box/)
[^MIME_type]: [MIME-тип](https://developer.mozilla.org/ru/docs/Glossary/MIME_type)
[^MIME]: [MIME](https://ru.wikipedia.org/wiki/MIME)
[^Список_MIME-типов]: [Список MIME-типов](https://ru.wikipedia.org/wiki/%D0%A1%D0%BF%D0%B8%D1%81%D0%BE%D0%BA_MIME-%D1%82%D0%B8%D0%BF%D0%BE%D0%B2)
[^2.10]: [Элементы figure и figcaption](https://metanit.com/web/html5/2.10.php)
[^2.11]: [Фреймы](https://metanit.com/web/html5/2.11.php)
[^object]: [object — внешние объекты](https://htmlacademy.ru/blog/html-tags/object)
[^embed]: [embed](https://htmlacademy.ru/blog/html-tags/embed)
