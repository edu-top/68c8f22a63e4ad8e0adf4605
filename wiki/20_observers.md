...
- [Наблюдатели](#наблюдатели)
  - [MutationObserver: наблюдатель за изменениями](#mutationobserver-наблюдатель-за-изменениями)
    - [Синтаксис](#синтаксис)
    - [Использование для интеграции](#использование-для-интеграции)
    - [Использование для архитектуры](#использование-для-архитектуры)
    - [Пример динамической подсветки синтаксиса](#пример-динамической-подсветки-синтаксиса)
...
Сначала мы познакомимся с синтаксисом, а затем разберём примеры использования.[^mutation-observer]
...
Идём дальше. Представим, что мы собираемся динамически подгружать материалы с сервера. Позже в учебнике мы изучим [способы для этого](https://learn.javascript.ru/fetch). На данный момент имеет значение только то, что мы получаем HTML-статью с веб-сервера и показываем её по запросу:
```js
...
[^mutation-observer]: [MutationObserver: наблюдатель за изменениями](https://learn.javascript.ru/mutation-observer)
