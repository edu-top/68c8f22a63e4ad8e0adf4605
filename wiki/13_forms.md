...
- [События элементов управления](#события-элементов-управления)
  ...
  - [События: change, input, cut, copy, paste](#события-change-input-cut-copy-paste)
    ...
    - [События: cut, copy, paste](#события-cut-copy-paste)
      ...
      - [Проверка жеста пользователя](#проверка-жеста-пользователя)
    - [Итого](#итого-2)
    - [Задачи](#задачи-2)
  ...
  - [Сброс формы: reset](#сброс-формы-reset)
    - [Событие reset](#событие-reset)
    - [Метод reset()](#метод-reset)
    - [Итого](#итого-3)
    - [Задачи](#задачи-3)
  ...
  - [Выделение](#выделение)
    ...
    - [Selection](#selection)
      - [Свойства Selection](#свойства-selection)
      - [События при выделении](#события-при-выделении)
        - [Демо отслеживания выделения](#демо-отслеживания-выделения)
        - [Демо получения выделения](#демо-получения-выделения)
      - [Методы Selection](#методы-selection)
    - [Выделение в элементах форм](#выделение-в-элементах-форм)
      ...
      - [Пример: вставка на месте курсора](#пример-вставка-на-месте-курсора)
    - [Сделать что-то невыделяемым](#сделать-что-то-невыделяемым)
    - [Ссылки](#ссылки-1)
    - [Итого](#итого-4)
  ...
  - [Практическая работа. Создание депозитного калькулятора](#практическая-работа-создание-депозитного-калькулятора)
    - [Задание](#задание-2)
  - [Практическая работа. Модальное диалоговое окно с формой](#практическая-работа-модальное-диалоговое-окно-с-формой)
    - [Задание](#задание-3)
  - [Практическая работа. Депозитный калькулятор](#практическая-работа-депозитный-калькулятор)
    - [Задание](#задание-4)
  - [Практическая работа. Модальное диалоговое окно с формой](#практическая-работа-модальное-диалоговое-окно-с-формой-1)
    - [Задание](#задание-5)
...
### Формы и их элементы
Один из способов взаимодействия с пользователями представляют html-формы. Например, если нам надо получить от пользователя некоторую информацию, мы можем определить на веб-странице форму, которая будет содержать текстовые поля для ввода информации и кнопку для отправки. И после ввода данных мы можем обработать введенную информацию.[^10.1]
...
Также мы можем при необходимости в процессе отправки изменить адрес, на который направляются данные:
```js
function sendForm(e){
...
В данном случае, если длина текста меньше 3 символов, то текст отправляется, только теперь он направляется по адресу `PostForm`, поскольку задано свойство `action`:
```js
document.search.action="PostForm";
```
...
Поскольку событие `keypress` возникает при нажатии на клавиши для печатаемых символов, то такие символы отображаются в текстовом поле. Однако есть и другие клавиши, которые оказывают влияние на текстовое поле, но они не дают отображаемого символа, поэтому не отслеживаются событием `keypress`. К таким клавишам относится клавиша Backspace, которая удаляет последний символ. И для ее отслеживания также обрабатываем событие keydown. В обработчике `keydown` удаляем из строки в блоке `printBlock` последний символ.[^10.3]
...
Метод `item()` — это стандартный способ доступа к элементам коллекций `NodeList` или `HTMLCollection` в JavaScript, эквивалентный использованию квадратных скобок. Метод `item(index)` возвращает `null`, если индекс выходит за границы коллекции.
...
##### Проверка жеста пользователя

<dfn title="проверка жеста пользователя">Проверка жеста пользователя</dfn> (*user gesture*) — это механизм безопасности браузеров, который гарантирует, что доступ к чувствительным функциям (буфер обмена, уведомления, полноэкранный режим, автопроигрывание) возможен только из прямого действия пользователя.

Что считается жестом пользователя?
...
Как проверить наличие жеста пользователя?
```js
...
### Сброс формы: reset
Событие `reset` и метод `reset()` в JavaScript связаны с обработкой HTML-форм и позволяют сбрасывать их состояние.

#### Событие reset
Событие `reset` срабатывает на элементе `<form>`, когда форма возвращается к исходным значениям полей — обычно при нажатии кнопки `<button type="reset">` или программном вызове метода `reset()`. Оно полезно для выполнения дополнительных действий, таких как очистка ошибок валидации, логирование или сброс стилей.

Обработчик добавляется так:
```js
form.addEventListener('reset', function(event) {
  console.log('Форма сброшена');
  // Можно предотвратить: event.preventDefault();
});
```

Вы можете отменить стандартный сброс с помощью `preventDefault()` для подтверждения у пользователя.

#### Метод reset()
Метод `HTMLFormElement.reset()` сбрасывает все поля формы (`input`, `select`, `textarea`) к их начальным значениям из HTML, не отправляя данные на сервер. Это эквивалентно клику на кнопку `reset` и не требует перезагрузки страницы.
...
Метод не возвращает значение и работает мгновенно, восстанавливая атрибуты вроде `disabled`

 Согласно документации MDN, метод `HTMLFormElement.reset()` восстанавливает стандартные значения полей формы и идентичен действию кнопки `<button type="reset">`, которая генерирует событие `reset`. При программном вызове `form.reset()` это событие также срабатывает синхронно перед завершением сброса.

!!! warning "Метод reset() вызывает событие reset"

    В отличие от `form.submit()`, который не генерирует событие `submit`, метод `form.reset()` генерирует событие `reset`.

    Ключевое различие
    - `form.submit()`: обходит обработчики события `submit`, предполагая, что разработчик уже выполнил валидацию и обработку. Форма отправляется напрямую.

    - `form.reset()`: вызывает событие `reset` синхронно, точно как кнопка `<button type="reset">`. Обработчики `form.addEventListener('reset', ...)` срабатывают.

    Согласно MDN и спецификации HTML, `reset()` "выполняет действие идентичное нажатию кнопки "reset", включая генерацию (*dispatch*) события `reset`. Это всплывающее событие, отменяемое через `preventDefault()`.

    Здесь обработчик выполняется при вызове `reset()`, в отличие от `submit()`:

    ```js
    const form = document.querySelector('form');
    form.addEventListener('reset', () => console.log('Событие reset сработало!'));
    form.reset(); // Выведет сообщение
    ```

Событие `reset` — это bubbling event, поэтому его можно перехватить на родительских элементах. В обработчике можно вызвать `event.preventDefault()`, но это не остановит уже запущенный сброс от `reset()`.

#### Итого
Для элементов управления в JavaScript предусмотрены следующие основные события: `focus`/`blur` и всплывающие `focusin`/`focusout` при получении/потере фокуса, `input`, реагирующее на любое изменение значения в реальном времени, `cut`/`copy`/`paste` для буфера обмена, `submit` при отправке формы, `reset` при сбросе формы. Эти события обеспечивают отзывчивое взаимодействие с формами, валидацию и доступность; большинство из них являются всплывающими, что упрощает делегирование обработчиков.
...
- **`maxlength`** задает максимальную длину строки (в символах)

- **`minlength`** задает минимальную длину строки (в символах)

- **`pattern`** задает регулярное выражение для проверки формата значения (например, email, телефон)

- **`type`** определяет ожидаемый тип данных (`email`, `url`, `tel`, `number`, `date` и др.), автоматически активируя соответствующую валидацию

- **`multiple`** разрешает множественный выбор (для `<select>` и `<input type="email">`)

- **`step`** задает шаг изменения значения (для `number`, `range`, `date`)

- **`max`** и **`min`** также работают с датами и временем при `type="date"`, `type="time"` и т.д.

Дополнительно форма может содержать атрибуты `novalidate` (отключает браузерную валидацию) и элементы `<datalist>` для подсказок. Состояние валидации проверяется через свойства `validity`, `validationMessage` и методы `checkValidity()`, `reportValidity()`.
...
![Нативная валидация в HTML5](../img/validation3_cp.png)
...
![Получение информации о валидации полей формы в JavaScript](../img/validation1_cp.png)
...
![Вывод ошибок валидации полей формы в JavaScript](../img/validation2_cp.png)
...
![Установка ошибок валидации в JavaScript](../img/validation4_cp.png)
...
#### Заключение
HTML5 Constraint Validation API — эффективный встроенный инструмент для валидации форм, покрывающий абсолютное большинство задач, опираясь на специальные атрибуты (`required`, `minlength`, `pattern` и т.п.) и типы элементов (`"email"`, `"url"` и т.п.). Для стилизации состояния проверки удобно использовать встроенные стили `:invalid`/`:valid`. Для управления валидацией используются методы `checkValidity()` и `reportValidity()`, подключая обработчики `blur`+`input`+`submit` с показом сообщений, хранящихся в свойстве `validationMessage`. Более того, HTML5 Constraint Validation API позволяет устанавливать собственные ошибки с помощью метода `setCustomValidity("")`, что делает данный инструмент идеальным решением, часто позволяющим обойтись без внешних библиотек.
...
- `range.setEnd(p, 2)` – расширяет диапазон до 2-го (но не включая его) дочернего элемента тега `<p>` (это текстовый узел `" and "`, но так как конец не включён, последний включённый узел – это тег `<i>`).
...
![Range example](../svg/range-example-p-2-b-3.svg)
...
- начинается со второй позиции первого дочернего узла тега `<p>` (захватываем всё, кроме первых двух букв "Ex**ample:** ")
- заканчивается на 3 позиции первого дочернего узла тега `<b>` (захватываем первые три буквы «**bol**d», но не более):
...
![Range example](../svg/range-example-p-2-b-3-range.svg)
...
Выделение может включать ноль или более диапазонов. По крайней мере, так утверждается в [Спецификации Selection API](https://www.w3.org/TR/selection-api/). На практике же выделить несколько диапазонов в документе можно только в Firefox, используя <kbd>Ctrl</kbd>+`click` (<kbd>Cmd</kbd>+`click` для Mac).

Ниже представлен скриншот выделения с 3 диапазонами, сделанный в Firefox:

![Selection Firefox](../svg/selection-firefox.svg)
...
Дополнительные свойства выделения:

- `baseNode` и `baseOffset` – направленные аналоги `anchorNode`/`anchorOffset`, где `base` всегда является началом направления ввода (слева направо или справа налево).
​
- `extentNode` и `extentOffset` – конец направления ввода, противоположный `base`.

- `collapsed` – синоним `isCollapsed`, `true` когда `anchorNode === focusNode` И `anchorOffset === focusOffset`.

!!! info "Конец выделения может быть в документе до его начала"
    ...
    ![Selection direction forward](../svg/selection-direction-forward.svg)
    ...
    ![Selection direction backward](../svg/selection-direction-backward.svg)
...
!!! info "Чтобы что-то выделить, сначала снимите текущее выделение"
...
![Selection example](../img/selection_05-1.png)

![Selection example](../img/selection_05-2.png)

![Selection example](../img/selection_05-3.png)

#### Сделать что-то невыделяемым
Существуют три способа сделать что-то невыделяемым:

1. Использовать CSS-свойство `user-select: none`.
...
Мы можем использовать это, как для того, чтобы получить позицию курсора, так и чтобы переместить его, установив `elem.selectionStart` и `elem.selectionEnd`.[^selection-range]

#### Заключение
Выделение текста в веб-разработке играет ключевую роль в создании интуитивных интерфейсов, где объекты `Selection` и `Range` в DOM API обеспечивают гибкий контроль над фрагментами текста, позволяя разработчикам точно определять начало, конец и содержимое выделенного диапазона через методы вроде `getSelection()` и `Range.setStart/end()`. В элементах форм, таких как `<input>` и `<textarea>`, выделение реализуется проще с помощью свойств `selectionStart`, `selectionEnd` и методов `setSelectionRange()`, что упрощает задачи вроде автозаполнения, валидации или копирования фрагментов. В итоге, эффективное использование этих механизмов не только повышает пользовательский опыт, минимизируя ручной ввод, но и открывает двери для продвинутых функций, таких как кастомные редакторы или drag-and-drop выделения, делая веб-приложения более динамичными и доступными.
...
2. **Редактируемая ячейка таблицы**
    ...
    - По клику ячейка должна стать «редактируемой» (`textarea` появляется внутри), при этом можем изменять HTML. Изменение размера ячейки должно быть отключено.
    - Кнопки OK и ОТМЕНА появляются ниже ячейки и, соответственно, завершают/отменяют редактирование.
    - Только одну ячейку можно редактировать за один раз. Пока `<td>` в «режиме редактирования», клики по другим ячейкам игнорируются.
...
### Практическая работа. Модальное диалоговое окно с формой

#### Задание

Создать функцию `showPrompt(html, callback)`, которая выводит форму с сообщением (`html`), полем ввода и кнопками `OK/ОТМЕНА`.
...
### Практическая работа. Депозитный калькулятор

#### Задание

Создать интерфейс, позволяющий ввести сумму банковского вклада и процент, а затем рассчитать, какая это будет сумма через заданный промежуток времени. Любое изменение введённых данных должно быть обработано немедленно.

Пример интерфейса:

![Deposit calculator](../img/deposit_calculator.png)
...
Проверка жеста пользователя (*user gesture*)
