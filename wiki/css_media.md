...
Именно поэтому для них нужны специальные свойства вроде `object-fit` и `object-position` — обычные `width`/`height` лишь задают контейнер, а содержимое подстраивается отдельно. Например, войство `object-fit` применяется для управления тем, как заменяемый элемент (`img`, `video`) вписывается в свой контейнер (`cover`, `contain`, `fill`).
