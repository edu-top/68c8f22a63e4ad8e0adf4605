...
Свойство `background-clip` решает, должен ли фон заполнять площадь всего элемента или не выходить за рамки контентной области, а `background-origin` — управляет тем, какую область фоновая картинка будет занимать внутри элемента.
